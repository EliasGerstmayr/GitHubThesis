...
This scale defines the ``strong-field frontier'', which is complementary to the ``energy frontier'' \cite{Scandale2014_ppColliders}, that is traditionally explored with accelerators. 
...
The intensity is the unit energy delivered per unit area and time, commonly given in units $\mathrm{W/cm^2}$. The peak intensity, $I_0$, for a laser pulse which is Gaussian in time and space is given by \cite{PoderThesis}
\begin{equation}
...
\end{figure}
 ...
 In the seminal E-144 experiment at SLAC a 46.6 GeV electron beam was collided with a laser of intensity $a_0 = 0.3$ which enabled the production of high energy radiation but also the production of particles from the nonlinear Breit-Wheeler process. 
...
Critical field  \cite{Sauter1931_ECRIT,Heisenberg1936_ECRIT}. We see that we can reach these values by either reaching high $a_0$ or high $\gamma$, such that this is a field that is approached from two sides, conventional accelerators and multi-petawatt laser facilities. In this so-called \textit{nonperturbative strong-field regime of QED} ($I'\!\gtrsim\!I_{cr}$) the recoil of emitted photons significantly perturbs electron/positron trajectories (strong-field radiation reaction) and prolific electron-positron pair production becomes substantial (vacuum breakdown). 
\vspace{\baselineskip}
...
These facilities will open an exciting new research field within high-energy-density physics (HEDP) \cite{Grismayer2017_SeededQEDCascades,Ridgers2012_DENSE,Nerush2011_LaserAbs_PAIRS}. This is also at the focus of the US community (see reports from Brightest Light \cite{National2018_BrightestLight} and Frontiers of Plasma Science \cite{DoE2015_FES}).
\vspace{\baselineskip}
...
There is a wide range of phenomena and research opportunities in the approach and the excess of $\eta = 1$ \cite{DiPiazza2012_ICS}. This ranges from a detailed study of Compton scattering: radiation production meaning compton scattering from classical \cite{Yan2017_ICS} and the perturbative quantum \cite{Bula1996_RR} to the non-perturbative strong-field regime \cite{Bula1996_RR,TaPhuoc2012_ICS,Chen2013_ICS,Powers2014_ICS,Sarri2014_ICS,Khrennikov2015_ICS,Mackenroth2013_nlCompton}. 
\vspace{\baselineskip}

The high field strengths also give rise to significant radiation reaction, which is the knock back force a particle feels when radiating. Radiation reaction is a very fundamental process but surprisingly there is no practicable solution for this so far covering several regimes. Initial attempts resulted in runaway solutions. There are classical descriptions, most notably the Landau-Lifschitz equation, but they do not have an intrinsic cut-off energy and also do not consider the stochasticity. In the quantum regime the emission is very energetic and stochastic, here the radiation reaction force is the result of many incoherent discrete emission events (high energy). The numerical methods are evolving but are currently typically intrinsically semi-classical and rely on locally constant fields relative to the emission time of the photons (LCFA) \cite{DiPiazza2018_LCFA,DiPiazza2019_LCFANUM}.
At high energies per single emission it becomes quantum radiation reaction or strong field radiation reaction \cite{Harvey2017_QUENCHING,Blackburn2019_RRReview,Blackburn2014_QRR,Ritus1985_QRR,Ridgers2017_QRR,Shen1972_STRAGGLING,Thomas2012_LL,Vranic2014_RR,Dinu2016_QRR}
In intense interactions it has also been proposed that beams obtain spin polarisation \cite{DelSorbo2017_SPIN,Seipt2018_SPIN,Li2019_singleshot_beampol}.
Spin polarisation is important for particle physics (spin dependent cross sections). At the same time a detailed study of radiation reaction is also important to develop mitigation strategies for future accelerators.

There are different geometries to probe radiation reaction besides colliding relativistic particles, although this is where the fields are enhanced the most. They also become relevant in standing wave configurations and laser-solid interactions.
It can also be probed in aligned crystals \cite{DiPiazza2017_CrystalRR,Artru1994_CHANNELING,Katkov1998_CHANNELING,Wistisen2018_RR,Wistisen2019_RR}.
\vspace{\baselineskip}

These interactions also enable the study of pair production, astrophysical equivalent of cascades and showers cascades and showers, multi-step  \cite{Bulanov2013_Cascade,Grismayer2017_SeededQEDCascades,Sokolov2010_QEDPAIRS}.
...
The intense interactions described above produce highly energetic radiation which can be used in conjunction with a second laser or different light source to enable detailed studies of light-light interactions (AGAINST SUPERPOSITION PRINCIPLE): Examples are pair production from nonlinear BW \cite{DiPiazza2016_nonlinearBW} and linear Breit-Wheeler (perutrbative and nonperturbative), to the intense region of vacuum pair production (BREAKING THE VACUUM \cite{Cartlidge2018_SEL}). Then nonlinear vacuum interactions like light-light scattering which is classically forbidden (superposition principle). We can realise photon-photon colliders, and see an analogue to nonlinear physics and optics. Now the vacuum acts as a medium. We investigate vacuum birefringence \cite{King2016_VB,Nakamiya2017_VB} and vacuum dichroism \cite{Bragin2017_VBVD}. Similarly, there are vacuum recollision processes \cite{Meuren2015_HERecollision,Kuchiev2007_Recollision}, which are reminiscent of atomic recollision.
This also can be used to probe physics beyond the standard model in photon-photon \cite{Beresford2019_PhPh_SLEPTONS,Knapen2017_PhPh_AXIONLIKE,Baldenegro2018_PhPh_AXIONLIKE}.
...
Reaching supercritical $\eta > 1$ fields is difficult but we can reach nonperturbative physics \cite{Yakimenko2019_NONPERTURB,Blackburn2019_SUPER,Baumann2019_NONPERTURB}. 
