...
Conventional accelerators with access to high-intensity lasers will also be able to probe this regime and will benefit from their superior beam quality and stability, but will have to overcome challenges arising from synchronising the electron beam and the laser pulse. Moreover, beam sizes are typically larger such that a large background of radiation from low-intensity interactions is expected.
...
At $\eta \sim 1$ we gain access to pair production from the vacuum via \textit{Schwinger pair production}, \textit{vacuum tunnel ionisation} or \textit{vacuum breakdown} \cite{Schwinger1951_VACPAIRS,Brezin1970_VACPAIRS,Popov1971_VACPAIRS}. 
