...
One method to detect and characterise hard X-rays and gamma rays is using blocks or slabs of scintillating material extending in the propagation direction of the radiation. The response of the material, i.e. how photons or particles deposit energy and how the scintillator reacts, is then modelled using Monte-Carlo codes, e.g. GEANT4 \cite{GEANT4} or MCNP \cite{Goorley2012} or FLUKA (REF), and compared to the response measured. In general, the penetration depth and the energy deposited in the crystals are proportional to the energy of the radiation transmitted. To discriminate the spectrum even further, different materials can be overlaid as the response varies from material to material. This might be more complicated for very high energetic gamma rays as the cross section is almost identical for most materials at the MeV scale.
...
\caption{Examples of raw response recorded on CsI scintillator arrays. Top: Taken with RAL stack and lead collimator. Round circles due to aperture in the steel plate. Bottom: Imaging one side of the front dual-axis stack without collimator setup and extended stack.}
...
\end{tabular}
\caption{Overview of scintillator arrays used in the experiments discussed in this thesis and their properties, namely the array size, divider and front plate material. All crystals used are made of caesium-iodide doped with thallium with the dimensions $5 \times 5 \times 50$ mm.}
...
\caption{GEANT representations of the two scintillator arrays used in the experiments.}
\end{figure}
...
\caption{Left:Two examples of electron spectra measured in an experiment and used as input for GEANT bremsstrahlung simulation. Right: Corresponding simulated bremsstrahlung produced using 1 mm of bismuth as converter material.}
...
\caption{Left: Measured experimental response of the detector for bremsstrahlung (blue) and the corresponding simulated response (orange). Right: Correction factor for the crystal rows obtained by dividing the two curves on the right.}
\end{figure}
...
\caption{GEANT simulations for detector response of BW detector with GP. Left: energy deposited (y-axis) in longitudinal crystal row (x-axis) per photon of variable energy (c-axis) from 1 to 500 MeV. Right: Longitudinal crystal with maximum energy deposition with respect to the energy of the incoming photon for different detector configurations.}
...
\caption{Gamma Profile screen (left) and corresponding pixellated detector response from integrated vertically (middle) and horizontally (right) for symmetric bremsstrahlung source (top) and extended betatron radiation (bottom). The transverse crystal indicates pointing and in case of the extended source also the spatial extent of the source.}
