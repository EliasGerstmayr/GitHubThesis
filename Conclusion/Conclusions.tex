...
\subsubsection{Phenomena: Precision Measurement of RR at high-intensities (quantum corrections)}
...
\subsubsection{Phenomena: Very high-intensity collisions with pair production}

\cite{Blackburn2017_pairs}

\subsubsection{Phenomena: Very high-intensity collisions with spin polarisation measurement}

\cite{SokolovTernov1964_POL}
\cite{DelSorbo2017_SPIN,Seipt2018_SPIN}

\subsubsection{Phenomena: QED Cascades}

\subsubsection{Phenomena: dense QED plasma}

\subsubsection{Phenomena: Schwinger pair production (vacuum breakdown)}


\subsubsection{Phenomena: LCFA low X-ray measurement}

\cite{Ritus1985_QRR}
\cite{DiPiazza2018_LCFA}

\subsubsection{Phenomena B: Nonlinear BW measurement in isolation}

\subsubsection{Phenomena B: Linear BW measurement in isolation}

In comparison to measurements of virtuality

\subsubsection{Phenomena B: Vacuum birefringence}

\subsubsection{Phenomena B: Hawking-Unruh effect}

\subsubsection{Phenomena B: Physics beyond the Standard Model (axions)}

In nonlinear wakes and in nonlinear Compton scattering

\subsubsection{Phenomena C: strong-coupling regime of SFQED}

\subsubsection{Diagnostics: Spectral Measurement increased accuracy}

Detector as in \cite{Cole2018_RR,Behm2018_Gamma}
combined with \cite{Corvan2014_Gamma}
as proposed in \cite{Lisi2018_Gamma}.

\subsubsection{Diagnostics: On-shot interaction intensity}

\cite{HarShemesh2012_INTENSITY,Yan2017_ICS,Blackburn2019_ModelIndependentLaser}

\subsubsection{Diagnostics: Electron spectrometer (multi-screen+fiudicial)}

\subsubsection{Diagnostics: Electron polarisation measurement}

\subsubsection{Diagnostics: Electron profile (ICS)}

\subsubsection{Diagnostics: }

\subsubsection{Measurement routes}
...
For laser interactions three geometries for high-field (where particles are constrained and not ponderomotively lost)
\begin{itemize}
...
Overview of LWFA projects: ELI pillars, CoReLS (Gwangju, Korea), Gemini/EPAC, APOLLON, SULF, HERCULES (ZEUS), Nebraska, Munich (FOR or Cala)

Overview of conventional accelerator projects:  LUXE (XFEL Hamburg), FQED (FACET-II, SLAC), SULF CHINA XFEL
 
Overview of crystal measurements: CERN

Future Beam-beam measurements also for photon-photon collisions (also Narozhny conjecture) (so far CERN or so for ultra-peripheral).
...
\item two fast transverse probe (shadowgraphy)
