...
Several PW laser systems are either (soon) starting their operation or existing laser systems are being upgraded (ELI-NP \cite{Gales2018_ELINP}, ELI-Beamlines \cite{Weber2017_ELIBeamlines}, EPACS (no REF), HERCULES \cite{Yanovsky2008_HERCULES}, Apollon \cite{Zou2015_Apollon}, SULF \cite{Li2017_SULF}, BELLA, XCELS). Future facilities already aim for QED studies in the 100 PW regime \cite{Shen2018_SULF}. 
Maximum electron energies from LWFA on the other hand have reached up to 8 GeV recently \cite{Gonsalves2019_GEV}.
...
Another measurement of radiation reaction was performed at LHC \cite{Wistisen2018_RR}, however using planar channeling in crystals instead of ICS.
...
The histograms in Figure XX show the the sampled electron energies, which remain unperturbed when ignoring radiation reaction and are labelled with `RR off'. The post-interaction distribution of energies (orange) is calculated using EQUATION XX for $a_0 = 8$ which is the lower end of the estimated interaction intensity in \cite{Cole2018_RR}. For both `RR on' distributions the mean energy has shifted down by more than $50\,\mathrm{MeV}$ and the width of the normal distribution is reduced. EQUATION XX predicts a higher absolute energy loss for higher initial electron energies which over the entire spectrum leads to a cooling and reduction of the spread of energies. Due to the larger initial spread of sampled energies, the overlap of the initial and final distribution is larger for the Cole case. This intuitively tells us that it will be more challenging in case COLE than case PODER to confidently identify whether a measured electron spectrum was produced from a model without radiation reaction, `RR off', and for instance from a LL-RR model.
\vspace{\baselineskip}
...
where $\sigma_X = \sigma/\sqrt{n}$ is standard deviation on the mean. X1 and sigma1 are the $\epsilon = 550\,\mathrm{MeV}$ and $\sigma_{Cole}$ or $\sigma_{Poder}$, respectively. The mean and standard deviation is calculated from the post-interaction distribution.
...
At $a_0 = 8$, $\epsilon = 550\,\mathrm{MeV}$  the number of collisions shots required to reach $Z = 5$ is then XX for Cole and XX for Poder. Even at the same mean energy this makes a difference.
\vspace{\baselineskip}

If we expand this method to a range of standard deviations and electron energies, we get a matrix. Figure XX shows the minimum shot numbers for $Z=5$ and $a_0 = 8$ for 1-10 percent standard deviation and 100 to 2500 MeV electron energy. The blue and green marker now show the conditions of Poder and Cole. Including the higher electron energy the number of shots required to distinguish the models is less than 1, so one successful collision will work out well if the reference data is characterised sufficiently. 
...
\caption[]{Number of successful collisions to reach a 5 sigma confidence level for a combination of standard deviation and mean electron energy for $a_0 = 8$. The colour scale is logarithmic. The dashed white lines indicate the contours for the different regions for each order of magnitude. The blue and green triangle mark the conditions Cole and Poder were taken at, respectively.}
\end{figure}

At $a_0 = 20$ the region $N < 1$ quickly expands and both scenarios are clearly visible and it is possible to distinguish models within one single shot, even for relatively large energy fluctuations of 10 percent.
...
Gas jet can be used to interact at higher intensities, so the lower stability is not an issue at those conditions. 
\vspace{\baselineskip}
