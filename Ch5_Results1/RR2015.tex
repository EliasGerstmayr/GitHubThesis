...
\begin{figure}
\centering
\includegraphics[trim={4.9cm 0 5cm 0}, clip, width=.9\columnwidth]{ElecEdge_Example.png}
...
We split the varying components into two parts: one is random noise fluctuating from shot to shot, the second is the mean around which the fast fluctuations take place, which is slowly increasing with time. We assume a normal distribution. The increase of the mean correlates well with time at a correlation coefficient of 0.86 at a rate of $21.6\,\mathrm{MeV}$/hour. 

\subsubsection{Intrinsic statistical fluctuations}

Now we look at the deviation $\Delta E$ from this slow drifting mean in relative terms ($\Delta E/E$) as depicted in Figure XX NUMBER and estimate the underlying density distribution using a Kernel Density Estimate (KDE). The KDE agrees with the hypothesis of a normal distribution within its error bars. 
...
The smaller sub-set of data including the high-intensity interactions was taken in a time span of $\sim 30\,$ minutes.
...
\begin{figure}
\centering
\includegraphics[angle=-90, width=0.8\columnwidth]{Espec_RR15ColeExample_CsI.png}
\caption[]{Example of a gamma-detector signal.}
