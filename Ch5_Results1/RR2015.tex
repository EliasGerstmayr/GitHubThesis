...
\begin{equation}
\boxed{S_{BG} = c_{BG} \int_{\gamma_{min}}^{\gamma_{max}} \left[\mathrm{d}N_e /\mathrm{d}\gamma\right]\,\gamma^2 \mathrm{d}\gamma = c_{BG} Q \left\langle \gamma^2 \right\rangle,}
\end{equation}
...
\begin{equation}
\boxed{S_{ICS} = c_{ICS} \int  a_0^2 (\gamma) \left[\mathrm{d}N_e /\mathrm{d}\gamma\right]\, \gamma^2 \mathrm{d} \gamma,}
\end{equation}
...
\begin{equation}
\boxed{S_{ICS, norm} [\sigma_{BG}] > 5 \longrightarrow \mathrm{Succesful\,Collision}}
\end{equation}
...
More details on the procedure can be found in \cite{Cole2018_RR,Behm2018_Gamma} and also in the \nameref{Chap:Methods} as this technique is used in different variations to infer the spectra of high energy radiation in the other two experiments presented throughout this thesis (see Chapter \ref{Chap:linICS} and \ref{Chap:BW}).
\vspace{\baselineskip}
...
For both the semi-classical and the full quantum model the local constant field approximation (LCFA) was used in the derivation \cite{Ritus1985_QRR}.
\vspace{\baselineskip}
...
A model-independent technique to determine $a_0$ at the interaction on-shot \cite{HarShemesh2012_INTENSITY,Yan2017_ICS}(REF BLACKBURN IF PUBLISHED THEN) would also help to constrain the experimental conditions further, which in turn helps further separating experimental from model uncertainties.
