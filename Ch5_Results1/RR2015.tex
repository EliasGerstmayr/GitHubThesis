...
ICS provides a promising route to generate bright beams of high energy radiation reaching 100's of keV or even MeV photon energy, suitable for medical (REF) and imaging applications (also of high-Z objects) (REF) and to stimulate nuclear transitions (REF), even at small-scale facilities \cite{Albert2016_APP}.
...
In addition, the high photon energies offer the opportunity to directly measure the effect of radiation reaction on the beam dynamics itself and to observe energy loss, if the emitted photon carries an energy comparable to the energy of the electron $\gamma^2 a_0 \frac{2 r_e \omega}{3c} \sim 1$ \cite{Albert2016_APP}. Classical descriptions of radiation reaction exhibit unphysical predictions like self-acceleration, resulting in runaway solutions, or an unbounded emission spectrum, i.e. energy loss higher than the initial energy of the electron is allowed. A universally accepted description of radiation reaction has not been found yet.
...
At facilities housing the next generation of PW lasers, the reflective scheme will also be able to yield very promising results to probe radiation reaction due to the high intensities involved.
...
In the wake of increasing interest in measuring QED phenomena in laser interactions, ICS has also moved into the focus of conventional accelerator facilities. New dedicated projects like LUXE at the European X-FEL \cite{Burkart2019_LUXE} and the SFQED project at FACET-II \cite{SFQEDOverview2019} aim to continue the success of the seminal E144 experiment at SLAC conducted in the 90s \cite{Bula1996_RR,Burke1997_RR}. Whilst the high-quality beams at these facilities are superior to LWFA studies in terms of charge, energy and energy stability, these projects are facing other challenges due to significant amount of background noise in the accelerator tunnels, and typically longer and larger electron bunch sizes resulting in a significant overlay from linear scattering events in an ICS setup.

Another measurement of radiation reaction was performed at the LHC \cite{Wistisen2018_RR}, however using positrons and planar channeling in crystals as tool to probe radiation reaction instead of ICS.

An overview over the high field QED projects currently being undertaken can be found in \cite{SFQEDOverview2019}.

\section{Chapter Outline}

The work presented in this Chapter relates to an inverse Compton scattering (ICS) experiment aimed at measuring radiation reaction that was performed at the Gemini laser facility in late 2015. The experimental team succeeded in colliding electrons of energy $\epsilon \approx 550\,\mathrm{MeV}$ at a laser intensity of $a_0 \approx 10$, reaching critical gamma-ray energies, $\epsilon_{crit}$, in excess of $30\,\mathrm{MeV}$. This was the highest gamma-ray energy from an all optical ICS source published at that point and consists the first published measurement of radiation reaction in an LWFA setup \cite{Cole2018_RR}. 
\vspace{\baselineskip}
...
\caption[Energy of the electron edge energy as a function of time.]{Left: Distribution of the energy of the spectral feature for null shots or negligible ICS signal plotted against the relative time the data was taken. The energies are scattered around a line that slowly increases with time. A regression with corresponding $95\%$ confidence interval (shaded area) is drawn. Right: By assuming a random distribution around a slowly drifting mean the dataset was split into individual time slices.}
...
By factoring out known correlations of the spectra, scaling them accordingly (slow drift for gas jet data (Cole et al.) and laser energy for gas cell data (Poder et al.)), and by normalising the spectra to their total charge, it is possible to compare the spectra. With this processed dataset expected intrinsic or non-attributed fluctuations can be characterised and taken into consideration.
...
Close to the exit the electron beam is still very small $\sim \mathrm{\mu m}$ and a tightly focused laser spot can interact at a comparable intensity with a large fraction of the beam. These interactions are then likely to occur in a parameter space where $N_{min}=1$ again.
...
This analysis also neglects effects that could become important for longer interactions and tight focusing, such as change in intensity due to focusing effects and a chirp in the electron beam energy (REF IF PUBLISHED JOEL AND TOM BLACKBURN).

Furthermore, we used the Landau-Lifschitz description to calculate the post-interaction distribution. It would be of interest, in particular for future precision studies, to discriminate different radiation reaction models, for instance the full quantum model including stochasticity and the semi-classical model introduced in the previous section. Limitations of the local constant field approximation (LCFA) \cite{Ritus1985_QRR}, which is basis for the models including quantum corrections, are also an active field of research (REF) and require experimental validation (REF).
