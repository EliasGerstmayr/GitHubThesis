...
At XFELs, for instance, this is interesting for the study of warm-dense-matter states (REF) and high-energy-density physics (HEDP) (REF), involving pump-probe measurements, where X-rays from an insertion device probe matter structures and changes induced in them in the interaction with an intense laser pulse (REF).
In the wake of increasing interest in measuring QED phenomena in laser interactions, ICS has also moved into the focus of conventional accelerator facilities. New dedicated projects like LUXE at the European X-FEL \cite{Burkart2019_LUXE} and the SFQED project at FACET-II (REF) aim to continue where the success of the seminal E144 experiment at SLAC from the 90s \cite{Bula1996_RR,Burke1997_RR}. Whilst the high-quality beams at these facilities are superior to LWFA studies in terms of charge, energy and energy stability, these projects are facing other challenges due to significant amount of background noise in the accelerator tunnels, and typically longer and larger electron bunch sizes resulting in a significant overlay from linear scattering events in an ICS setup.

Another measurement of radiation reaction was performed at the LHC \cite{Wistisen2018_RR}, however using positrons and planar channeling in crystals as avenue to probe radiation reaction instead of ICS as tool.
...
After an outline of the experimental setup, such a method to identify successful collisions is presented. For this purpose, the yield on the gamma detectors is correlated with the energy in the electron beam on a shot-to-shot basis. Without the second scattering beam, the radiation measured is produced from bremsstrahlung as dispersed electrons interact with the walls of the vacuum chamber. It is assumed that the second laser adds ICS as an additional source of radiation and that particularly intense interactions produce a significant excess signal on the gamma detector. 

The chapter continues with an analysis of the measured electron spectra of this dataset. The electron spectra consistently exhibit a distinct spectral feature in shape of a sharp edge-like fall-off in charge at around $\sim 550\,\mathrm{MeV}$. The intrinsic fluctuations of the electron source are characterised in a statistical analysis after removing correlations.
...
Finally, the electron spectra from this experiment are compared to a second measurement of radiation reaction at the same laser system using a gas cell target, and colliding electrons of up to $2\,\mathrm{GeV}$ energy at $a_0\approx 10$ \cite{Poder2018_RR}. The analysis of the stability of the electron energies of both measurements are used estimate the minimum number of successful one would require in an experiment to reach a `discovery' $5\sigma$ level of confidence that a model excluding energy loss from radiation is not describing the data accurately. This discussion is expanded to a wider set of parameters and the discrimination of different models including radiation reaction in \cite{Arran2019_RR_PPCF, Arran2019_RR_SPIE}, but is beyond the scope of this thesis.
...
\caption[Montage of gamma spectrometer and electron spectrometer images for collision and null shots.]{Raw images from the gamma ray spectrometer (upper row) and electron spectrometer (bottom row) at shots with the colliding laser beam off (left) and on (right), using the same colour scale. Some shots with the colliding beam show bright signals on the scintillator array and could be indicator for a successful collision.}
...
On shots with a successful collision with the scattering beam, a burst of gamma rays from inverse Compton scattering (ICS) will be produced. At the detector the signal is then a combination of bremsstrahlung as without the scattering beam, following the same relation with slope $c_{BG}$, and the ICS signature. The emitted energy of the ICS radiation and hence the resulting detector signal, $S_{ICS}$, is also proportional to $\gamma^2$ similarly as for the bremsstrahlung process but also scales with the normalised vector potential $a_0$ in interaction with an electron with energy $\gamma$, valid for $\gamma a_0^2 < 4.4 \times 10^5$ \cite{Thomas2012_LL,Corde2013_Rad}:
...
\caption[Example of an electron spectrum measured on the radiation reaction campaign.]{Top: Example of a typical electron spectrum measured in the experiment. The x-axis indicates the energy, the y-axis the divergence of the electrons. The in dispersion and divergence axis integrated spectrum are shown on the respective axes.
...
\caption[Laser energy as a function of time, and correlation of electron edge energy and laser energy.]{Left: On-shot driver laser energy over a 2 hour window. The actual data set extends further but the laser energy meter does not cover the entire time. Right: Laser energy on the shots with the respective drift-corrected relative energy shift. Right: Correlation of laser energy with spectral feature for the core data set.}
...
The cumulative probability to observe a spectral feature below 500 MeV on one shot is $23\%$, based on the statistical analysis we performed. To observe this on all four successful collisions is then $0.3\%$. This is unlikely enough for us to conclude that the lower energies measured on these shots are due to energy loss in the successful collision and statistically significant.
\vspace{\baselineskip}

To estimate whether the energy loss relates to a realistic intensity at the interaction, we use the analytic expression for energy loss from the classical Landau-Lifschitz radiation reaction force for a plane wave with a Gaussian temporal envelope as given in \cite{Thomas2012_LL}:
...
For an $80\,\mathrm{MeV}$ energy loss from the mean of 550 MeV down to 470 MeV, for instance, we obtain an $a_0$ of 8.9.
...
It was estimated that 1 in 3 collisions would under these circumstances be on average successful, which closely matches the observed 4 in 13 successful collisions in this experiment.
...
The gamma spectra were inferred by analysing the signal on the gamma detector, a stack of scintillating caesium-iodide crystals (see \nameref{Chap:Methods}), and comparing the shape of the energy deposition throughout the detector with the in GEANT simulated detector response.
...
\caption[Gamma-ray spectra for collision and non-collision shots.]{Left: Fitted detector response with critical energies for exemplary shots with scattering beam on and off, from \cite{Cole2018_RR}. Right: Fitted critical energies (y-axis) vs. yield on the gamma detectors above expected background (x-axis) with the $\pm 2\sigma$ interval for the background (blue shaded) and $>5\sigma$ indicating successful collisions (shaded orange).}
...
We identified 4 successful collisions in 13 shots, which is consistent the measured spatio-temporal jitter of the accelerator and the laser.
...
Using the LL equation we estimated that the intensity at the interaction point must have around $a_0 \sim 9$ which matches the experiment conditions due to a mismatch in the timing (Section \ref{Chap:RR:Sec:Intensity}).
...
The red contour is generated using a semi-classical model. Here the equation of motion of the particles is governed by a classical description and is deterministic, but the emission power is reduced to match the quantum description, using the Gaunt factor which in the limit of low-intensity and low-energy interactions approaches 1 (REF). Due to the reduced emission spectrum the predicted critical energies are lower than for the classical model (orange) for the same final electron energy, which means that the purely classical description systematically underestimates $a_0$ relative to the semi-classical model.

Finally, the blue contour indicates the parameter space most likely to be occupied by interactions correctly described by a full quantum model which includes the stochastic nature of emissions in the interaction (REF). It closely aligns with the semi-classical model as the average emission power is by definition matched. However, at lower final energies, i.e. more intense interactions, the contour starts to separate towards higher critical energies which is an indicator of a phenomena called `straggling', where an electron propagates through the laser pulse for relatively long before finally emitting one individual high energy photon (REF). This only occurs when the stochastic nature of the process is considered and leads then to a spectral hardening of the emitted radiation.

For both the semi-classical and the full quantum model the local constant field approximation (LCFA) was used.
\vspace{\baselineskip}
...
\caption[Example of a typical electron spectrum measured on the Poder radiation reaction campaign.]{Example of a typical electron spectrum taken on this experiment. The distinct lines in the spectrum are edges of a second layer of lanex.}
...
Using this method, we can easily determine $N_{min}$ for a range of standard deviations and electron energies. Figure \ref{RR:fig:a0_8_20_matrix} shows the minimum shot numbers for $Z=5, 3$ and $a_0 = 8, 20$ for standard deviations between 1-10\%, and 100 to 2500 MeV electron energy, spanning a realistic parameter space of electron properties at Gemini. The blue and green marker now indicate the conditions of Poder and Cole et al. If we include the higher electron energy measured by Poder et al. the number of successful collisions required to distinguish the models falls to 1, i.e. one successful collision suffices as long as the unperturbed electron spectrum is characterised well enough.
...
Close at the exit the electron beam is still very small $\sim \mathrm{\mu m}$ and a tightly focused laser spot can interact with a large fraction of the beam, at a comparable intensity. These interactions are then likely to occur in a parameter space where $N_{min}=1$ again.
If the laser focus is larger than the electron beam, the interaction can be reduced to a 1D problem which simplifies the modelling process. Gas jets have in the past produced very stable electron beams with narrow energy spread and have also reached energies beyond $1\,\mathrm{GeV}$ (REF), which indicates that the performance of this target was maybe not representative of what is achievable to-date. 
In addition, using a gas jet would allow us to use shock injection as mechanism which has the potential of producing high-charge beams with narrow energy spread, and would also constrain the injection point and as a result the relative timing.
...
This analysis also neglects effects that could become important for longer interactions and tight focusing, such as change in intensity due to focusing effects and a chirp in the electron beam energy.

Furthermore, we used the Landau-Lifschitz description to calculate the post-interaction distribution. It would be of interest, in particular for future precision studies, to discriminate different radiation reaction models, for instance the full quantum model including stochasticity and the semi-classical model introduced in the previous section. Limitations of the local constant field approximation (LCFA), which is basis for the models including quantum corrections, are also an active field of research and require experimental validation.
