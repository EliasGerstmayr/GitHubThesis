...
Whilst these details are important for a detailed measurement of radiation reaction, they might be secondary concerns if simply radiation production is the aim.
...
.... several PW laser systems, 1 to 10 PW (recently reached REF)

... new facilities looking into this topic predominantly like ELI pillars REF

... several GeV of electron energies: Current LWFA energies for electrons of order 4 GeV (Korea) and guided up to almost 10 GeV. REF

.... SULF 100 PW. REF

... opens up new research topics: Strongly non-linear, quantum regime, photon-photon collider, BW, Schwinger limit. REF REF

... research into limitations of all-optical ICS. GUILLERMO REF.

\subsubsection{ICS at other types of facilities}

ICS is used at conventional accelerators to determine the polarisation of particle beams (polarimetry).

The increasing ease of access to ~10's TW laser systems, the advent of commercially available PW-class lasers and the ambitions of the LWFA have also spawned a renewed interest into the field of high-field phenomena at conventional accelerator facilities that can provide high-quality relativistic electron beams. What to do with an electron beam. XFELs. Lasers are interesting for warm dense matter and HEDP experiments, pump-probe, so are available already.

Examples are for instance LUX at XFEL in Hamburg (Electron energies and intensities XXX NUMBER?), SFQED at FACET-II/SLAC (30 TW, 13 GeV XX NUMBERS?). E144 at SLAC (46 GeV with $a_0 \sim 0.3$)
\vspace{\baselineskip}
...
This chapter includes significant contributions to this publication, in particular finding a method to identify successful collisions and characterising the electron spectra. As a result, the reader will find many parallels and in some instances a similar line of arguments between \cite{Cole2018} and this chapter.
\vspace{\baselineskip}

\EliasComm{Stuart: To make a measurement of radiation reaction we need to observe that when a laser-electron beam collision occurs the electron energy is lower. 
...
\EliasComm{This is all for one shot. Write about standard deviation, normal distribution and the fitting function.}
...
\EliasComm{Quantify the number of shots expected with overlap.}

\begin{equation}
S_{ICS,norm} (Q \left\langle\gamma^2\right\rangle)= \frac{S_{total} - f_{BG}(Q \left\langle\gamma^2\right\rangle)}{\sigma_{BG}}
\end{equation}

This method not only allows the identification of successful collisions, but on the other hand enables us to also identify collisions where ICS was negligible. 


\section{Characterisation of the Electron Spectrum}

In this experiment radiation reaction will manifest itself in two measurable effects: one is the energy loss of the electrons in the interaction with the laser pulse, the second is the radiation that this energy is being transferred into. 

The yield of the generated radiation was used in the previous section to identify successful collisions. In this section the electron spectra are investigated to estimate whether an energy loss can be inferred.
...
The position of the spectral feature is identified by finding a maximum in the derivative of the integrated spectrum resulting from the sharp cut-off in the integrated spectrum. This is shown in some examples in Figure \ref{Results:Figs:Edge:ShotsNull}.
...
In the previous sections we showed that by comparing the measured gamma-ray signal with the expected bremsstrahlung noise produced by the electron beam, successful collisions can be identified. The next step is quantifying whether there was a measurable amount of energy loss visible in the electron spectrum.
...
To estimate the energy loss accurately we have to take the intrinsic variations of the electron source and correlations to other fluctuating variables such as laser energy into account. This section outlines a statistical analysis of shots without the scattering beam turned on or where ICS was judged to be negligible, characterising the typical electron spectrum and the extent of its fluctuations. These shots are referred to as reference data or null shots. 
