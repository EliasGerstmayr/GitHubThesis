...
In the wake of increasing interest in measuring QED phenomena in laser interactions, ICS has also moved into the focus of conventional accelerator facilities. New dedicated projects like LUXE at the European X-FEL \cite{Burkart2019_LUXE} and the SFQED project at FACET-II \cite{SFQEDOverview2019} aim to continue where the success of the seminal E144 experiment at SLAC from the 90s \cite{Bula1996_RR,Burke1997_RR}. Whilst the high-quality beams at these facilities are superior to LWFA studies in terms of charge, energy and energy stability, these projects are facing other challenges due to significant amount of background noise in the accelerator tunnels, and typically longer and larger electron bunch sizes resulting in a significant overlay from linear scattering events in an ICS setup.

Another measurement of radiation reaction was performed at the LHC \cite{Wistisen2018_RR}, however using positrons and planar channeling in crystals instead of ICS as tool to probe radiation reaction
...
Then key results from the analysis of the gamma spectra, first performed by Jason Cole (Imperial College London) and Keegan Behm (University of Michigan), are introduced. The spectra are inferred by fitting the with GEANT \cite{GEANT4} simulated response of the detector, a stack of scintillating caesium-iodide (CsI) crystals, to the signal measured in experiment. Details of the analysis can be found in \cite{Behm2018_Gamma,Cole2018_RR} or also in the \nameref{Chap:Methods} Chapter of this thesis.
...
The gamma spectra were inferred by analysing the signal on the gamma detector, a stack of scintillating caesium-iodide crystals (see \nameref{Chap:Methods}), and comparing the shape of the energy deposition throughout the detector with the in GEANT \cite{GEANT4} simulated detector response.
...
For both the semi-classical and the full quantum model the local constant field approximation (LCFA) was used \cite{Ritus1985_QRR}.
\vspace{\baselineskip}
...
A model-independent technique to determine $a_0$ at the interaction \cite{HarShemesh2012_INTENSITY,Yan2017_ICS}(REF BLACKBURN IF PUBLISHED THEN) would also help to constrain the experimental conditions further, which in turn helps further separating experimental from model uncertainties.
...
If the laser focus is larger than the electron beam, the interaction can be reduced to a 1D problem which simplifies the modelling process. Gas jets have in the past produced very stable electron beams with narrow energy spread \cite{Faure2006_STABLEJET} and have also reached energies beyond $1\,\mathrm{GeV}$ (REF), which indicates that the performance of this target was maybe not representative of what is achievable to-date. 
In addition, using a gas jet would allow us to use shock injection as mechanism which has the potential of producing high-charge beams with narrow energy spread \cite{Buck2013_SHOCK}, and would also constrain the injection point and as a result the relative timing.
