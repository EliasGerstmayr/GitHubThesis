...
\caption{Comparison of all-optical ICS experiments using LWFA. The circles (cyan) indicate one-beam experiments that use a plasma mirror to reflect the LWFA driver beam and scatter off the electron bunch it accelerated before. The triangles (blue and green) indicate experiments that used two laser beams, one to accelerate electrons from LWFA and one separate laser to scatter off them. Top: Maximum gamma energies measured and their year of publication. Bottom: Electron energies and laser intensities in recent ICS experiments using LWFA.}
\end{figure}
...
These details are important for a detailed measurement of radiation reaction, but might be secondary concerns if a well-defined spectrum is not crucial for the application.
At facilities housing the next generation of real PW-class lasers, the reflective scheme will still be able to yield very promising results to probe radiation reaction.
...
At XFELs, for instance, this is interesting for the study of warm dense matter states and HEDP, involving pump-probe measurements, where X-rays from an insertion device probe matter structures and changes in them in interaction with a laser.
In the wake of increasing interest in measuring QED phenomena in laser interactions, ICS has also moved into the focus of conventional accelerator facilities. New dedicated projects continue where the seminal E144 experiment at SLAC left off in the 90s (46 GeV electron beam with laser $a_0 \sim 0.3$, \cite{Bula1996_RR,Burke1997_RR}) with LUXE at the X-FEL Hamburg \cite{Burkart2019_LUXE}, SFQED at FACET-II (30 TW, 13 GeV XX NUMBERS?). Whilst the high-quality beams at these facilities are superior to LWFA studies in terms of charge, energy and energy stability, these projects are facing other challenges due to significant amount of background noise in the accelerator tunnels, and typically longer and larger electron bunch sizes resulting in a significant overlay from linear scattering events in an ICS setup.

Another measurement of radiation reaction was performed at the LHC \cite{Wistisen2018_RR}, however using positrons and planar channeling in crystals instead of ICS.
...
Then key results from the analysis of the gamma spectra, first performed by Jason Cole (Imperial College London) and Keegan Behm (University of Michigan), are introduced. The spectra are deduced by simulating the response of an extended CsI scintillator stack to gamma radiation and comparing this to the measured signal. Details on the analysis can be found in \cite{Behm2018_Gamma},\cite{Cole2018_RR} or also in the \nameref{Chap:Methods} Chapter of this thesis.
...
The chance for this to occur is 1 in 3000. This supports the hypothesis further that we have measured radiation reaction.
\vspace{\baselineskip}
...
The correlation coefficient for the driver laser energy over time is $-0.11$, and the confidence interval for the slope of the linear regression includes negative and positive values. This indicates that the mean laser energy does not drift significantly over this time period, and only fluctuates around a constant or slowly decreasing mean (see Figure \ref{RR15:Figs:laser_elec_corr}). 

Self-injection is coupled depends strongly on the performance of the laser and its evolution in the medium. If the mean electron energy increases over time without changes to the laser properties and energy, changes in the medium can be suspected.

The appearance of the high charge component of the spectrum is consistent with other electron beams injected through shock injection at Gemini (NO REF AVAILABLE YET). The shock could be generated due to a damage in the gas nozzle or intrinsically due to its design. The drifting mean is consistent with a shock shifting position closer to the leading edge of the gas target, increasing the acceleration length and energy over time. This behaviour typical for shock injection has been observed at lower intensities at other laser systems \cite{Schmid2010_shock,Buck2013_shock,Tsai2018_shock}. Unfortunately, there is no optical probe data, e.g. from a shadowgraphy, to provide conclusive evidence for this theory. Progressing damage to the nozzle material could provide the seed for a shifting shock position.
...
More details can be found in \cite{Cole2018_RR,Behm2018_Gamma} and in the \nameref{Chap:Methods}.
...
The intensity of the laser pulse is significantly lower than the peak $a_0$ achievable in this geometry. Since the energy calibration are reliable, this indicates a spatio-temporal offset and that the interaction is not occurring at the focal plane of the laser.
...
After having now narrowed down the conditions at the interaction, we confirmed that the number of successful collisions match what we see in the experiment. We can now look at the detailed relations of these shots and how they line up with actual models.
...
Now here some part about the models and how they relate, how the plot is made but this is all Jason Cole's and Tom Blackburn's work and should be referenced as such.
...
The experimental results show agreement with models including radiation reaction. Due to the low number of shots the confidence levels are low. In addition, there seems to be a first sign of stress between how well it agrees with certain models. A definite discrimination of radiation reaction models is not possible at this point. More data, stability, higher electron energies, laser intensities and additional observables could enable this as part of a future precision measurement.
...
The results from the statistical analysis are then being used to estimate how many shots would be required to achieve $5\sigma$ confidence for either data set. This is used to indicate what kind of gas target is preferable.

\subsubsection{Experimental setup}

The experimental setup in \cite{Poder2018_RR} is very similar to the experiment described at the beginning as it is based on the same facility, relying on similar optics and beam line designs. A sketch of the setup is show in Figure \ref{RR15:figs:exp_sketch_Poder}.
...
The scaled and averaged spectra from the two experiments are shown in Figure \ref{fig:comparison_especs}. The spectra were processed as outlined before and then averaged over all available data shots. The lineouts are normalised by their total charge such that the integral of each spectrum is set to 1. Typical energy reached in the gas cell from Poder et al. is significantly higher by at least a factor two for the majority of the charge distribution. The higher electron energy also enables a potentially higher $\eta$ parameter in an interaction as $\eta \propto \gamma$. The shaded region around the scaled spectra indicates the standard deviation of the averaged spectrum at that particular energy. The energy-dependent variances are also shown in Figure \ref{fig:comparison_varianceLog} on a logarithmic y-axis. The variations for the data taken with the gas jet is in particular strong around the important position of the spectral edge and the variance reaches a 10-times higher peak and total value.
\vspace{\baselineskip}
...
\caption[]{Energy-dependent variance of the average spectra in introduced in Figure \ref{fig:comparison_especs}. In blue the data taken on the experiment related to Cole et al., in green to Poder et al.. The total variance for Cole et al. was $1.5 \times 10^{-7}$ and $1.98 \times 10^{-8}$ in the case of Poder et al..}
...
\caption[]{Electron energies sampled from normal distributions with standard deviation $\sigma_{Cole}$ (left, blue) and $\sigma_{Poder}$ (right, green), and the calculated post-interaction energy distribution due to radiation reaction (orange) at $a_0=8$.}
\end{figure}


The histograms in Figure XX show the the sampled electron energies, which remain unperturbed when ignoring radiation reaction and are labelled with `RR off'. The post-interaction distribution of energies (orange) is calculated using Equation \ref{RR15:eq:LLThomas} for $a_0 = 8$ which is the lower end of the estimated interaction intensity in \cite{Cole2018_RR}. For both `RR on' distributions the mean energy has shifted down by more than $50\,\mathrm{MeV}$ and the width of the normal distribution is reduced. Equation \ref{RR15:eq:LLThomas} predicts a higher absolute energy loss for higher initial electron energies which over the entire spectrum leads to a cooling and reduction of the spread of energies. Due to the larger initial spread of sampled energies, the overlap of the initial and final distribution is larger for the Cole case. This intuitively tells us that it will be more challenging for the distribution with $\sigma_{Cole}$ than for the one with $\sigma_{Poder} < \sigma_{Cole}$ to confidently identify whether a measured electron spectrum was produced from a model without radiation reaction, `RR off', or for instance from a `RR on' model (here LL).
\vspace{\baselineskip}

To quantify this, we perform a statistical Z-test.
The parameter Z indicates the confidence at which two distributions can be distinguished from each other and is equivalent to a $\sigma$ confidence level. It is calculated by comparing the mean and the standard deviation on the mean for two distributions:

\begin{equation}
Z = \frac{\bar{X_1} - \bar{X_2}}{\sqrt{\sigma_{X_1}^2 + \sigma_{X_2}^2}},
\end{equation}

where $\sigma_X = \sigma/\sqrt{n}$ is standard deviation on the mean. $\bar{X}_1$ and $\sigma_1$ are $\epsilon = 550\,\mathrm{MeV}$, and $\sigma_{Cole}$ or $\sigma_{Poder}$, respectively. The mean $\bar{X}_2$ and standard deviation $\sigma_2$ are calculated from the post-interaction distribution.
...
In an experiment we also have to consider that only 1 in X shots with both beams will result in a successful collisions. In this case it was estimated to be a 1 in 3 chance, so the total number of shots would then be $4n$ or in general terms $(X+1)n$ shots. We also assumed that we can identify successful collisions using the method described at the beginning of this chapter.

At $a_0 = 8$, $\epsilon = 550\,\mathrm{MeV}$ the number of collisions shots required to reach $Z = 5$ is then $n = 15.7$ for Cole and $n = 3.4$ for Poder. Even at the same mean energy this makes a difference.
\vspace{\baselineskip}

If we expand this method to a range of standard deviations and electron energies, we get a matrix. Figure \ref{RR:fig:a08_matrix} shows the minimum shot numbers for $Z=5$ and $a_0 = 8$ for 1-10 percent standard deviation and 100 to 2500 MeV electron energy. The blue and green marker now show the conditions of Poder and Cole. Including the higher electron energy of Poder the number of shots required to distinguish the models is less than 1, so one successful collision will work out well if the reference data is characterised sufficiently. 

At this value of $a_0$ the lower energies and larger shot-to-shot energy fluctuations result in more shots being required.
This means we either have to improve the gas target or use a gas cell.
...
\caption[]{Minimum number of successful collisions required, $N_{min}$ to reach a 5$\sigma$ confidence level for a range of standard deviations and mean electron energies at $a_0 = 8$. The colour scale is logarithmic. The dashed white lines indicate the contours for the different regions for each order of magnitude. The blue and green triangle mark the conditions Cole and Poder were taken at, respectively.}\label{RR:fig:a08_matrix}
\end{figure}

At $a_0 = 20$ (see Figure \ref{RR:fig:a020_matrix}) the region $N < 1$ quickly expands and both scenarios are clearly visible and it is possible to distinguish models within one single shot, even for relatively large energy fluctuations of 10 percent.

This means that at lower intensities and energies the fluctuations are quite important to distinguish `no RR' and RR models, but that at sufficiently high intensities and energies this becomes less important.
Gas jet can be used to interact at higher intensities, so the lower stability can be outweighed at these conditions. 
\vspace{\baselineskip}
...
\caption[]{Left: Minimum number of shots. Right:}\label{RR:fig:a020_matrix}
\end{figure}

Caveats for this analysis: the energy loss was calculated using the Landau-Lifschitz radiation reaction force. At higher intensities the energy loss in quantum models is reduced scaled by the Gaunt factor. This means the shift in energy is smaller. At the same time the stochasticity of a full quantum model might increase the standard deviation of the final distribution (it will probably still cool down but not as much as currently). This means that if we want this to be an estimate `no RR' to RR including the other models, we will require more shots than estimated for LL which will predict higher energy losses.

This is also considering one fixed intensity only, which might change, so the distribution changes.
\vspace{\baselineskip}

This is looking at no RR and RR, and not how the parameters affect distinguishing different models including radiation reaction.

Motivate that looking at Jason's plot the quantum and semi-classical model is very close to each other. This is due to the matched emission power. Stochasticity and other factors will eventually make it possible to discriminate them, but since the parameters diverge very slowly the number of shots will remain high. Other parameters are more sensitive, see Theory and \cite{Ridgers2017_QRR,Arran2019_RR_PPCF,Arran2019_RR_SPIE}.
...
\caption[]{Minimum number of shots to reach a $3\sigma$ confidence level for $a_0=8$ (left) and $a_0=20$ (right).}\label{RR:fig:Nmin_3sigma_matrix}
\end{figure}

For more distinct models, this might be again more challenging, but see \cite{Arran2019_RR_PPCF,Arran2019_RR_SPIE}. Discuss advantages of using a gas jet, shock injection and so on.
Intense interactions, electrons are small, 1D approximation for interaction. Timing and injection through shock injection. Distinct features and low energy spread.

This line of thought has been explored more explicitly and in more detail in \cite{Arran2019_RR_PPCF,Arran2019_RR_SPIE}. The aim of these publications is to indicate which experimental observables are suited to discriminate different models of radiation reaction and to give an estimate on the number of shots one would require in an experiment taking fluctuations into account.

\cite{Arran2019_RR_PPCF,Arran2019_RR_SPIE} uses a Bayesian inference approach to determine how closely related the predictions of the different models are, the overlap of the models.
\vspace{\baselineskip}
...
In this case, the stability of the electron beam energy and of the spectral shape was superior for the data using a gas cell target \cite{Poder2018_RR}, with half of the standard deviation on a normal distribution for the energies and a 10 times smaller total variance compared to the gas jet target used in \cite{Cole2018_RR}.
