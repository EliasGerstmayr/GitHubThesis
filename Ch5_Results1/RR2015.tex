...
In an experiment we also have to consider that only 1 in X shots with both beams will result in a successful collisions. In this case it was estimated to be a 1 in 3 chance, so the total number of shots would then be $4n$. We also assumed that we can identify successful collisions using the method described at the beginning of this chapter.


For this and $Z=5$ the number of collisions shots is then XX for Cole and XX for Poder.
...
If the collision likelihood is 1 in 3, the total number of shots is then $4n$.
...
Gas jet can be used to interact at higher intensities, so the lower stability is not an issue. For more distinct models, might be more challenging, but see C ARRAN. Discuss advantages of using a gas jet, shock injection and so on.
