...
We can check if this correlates with laser energy. There appears to be a positive correlation but only at lower confidence. The correlation coefficient is around 0.35 and the confidence interval for the slope encloses only very small gradients of order few percent over 4 J energy range (whereas the standard deviation is already scattered at 8 percent). This all indicates that this was not a very important factor in this context and we will hence ignore this factor in our analysis. Also not relevant for the immediate data set as for that the correlation is dropping to zero.
...
I calculate the beam size assuming the e-beam behaves like a Gaussian beam
so 
...
The electrons accelerated via LWFA are dispersed by a dipole magnet of integrated field strength $\int B \mathrm{d}x \approx 0.15 \,\mathrm{Tm}$ onto a scintillating LANEX screen.
...
\subsubsection{Radiation Reaction and spectra}
