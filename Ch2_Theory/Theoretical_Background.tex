...
Based on Jackson:
From that we get the wave equation where $\mathbf{E}$ and $\mathbf{B}$ satisfy this:
...
The plane wave solution of this is 

\begin{equation}
u = e^{i \mathbf{k}\cdot\mathbf{x} - i \omega t},
\end{equation}
with
\begin{equation}
k = \frac{\omega}{v} = \sqrt{\mu \epsilon} \frac{\omega}{c}
\end{equation}
...
Electric field of laser in vacuum:
\begin{equation}
\mathbf{E}(r,z,t) = E_0 \mathbf{x} \frac{w_0}{w_z} \exp\left(-\frac{r^2}{w(z)^2}\right) \exp\left[-\frac{(z-v_g t)^2}{(c \tau)^2}\right] \exp\left[-i\left(kz - \omega t + l \frac{r^2}{2 R(z)} - \psi(z)\right)\right]
\end{equation}
...
f number: focal length over beam diameter

how tightly focussed
\begin{equation}
w_0 = \frac{2 \sqrt{2}}{\pi} \lambda f \approx 0.9 \lambda f
\end{equation}

distance over which laser stays intense
\begin{equation}
...
Real beams are not Gaussian but flat top.
Deviations from wave front and curvature profile, spatio-temporal couplings and so on

\subsection{General Properties of Plasmas}
...
\EliasComm{Shorten this section as it is not really important in this context. Move details into appendix if necessary or just delete.}

Ionisation describes the process of removing an electron from the shell of a neutral atom or a not fully ionised ion. Already ionised atoms can be further ionised reaching higher ionisation states by stripping more and more electrons from the shells, but this process requires more and more energy as the remaining electrons are bound tightly to the core. The ionisation of a neutral atom can be expressed by $X + \mathrm{energy} \rightarrow X^+ + e^-$, whereas higher order ionisation will follow the pattern $X^{n+1} + \mathrm{energy} \rightarrow X^{n+} + e^-$.

In the context of this work high intensity lasers are used to drive waves in plasmas. A change in the number density of free electrons through ionisation effects can influence the dynamics of the system. The plasma is usually formed by filling a gas compartment or firing a gas jet with a neutral gas, assuming that the laser pulse ionises the gas fully to first approximation or by pre-ionising targets using an electrical discharge or a secondary laser.
\vspace{\baselineskip}
...
In single or multi-photon ionisation processes the absorbed photons have to bear at least the binding energy of the electron. Any excess energy will be transferred into additional kinetic energy of the freed electron.

In tunnel ionisation an external electric field deforms the binding potential of the atom and lowers the potential barrier. The rate of tunnel ionisation is given by...

(Using Keldysh parameter to estimate in which regime we are?).

\begin{equation}
\gamma_K = \omega \frac{\sqrt{2 m_e V}}{eE}
\end{equation}
At $\gamma_K >> 1$ multiphoton ionisation is expected and in the other extreme for very small $\gamma_K < 0.5$ tunnel ionization is important.
If the electric field is larger than the potential barrier suppression is dominating.

In barrier-suppression ionisation the external electrical field is even stronger and completely suppresses the barrier such that the bound electron is now not limited by any potential barrier any more.
Using the equation to calculate the potential and by setting the height of the barrier to the ionisation potential $V_e$, the electric field required for complete barrier suppression amounts to
...
$N^{6+} \rightarrow N^{7+} $ & $667$ & $1.6 \times 10^{19}$
\end{tabular}
...
\caption{Plasma frequency and wavelength at density.}
\end{figure}
...
At $\omega << \omega_p$ this the expression can be approximated by
\begin{align}
...
High intensity lasers interact with the electrons and their relativistic motion will lead to a modification of the refractive index
...
Local modifications of plasma density, laser frequency and laser intensity
\begin{align}
...
\subsection{Relativistic Self-focusing}

Laser diffraction by differentiating the waist size of Gaussian near focus ($z < z_R$) using 
\begin{equation}
...
For the plasma refractive index

\begin{equation}
\frac{\partial^2 w}{\partial \tau^2} = \frac{c^2}{\eta}\frac{\partial \eta}{\partial r} = -\frac{c^2}{8} \frac{\omega^2_p}{\omega^2_0} \frac{\partial}{\partial r} a^2 (r,z) \approx - \frac{1}{8}\frac{\omega^2_p}{\omega^2_0}\frac{a^2_0}{w0}c^2
\end{equation}
...
\begin{equation}
P[GW] \leq 17.3 \frac{\omega^2_0}{\omega^2_p}
\end{equation}

\subsection{Self-guiding}
...
Rate of compression in wave frame 

\begin{equation}
\frac{1}{L} \frac{\partial L}{\partial t} = -c \frac{\partial \eta}{\partial \xi}
\end{equation}

In bubble regime $n_e = n_0, n_e = 0$
