...
In the bubble regime the leading edge of the laser pulse sees a density $n_e = n_0$, whereas the remaining pulse propagates approximately in vacuum with $n_e = 0$. If we assume that the density transition occurs linearly over a distance $l$ then $\partial_z n_e = n_0/l$, then
\begin{equation}
...
In the context of wakefield acceleration the density gradient is produced by the intense laser pulse itself. A compression of the pulse duration as described, however, has to be accompanied by an increase of the bandwidth of the laser pulse.

Similarly as before, consider two points in the laser pulse at $z_1$ and $z_2$ with phase velocities $v_{p1}$ and $v_{p2}$, respectively. If we impose that the wavefronts at these points separated by a phase of $2\pi$, then the spatial separation corresponds then to the wavelength $\lambda_0 = z_2 - z_1$. After a time interval $\Delta t$, the spatial separation of the wavefronts is now $\Delta \lambda$
\begin{equation}
\Delta \lambda = \lambda_0 + (v_{p2} - v_{p1}) \Delta t.
\end{equation}
...
\end{align}
This equation now describes how the wavelength or frequency shifts as a function of the density gradient. A positive density gradient results in an increase of the frequency, $\omega$, i.e. blue-shifting or `photon acceleration'. A negative gradient, on the other hand, would lead to a decrease in the frequency, so red-shifting or `photon deceleration'. In the previous section we considered pulse compression as a result of a positive density gradient, so we now see that the pulse compression is accompanied by photon acceleration. This phenomenon has been more rigourously investigated theoretically in \cite{Mendoncca1994_PHOTONACC,Mendoncca2000_PHOTONACC} and has been confirmed in experiment \cite{Murphy2006_PHOTONACC}.
...
\end{equation}
which is also called the cold non-relativistic wave breaking limit (REF DAWSON)\addref (REF ESAREY REV MOD 2009)\addref.
In useful units we can write:
\begin{equation}
E_0 (GV/m) = 96 \sqrt{n_0 (10^{18} cm^{-3})},
\end{equation}
so that the maximum field sustained by a wave in a plasma of density $10^{18}\mathrm{cm^{-3}}$ is roughly $100\,\mathrm{GV/m}$.
Nonlinear plasma waves can exceed this value and the wavebreaking limit for a 1D relativistic, non-linear periodic plasma wave is, for instance, given by
\begin{equation}
...
A short and relativistic laser pulse can fully expels the electrons in its way. The pure ion cavity left behind is approximately a spherical `bubble' of radius \cite{Pukhov2002_BUBBLESIM}
\begin{equation}
...
\end{equation}
where $\gamma_{ph} = \omega/\omega_p \approx \sqrt{n_c/(3n_e)}$ and $r_b$ is the radius of the plasma bubble. This is process is referred to as self-trapping and triggering triggering the injection of electrons into the cavity solely through the evolution of the laser pulse is called self-injection.
...
\end{equation}
with $\alpha$ the fraction of energy contained in the \textsc{fwhm} of the laser focal spot, $P$ the laser power. $P_c$ is the laser power at which relativistic self-focusing and diffraction are balanced, with $P_c \approx 17 n_c/n_e\,\mathrm{GW}$ (see Section \ref{Theory:Sec:SelffocusingSelfguiding}).
...
A higher plasma density reduces the front velocity of the laser pulse and reduces the dephasing length
...
In Section \ref{Theory:Sec:TrappingSelfInjection} we introduced a condition for ambient electrons from the plasma to be trapped in the bubble cavity, and a threshold was given over which the evolution of the laser pulse and subsequently the bubble injects these suitable electrons. This injection mechanisms is called self-injection. In 1D this threshold is equivalent to the wavebreaking limit, whereas in 3D the crossing of electron sheets that trigger the injection can also occur without catastrophic wavebreaking. 

There are also other injection mechanisms that have been developed to produce electron beams with specific properties, for instance high charge or low energy spread. Out of the plethora of different techniques we will only introduce the mechanisms used in the context of this thesis, which are, besides self-injection, ionisation and shock injection.

\subsubsection{Ionisation Injection}

Instead of relying on the evolution of the laser pulse and subsequent wavebreaking, electrons can be released directly into the bubble cavity through delayed ionisation of the atoms or molecules close to the peak intensity of the laser pulse. 
Higher-Z gases bind electrons strongly and require higher field strengths to release them. When the intense laser pulse passes through it triggers an increased rate of tunnel ionisation near the centre of the pulse (see Section \ref{Theory:Sec:PlasmaFormation} on plasma formation). The released electrons can then be trapped and subsequently accelerated. This is called \textit{ionisation injection}.

Typically, a gas with low ionisation threshold like helium or hydrogen is used as base, which is then doped with a few percent of high-Z gas, for instance nitrogen, to provide the injection events. The interplay of the laser, its evolution and intensity, and the properties of the dopant determine the amount of charge released into the cavity and the duration of the injection event. The duration of the injection translates into energy spread and hence provides a way to tune the properties of the accelerated electron beam.
...
\EliasComm{If synchrotron included then common classical trajectories are figure of eight motion.}
...
\EliasComm{Here a simpler equation in a specific limit to see scalings.}
...
\begin{equation}
E_f = E'_f \gamma (1+\beta \cos \theta'_f) \approx E'_f \gamma (1+ \cos \theta'_f) \sim \gamma^2 E_i,
\end{equation}
...
The photon energy is then boosted by $(2\gamma)^2$ to $E_{f,max} = 2 \gamma E'_f = 2 \gamma E'_i = 4 \gamma^2 E_i$. In the rest frame of the electron this corresponds to $E'_i = E'_f$, which again indicates Thomson scattering and motivates the alternative name for this process, relativistic Thomson scattering.
...
At higher intensities, i.e. $a_0 \rightarrow 1$ and $a_0 > 1$, the interaction changes as nonlinear effects gain significance. In a classical picture the electrons now perform a figure-of-eight motion (see Section \ref{Theory:Sec:SingleParticle:FigOfEight}) which gives rise to harmonics as becomes evident when the trajectory is evaluated in a Fourier analysis, whilst also leading to a relativistic mass increase. In a quantum picture the contribution of higher-order Feynman diagrams that describe the process of multiple photons being absorbed and then emitted as one photon by a single electron becomes increasingly significant (see FIGURE\addnum). The relativistic mass increase, on the other hand, results in red-shifting of the emitted radiation.
This behaviour draws parallels in the description of electrons in insertion devices that is also described by the figure-of-eight motion (see SECTION\addnum). Here the wiggler parameter, $K$, indicates whether distinct harmonics ($K\ll1$) or broadband radiation is emitted ($K\gg1$). In the case of inverse Compton scattering, a laser wiggler, the parameter $a_0$ takes the place of $K$. 
...
\end{equation}
\EliasComm{Describe here that in the quantum picture for $n>2$ it becomes increasingly tedious to solve all Feynman diagrams, such that we transition to Volkov states in the Furry picture.}
\begin{figure}
\centering
...
\caption{Klein-Nishina and higher orders at $a_0$ for circular polarisation. Adapted from Tom Thesis REF}
\end{figure}
...
In the previous sections we discussed the emission of radiation from charged particles through different mechanisms. Based on conservation rules the particles lose energy in the process and also have to experience a knock-back force whenever they radiate. Such a knock-back or commonly referred to as radiation reaction can in strong electromagnetic fields significantly alter the particle dynamics. Whilst the instantaneous force felt by the emitting particles is small in most settings, it is particularly important in the astrophysical context (EXAMPLES) and will be the subject of research at high-intensity laser facilities. Here a single particle can emit multiple photons in a short span of time and the radiation reaction force has to be considered. In extreme cases the energy emitted by the particle approaches its initial energy, which requires modelling the process in a quantum picture, i.e. we then consider quantum radiation reaction.
...
Reviews on this topic can be found in parts of REF DiPiazza\addref{} and in REF Blackburn2019\addref{}.

\subsection{Classical Radiation Reaction}

We will start with descriptions of the radiation reaction force rooted in classical electrodynamics. Out of a variety of different approaches we will focus on the two most commonly discussed models and their implications: the Lorentz-Abraham-Dirac (LAD) equation and the Landau-Lifschitz (LL) description. See for instance BURTON NOBLE 2-14 REF\addref{} for a review of other classical theories of radiation reaction.
...
In the Landau-Lifschitz model we can also find an analytic expression of the energy loss an electron experiences. For an electron interacting with a plane wave with a Gaussian temporal envelope this is given by \cite{Thomas2012_LL,Bulanov2011_LL} VALID\addnum{}
\begin{equation}
...
\end{equation}
where $\tau_0 = 2 e^2/3m_e c^3 = 6.4 \times 10^{-24}\,\mathrm{s}$, the pulse duration $t_L$, the wavelength $\lambda$.
\vspace{\baselineskip}

Whilst this description is free from runaway solutions unlike the LAD equation it violates in some cases energy conservation when considering abruptly changing fields, causing the energy loss to exceed the initial energy of the particle (REF Baylis PLA 2002). 
...
\begin{itemize}
\item corrections regarding radiation cut-off in spectrum (quantum synchrotron radiation)
\item stochasticity (broadening in e-spectrum, straggling in radiation spectrum)
\end{itemize}
...
\EliasComm{Explicitly refer to plots and that energy distribution is symmetric for $E_1/E_2 = 1$, but asymmetric if $E_1,E_2 \neq 1$. Distribution functions are symmetric for $e+,e-$}
...
The second harmonic is for instance given by (\cite{Ritus1985_QRR} and T NOUSCH PHYSICS LETTER B 2012):
\begin{equation}
...
Similarly as for bremsstrahlung the differential cross section can in a simplified and approximated way also be expressed in terms of the radiation length, $X_0$ (REF\addref{} Tsai, PDG):
\begin{equation}
\frac{\dif \sigma}{\dif x} = \frac{A}{X_0 N_A} \left[ 1 - \frac{4}{3}x(1-x)\right],
\end{equation}
