...
The Lagrangian for a relativistic particle in an electromagnetic field with the potentials $\Phi$ and $\mathbf{A}$ is given by \addref (REF)
...
\end{equation}
where the second term disappeared as the curl of a gradient is always zero (REF)\addref.
...
with a frequency we will call the betatron frequency, $\omega_\beta$, which is related to the plasma frequency, $\omega_p$, by $\omega_\beta = \omega_p/\sqrt{2\gamma}$. The plasma frequency will be derived in a later section\addlink.
...
If we continue with the assumption $\dot{\gamma} = 0$, this requires $\beta^2 = \beta^2_r + \beta^2_z = const.$. This gives us $\beta_z = \sqrt{\beta^2 - \beta^2_r} \approx \beta (1-\beta^2/2)$, where we used a Taylor expansion and $\beta \approx 1$. If we use the solution for $r(t)$ derived above, $\beta_r = \dot{r}/c$ and $\sin^2 x = (1-\cos 2\theta)/2$, we obtain:
\begin{equation}
...
Now we add will add a constant axial field in the longitudinal (direction of electron propagation) to the previous case. This comes closer to the reality of a laser wakefield accelerator where electrons experience radial focusing and axial accelerating fields at the same time. Significant parts of this derivation are again based on Jonathan Wood's PhD thesis \cite{WoodThesis}.
...
\caption[Particle trajectory of an electron in a cylindrically symmetric electric field with a constant axial accelerating field.]{Particle trajectory of an electron in a cylindrically symmetric electric field with a constant axial accelerating field as described in Equation \eqref{Theory:Eqs:EoM_Betatron} as a function of propagation distance. \EliasComm{Give density used, add second density and add second x-axis for time.}}
...
The solution of this is then (REF ESAREY 2002?\addref):
\begin{subequations}
\begin{empheq}[box=\widefbox]{align}
...
\EliasComm{Explanation why we require Bessel functions?}

\subsection{Ponderomotive Force}

In laser wakefield acceleration (LWFA) a high intensity laser pulse propagates through an underdense plasma, expels electrons in its way and drives a wave\addref. The driving force is the so-called ponderomotive force which is proportional to the gradient of the laser intensity. 

The following relativistic derivation of the ponderomotive force is based on Alec Thomas' PhD thesis \cite{ThomasThesis}.
\EliasComm{Would it be more appropriate to write Alexander?}
...
The plasma frequency $\omega_p$ is the natural frequency at which electrons will oscillate in a plasma when displaced (REF KRUER OR SO\addref). It constitutes the typical time scale and the corresponding plasma wavelength $\lambda_p$ the typical length scale of the medium.
\vspace{\baselineskip}
...
\end{equation}
\EliasComm{Maybe explicitly say how intensity is calculated.}
...
\EliasComm{Write out. Maybe move to Methods.}
...
\EliasComm{Need to write this down coherently.}
...
\EliasComm{There seems to be some doubling.}
\begin{equation}
...
\EliasComm{Figure of eight motion?}
...
\EliasComm{Classical as in standard?}
...
\EliasComm{Thomson is classical limit of Compton.}
...
\EliasComm{s- and u-channels maybe not good description.}
...
\EliasComm{Write down coherently. Maybe just inverse Compton and then describe linear and non-linear case.}
...
Due to the relativistic boost the energy of the photons will look as if $2\gamma$ higher and we will be in the regime where the Klein-Nishina equation already indicates a headlight effect of the radiation being preferentially emitted in forwards direction.

\EliasComm{Maybe combine bits.}
...
\EliasComm{Explicitly write out LAD.}

\subsection{Classical Landau-Lifschitz (LL) Description}

A first self-consistent solution to including radiation reaction into a classical model was achieved by Landau and Lifschitz (REF)\addref.
...
\EliasComm{Write out coherently}
...
\EliasComm{Maybe not in too much detail as this is more Methods than Theory. Maybe need high fields and high energies, either accelerator (conventional or plasma) and laser or high fields in crystal lattices.}
...
\EliasComm{Change intro, not questions. Proposed by Dirac.}
...
\EliasComm{Better intro, say other phenomena can also be reached with these facilities.}
