...
This chapter lays the theoretical groundwork to understand the research presented in the Results chapters later in this work, addressing laser wakefield acceleration, radiation production from charged particles and the fundamental processes of radiation reaction and Breit-Wheeler pair production.

First, fundamental single particle motions are derived that are related to the motion of electrons in wakefield accelerators but also consist the basis for the working of a magnetic spectrometer.

Secondly, the propagation of a laser in a medium and its non-linear evolution, in particular plasmas, is discussed as this paves the way for the third section on laser wakefield acceleration.

Then, radiation production from electrons in different scenarios is discussed, starting with radiation in synchrotrons and insertion devices, then drawing the parallel to betatron radiation produced in wakefield acceleration and the radiation emitted in a laser wiggler, namely inverse Compton scattering in moderately and highly intense laser fields.

This leads over to the topic of radiation reaction, the knockback force a particle experienced when emitting a photon, that can be investigated through inverse Compton geometries.
...
\EliasComm{Based on Jon Wood's thesis.}
...
\caption{Plasma frequency and wavelength as function of plasma density. A PLOT OF SLAB DISPLACEMENT NEXT TO THIS.}
\end{figure}
...
\EliasComm{Add here more details, can find parts in Jason's thesis.}
...
These conditions are re-constructed in laser and accelerator facilities interacting ultra-relativistic particles with laser pulses. Radiation reaction was measured, for instance, at SLAC (REF E144), and at the LHC (REF ULRIK CRYSTALS) and first measurements have been undertaken at non-conventional accelerators such as the Astra-Gemini laser system at the Central Laser Facility in the UK (REF COLE, REF PRX). This work is part of this thesis (see Chapter XX RR).
\vspace{\baselineskip}
