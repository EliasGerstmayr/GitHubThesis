...
In the case of waves with an electric field $\mathbf{E} = \mathbf{E}(\mathbf{r},t) = \mathbf{u}(\mathbf{r}) v(t)$, where the spatial and temporal dependencies separate, the components solve two separate differential equations 
\begin{align}
...
\end{equation}
This family of solutions is referred to as Gaussian modes and the physical properties of the beam is captured in the parameters of the wave function. This beam focuses down to a spot of size $w_0$ at $z=0$ and then expands again.
...
\end{equation}
The wavefront is strongly curved around the focus $z < z_R$ and slowly flattens out at $z \gg z_R$.
...
\begin{equation}
\eta = 1 - \frac{1}{2}\frac{\omega^2_p}{\omega^2_0}\left(1+ \frac{\delta n}{n_0} - \frac{2 \delta \omega_L}{\omega_0} - \frac{a^2_0}{4}\right).
\end{equation}
...
\begin{equation}
P_c \approx 17 \frac{\omega^2_0}{\omega^2_p} \,\mathrm{GW},
\end{equation}
...
Since a laser pulse of finite duration requires a certain frequency bandwidth, the group velocity also shows a spread and results in different parts of the beam propagating faster than others:
\begin{equation}
...
The pulse duration after propagating a length $l$ is hence
\begin{equation}
\tau = \tau_0 - \frac{n_0 l}{2c n_c}.
\end{equation}
...
\EliasComm{Short paragraphs on the key theory bits without deriving it all.}
...
\EliasComm{add plot of dephasing length.}
...
The photon on the other hand has initally a momentum $\mathbf{p}_{\gamma, initial}$ and an energy $E_{\gamma, initial} = p_{\gamma, initial} c = hf$, where $h$ is Planck's constant and $f$ the frequency of the photon. The subscript $\gamma$ represents the photon in this interaction.
...
(E_{\gamma, initial} - E_{\gamma, final} + m_e c^2)^2 - (m_e c^2)^2 &= (p_{e, final} c)^2.
\end{align}
...
p_{\gamma, initial} - p_{\gamma, final} &= p_{e, final}\nonumber\\
(p_{\gamma, initial} - p_{\gamma, final})^2 &= (p_{e, final} )^2\nonumber\\
(p_{\gamma, initial}c)^2 + (p_{\gamma, final}c)^2 - 2 p_{\gamma, initial}p_{\gamma, final}c^2\cos(\theta) &= (p_{e, final} c )^2,\nonumber\\
E_{\gamma, initial}^2 + E_{\gamma, final} - 2 E_{\gamma, initial} E_{\gamma, final} \cos\theta &= (p_{e, final} c)^2.
\end{align}
