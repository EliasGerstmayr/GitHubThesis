...
\frac{\mathrm{d}\sigma}{\mathrm{d}\cos{\theta}} &= \frac{\pi \alpha^2}{m^2}\left(\frac{E_f}{E_i}\right)^2 \left(\frac{E_f}{E_i} + \frac{E_i}{E_f}-\sin^2{\theta}\right),
\end{empheq}
\end{subequations}
...
\begin{equation}
\boxed{\frac{\mathrm{d}\sigma}{\mathrm{d}\cos{\theta}} = \frac{\pi \alpha^2}{m_e^2} (1 + \cos^2\theta),}
\label{Theory:Eqs:ThomsonXsec}
\end{equation}
which is independent of photon energy and has a total cross section of $\sigma_{total} = 8\pi\alpha^2/(3m_e^2)$. This is the classical Thomson scattering cross section, $\sigma_T$.

The total cross section of the Klein-Nishina equation is:
\begin{equation}
\sigma = \frac{3}{4}\left[\frac{1 + x}{x^3}\left( \frac{2x(1+x)}{1+2x} - ln(1+2x)\right)\right] + \frac{1}{2x} ln(1+2x) - \frac{1+3x}{(1+2x)^2},
\end{equation}
...
\begin{equation}
\frac{\mathrm{d} \sigma'}{\mathrm{d}\cos \theta'} = \frac{\pi \alpha^2}{m^2} \left(1+\cos^2 {\theta'}^2\right).
\end{equation}
...
\EliasComm{Add plot showing the transformation of the cross section.}
...
\begin{figure}
\centering
\includegraphics[height=0.3\columnwidth]{Aivazis_PairAnnihilation.pdf}\hspace{1em}%
\includegraphics[height=0.3\columnwidth]{Aivazis_BreitWheeler.pdf}\hspace{1em}%
\includegraphics[height=0.3\columnwidth]{Aivazis_BetheHeitler.pdf}
\caption{Feynman diagrams for, from left to right, pair annihilation, linear Breit-Wheeler pair production and Bethe-Heitler process. Time axis from left to right.}
\label{Theory:Figs:Feynman:PairAnnihilation:PairProductionBWBH}
\end{figure}

The annihilation of an electron and a positron into two photons, matter transformed into light, was proposed by Dirac and is hence referred to as Dirac annihilation. It was experimentally confirmed by Klemperer in 1934.
...
\end{equation}
or in terms of the Feynman diagram in Figure \ref{Theory:Figs:Feynman:PairAnnihilation:PairProductionBWBH}.
...
The total cross section for the process is given by (REF\addref)
\begin{equation}
...
The threshold is $s > 1$.
 \cite{PikeThesis} ($\sigma \sim 2 \beta^2 \sigma$ relating annihilation and pair production
