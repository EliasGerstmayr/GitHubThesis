...
\EliasComm{In previous sections discussed the emission of radiation from charged particles.}
\EliasComm{Based on conservation rules the particles lose energy and experience a knockback based on the momentum/energy of the radiation emitted.}
\EliasComm{Knockback force can significantly alter the particle dynamics in strong electromagnetic fields.}
\EliasComm{In most daily scenarios the force acting on the emitting particles is small and is accurately represented by a slow continuous force (for instance in accelerators) on emitting particles but in most scenarios not important as it is a slow continuous force.}
\EliasComm{In astrophysical phenomena/context and in future accelerators this becomes more important. In addition this can be probed in high intensity laser fields.}
\EliasComm{Here a single particle can emit multiple photons in a short time and the radiation reaction force has to be considered.}
\EliasComm{In extreme cases the energy emitted approaches the initial energy of the particle, for which we have to model the process in the quantum model, i.e. consider quantum radiation reaction.}
\EliasComm{This is radiation reaction and will discuss this fundamental phenomenon in this section. No universally accepted practicable solution across regimes.}
\EliasComm{This will be probed in future high-intensity laser interactions.}
\EliasComm{Will initially introduce classical descriptions of radiation reaction and motivate limitations.}
\EliasComm{Then introduce quantum corrections/semi-classical and quantum radiation reaction, and how they differ from the classical description.}
...
Problems: energy loss can exceed initial energy for abruptly changing fields (REF Baylis PLA 2002). Stochasticity not included.
