...
\end{equation}
\EliasComm{Make Feynman diagram for single photon and then for two to show how many diagrams one would have to solve.}
...
\end{equation}
including the effect of the mass increase on the energy of the radiation in the laboratory frame is given by
\begin{equation}
E_f \approx \frac{2 \gamma^2 (1- \cos \theta)}{1+a_0^2/2 + \gamma^2 \theta^2}n E_i a_0,
\end{equation}
which for higher $a_0$ leads for a fixed harmonic $n$ to a redshifting of the radiation $E_f \sim 1/a_0$. This becomes already non-negligible for $a_0 \sim 1/10$ (REF\addref).
...
The emission/scatter rates for the $n$-th harmonic depend on the laser polarisation (Ritus REF, Tom Thesis REF\addref).
The rates (averaged spin, in and out, unpolarised states) for $n$ scattered photons for circular polarisation
\begin{equation}
...
\begin{equation}
\frac{\dif W_n}{\dif x} = \frac{2 \alpha m^2}{\pi E}\int^{\pi/2}_0 \left[-A^2_0 + a^2_0 \left(1-x+ \frac{1}{1-x}\right) (A^2_1 - A_0 A_2)\right]\dif \phi.
\end{equation}
The frequency of the incoming photons is $\omega$, the scattered photon carries an energy $\omega' = xE$.
$J_n$ is shorthand for $J_n(z)$, and $A_i = A_i(n,a,b)$, with
\begin{equation}
z = \frac{2 a_0}{\sqrt{1+a^2_0}}\frac{\sqrt{x(nu - (1+nu)x)}}{u(1-x)}.
\end{equation}
The function $A_i$ is defined as 
\begin{equation}
A_i(n,a,b) = \frac{1}{\pi}\int^\pi_0 \cos^i \phi \cos [(a+2b\cos \phi) \sin \phi - n\phi] \dif \phi,
\end{equation}
with $a$:
\begin{equation}
a = \frac{\sqrt{8}Qn a_0}{1+a^2_0 + Q^2}\cos \phi,
\end{equation}
and $b$:
\begin{equation}
...
\EliasComm{Reproduce KN eqns at low intensities.}
...
\EliasComm{Energy losses were ignored in equation of motion.}
\EliasComm{Knockback force on emitting particles but in most scenarios not important as it is a slow continuous force.}
\EliasComm{Becomes important in astrophysical context. REFS and future accelerators.}
\EliasComm{When multiple emissions by a particle in short time, we have to consider it in the equation of motion.}
...
\EliasComm{Start with classical model and then quantum corrections.}
