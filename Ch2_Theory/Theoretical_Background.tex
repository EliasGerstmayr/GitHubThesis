...
This is based on Jon Wood's thesis!
...
The velocity of the electron is $\beta c$ for $\beta \rightarrow 1$ in the highly relativistic case.

The Lagrangian of this scenario is described by

\begin{equation}
L = - m_e c^2 \sqrt{1-\beta^2} + e \phi,
\end{equation}
...
Assume there is no axial acceleration field. Assume the radial component of the velocity is small compared to c and the longitudinal component be close to c. $\dot{\gamma}$ is then close to zero.
...
\begin{equation}
\frac{d}{dt} (\gamma m_e \dot{z}) = \frac{\partial }{\partial z} L = e E_z
\end{equation}

This time the change in momentum in time is not negligible.
...
For relativistic electrons the velocity in z will be close to 1 and we can read off the change in momentum with time as $\dot{\gamma} = eE_z/m_e c$
...
\subsubsection{Low Intensity behaviour at low electron energies}

At low laser intensities and electron energies the scattering cross section is symmetric in forwards and backwards direction of the incoming photon. It forms two lobes.
From the Klein-Nishina formula based on energy and momentum conservation we see that each angular position in the lobe corresponds to a fixed energy with maxima on the forwards-backwards axis.

\subsubsection{Low intensity behaviour at relativistic electron energies}

At low laser intensities in the limit $a_0 \rightarrow 0$ and relativistic electron energies the interaction looks fairly similar in the rest frame of the electron. In the lab frame however the emitted radiation is now preferential in the backwards direction (the direction of the electron propagation). This is sometimes referred to as `headlight effect'. The electrons are passing through the laser pulse and oscillate up and down. The maximum energy is of order $\hbar \omega' \approx 4 \hbar \omega a_0 \gamma^2,$ based on the Lorentz boost. This energy is again located on the forwards-backwards axis. The cone of emitted radiation is of order $1/\gamma$.

\subsubsection{Rising laser intensity at relativistic electron energies}
...
\subsubsection{Ultrahigh intensities and relativistic electron energies}
