...
The energy of electrons in wakefield experiments can be determined in experiment using an electron energy spectrometer based on magnetic dispersion. The electrons are deflected using a dipole magnet and detected on a scintillator screen.
...
We choose the coordinate system such that $\mathbf{B} = (0,0,B)^t = B e_z$. This immediately gives us the result that the motion of the electron consists of two decoupled motions: one at the constant, initial velocity parallel to the magnetic field vector in z, the second motion is a circular motion in the x-y plane. This is called a cyclotron motion. For now we will ignore the dissipation of energy in form of radiation due to the acceleration of the electron and accept the result that the Lorentz force conserves the total momentum and energy of the particle in this case.
\vspace{\baselineskip}
...
Now consider an external magnetic field restricted to a circle of radius $r_b$ embedded in a completely field-free region. The following derivation is based on Stuart Mangles' (Imperial College) PhD thesis.
\begin{align}
...
\begin{figure}
\centering
\includegraphics[width=0.5\columnwidth]{Bfield_analytic.png}
\caption{Visualisation of an electron deflected in a homogeneous circular magnetic field (purple) with radius $r_b$\protect\footnotemark. The larger green circle indicates the Larmor radius, determining the electron trajectory (orange) from the point of entry into the field (A) to its exit point (C) resulting in a total deflection $\theta$.}
\end{figure}

\footnotetext{Image taken from Stuart Mangles' (Imperial College) PhD thesis.}
...
In a typical LWFA experiment $p_\perp$ will be dominated by the component on the laser propagation axis. 

\EliasComm{Write about Sokolov-Ternov effect here or later during radiation reaction?}

\subsection{Electron motion in a laser field}
...
\caption{Electron motion in an infinite, plane, linearly polarised EM wave in the lab frame (left), normalised by $a_0/k$ in x and $a^2_0/k$ in z, with the wavenumber $k = \omega/c$. On the right the figure-of-eight motion in the drift frame.}
\label{Theory:Figs:FigureOfEight}
\end{figure}


\section{Fundamental QED}

\subsection{Furry-picture and Volkov states?}

\section{Fundamental Interactions}

Here an overview of all tree-level diagrams
...
Classical Compton scattering describes the inelastic scattering of a photon off an electron which is at rest or close to it.
...
We can derive this by calculating the matrix element for the process. This derivation is based on Peskin and Schroeder REF.
...
The Klein Nishina Formula! At high energies.... see ICS.

\subsection{Inverse Compton Scattering (ICS)}
...
In inverse Compton scattering (ICS) a relativistic electron scatters with one or multiple photons. Whilst in Compton scattering (CS)  momentum is transferred from the photon to the electron (heating), resulting in a longer wavelength of the scattered photon, in ICS the photon gains energy (hence inverse Compton scattering) as it experiences a relativistic Doppler shift.

In a head-on collision the photons are boosted by $(2\gamma)^2$ to an energy $E' = \hbar \omega'$ of
\begin{equation}
\hbar \omega' \approx 4 \hbar \omega a_0 \gamma^2,
\end{equation}
where $a_0$ is the normalised vector potential. In case of very intense laser fields one electron can interact on average with $a_0$-times photons and re-emit them as one.
\vspace{\baselineskip}

The scattering process itself is in principle the same as for CS, except for the relativistic treatment. Hence, it is not surprising that the cross sections for ICS and CS are are identical, as naively expected.

\EliasComm{add some parts here from Jason's calculations.}

However, the photon energies accessible with sufficiently relativistic particles is multiple orders of magnitude higher. In all optical Compton sources \cite{TaPhuoc2012a} relativistic electrons of several hundreds of $\mathrm{MeV}$ have been collided with high-intensity laser pulses, producing tunable X-rays in the $\mathrm{keV}$ \cite{TaPhuoc2012a,Powers2014,Khrennikov2015} to $\mathrm{MeV}$ \cite{Chen2013a,Sarri2014} range with pulse durations of the order of tens of femtoseconds and a micron-scale source size.
...
\EliasComm{This is being used for polarisation measurements.}

\subsection{Electron-positron pair annihilation}

A well known and frequently observed process is the annihilation of electrons and positrons into two photons. The rest energy of both particles is converted into energy.

\begin{equation}
e^- + e^+ \longrightarrow \gamma + \gamma
\end{equation}

The cross section can be easily calculated by re-using the result for the Compton cross section we derived earlier. Both Feynmann diagrams are very similar and the cross-sections are relatable by making use of crossing symmetries.

Dirac (Prc Camb Phil Soc 26, 1930)

In the centre of mass frame the electron and the positron are heading for a head-on collision, each with a momentum p (with opposite signs). The related differential cross-section is:
\begin{equation}
\frac{\mathrm{d}\sigma}{\mathrm{d}\cos{\theta}} = \frac{2\pi \alpha^2}{s} \frac{E}{p}\left( \frac{E^2+p^2\cos^2\theta}{m^2+p^2\sin^2\theta} +\frac{2m^2}{m^2 + p^2 \sin^2 \theta}-\frac{2m^4}{(m^2+p^2\sin^2\theta)^2} \right).
\end{equation}

For $E \gg m$ this simplifies to:
\begin{equation}
\frac{\mathrm{d}\sigma}{\mathrm{d}\cos{\theta}} = \frac{2\pi\alpha^2}{s}\left(\frac{1+\cos^2\theta}{\sin^2\theta}\right).
\end{equation}

\subsection{2 photon annihilation: Linear Breit-Wheeler Process}


The inverse process of the electron-positron pair annihilation is the more elusive linear Breit-Wheeler process. Whereas the electron-positrons easily annihilate when close enough to each other, the inverse process - creating matter in form of an electron-positron pair from light - is much more difficult to achieve. The two photons have to have a centre-of-mass energy in excess of the pair rest energy of $2\times 511\,\mathrm{keV}$. Relatively high energetic photons like this are hard to produce in large numbers. Typical energies at synchrotron sources with large photon numbers are in the few keV region. Typical processes that allow reaching high energies, for instance Bremsstrahlung, are not directed and emit all their photons in $4\,\pi$ reducing the number of usable photons in an interaction drastically.

This might explain why the linear Breit-Wheeler process is the only fundamental tree-level interaction that has not been observed yet in experiment.

\begin{equation}
\gamma + \gamma \longrightarrow e^- + e^+
\end{equation}

Breit, Wheeler (Phys Rev 46, 1934)

Cross section X RIbeye

\begin{equation}
\frac{\mathrm{d}\sigma}{\mathrm{d}\Omega} = \frac{1}{64 \pi^2 s}\frac{2|p|}{\sqrt{s}}|M|^2,
\end{equation}

\begin{equation}
|M|^2 = 2e^4 \left[\frac{u-1}{t-1} + \frac{t-1}{u-1} - 4\left(\frac{1}{t-1} + \frac{1}{u-1}\right) - 4\left(\frac{1}{t-1}+\frac{1}{u-1}\right)^2\right],
\end{equation}

use CM frame velocity?
$\mathrm{d}\Omega = \mathrm{d}x\mathrm{d}\phi$ and $x=cos\theta_x$.

\subsection{Many photon annihilation: Nonlinear Breit-Wheeler Process}

Whereas it is difficult to interact two photons with the sufficient energy, it is much easier to find very intense light sources with many photons at lower energy, a laser for instance, to interact with one high energetic photon. This is the nonlinear Breit-Wheeler process.

\begin{equation}
n\gamma + \gamma \longrightarrow e^- + e^+
\end{equation}

Observed at SLAC (Burke PRL 79, 1997 REF)
Bamber PRD 60, 1999

(other processes are Gahn 2000 APL 77)
(Chen, PRL 102, 2009)

\subsection{Pair production in a nuclear field: Bethe-Heitler Process}

Bethe, Heitler (Proc R Soc Lond 146, 1934)
...
No tree-level diagram, just higher orders are possible.

\section{Radiation Production}

Charged particles always then produce radiation when accelerated.
...
The radiation emitted is commonly called bremsstrahlung, which translates in braking radiation.
...
Radiation reaction is a very fundamental problem of physics and a universally accepted description does not exist. Each description only exists in a certain limit and approximation.
...
Classical or quantum models to derive emitted power
Approximations?

Observables:
Energy Loss in the electron spectrum
Emission Spectrum of the gamma rays
Variance of the electron spectrum



\subsection{Physics beyond the Local constant cross field approximation}

Recent research aims at investigating.


\section{The Schwinger Limit}

The vacuum as described through quantum electrodynamics (QED) is not as void and empty as our classical understanding implies. The vacuum has energy, polarisation and is in general a lively space where every fraction of a second a vast number of particles are created and annihilated or as Feynman puts it: ``created and annihilated, created and annihilated -- what a waste of time".
Whilst the vacuum energy does not affect us in our daily lives and for most of physics done so far the classical description of a true vacuum might be a sufficient approximation, it opens up a variety of interesting phenomena, such as the vacuum polarisation or its break-down in an intense laser field, to investigate as the tools of physics become more powerful. 
\vspace{\baselineskip}

Julian Schwinger calculated under which conditions the properties of the quantum vacuum would become dominant or in other words when the vacuum would break down resulting in the production of electron-positron pairs \cite{Schwinger1951}. Although already considered before, Schwinger was the first one to properly understand the limit and its consequences. He calculated that an electric field strength of $E_S = \frac{m_e^2 c^3}{e \hbar} \approx 1.3 \cdot 10^{18} \,\mathrm{V/m}$, in terms of laser intensity around $I_S \approx 2 \times 10^{29}\, \mathrm{W/cm^2}$, would be required to accelerate the virtual particle pairs enough to mitigate the annihilation, i.e. to create particles out of the vacuum. This is the field strength required to accelerate an electron-positron pair over the distance of a Compton wavelength within the uncertainty of their production. $E_S$, however, is incredibly large. In terms of laser intensity it corresponds to an intensity of $2 \times 10^{29} \,\mathrm{W\, cm^{-2}}$, which is around 6 orders of magnitude above what is achievable with laser systems today and probably beyond the technological scope the near future at least. Nevertheless, if these predictions are true, they would pose a real definite limit on the peak intensity of lasers as pulses could not propagate or focus down to higher intensities as they would lose energy dramatically to the creation of particle cascades.
\vspace{\baselineskip}

Even though the Schwinger limit is beyond our reach for now, that does not mean that interesting quantum phenomena in intense fields ca not be investigated today already.

For instance, the effective field of a laser pulse seen by an electron can be enhanced in so-called colliding pulse experiments, where a laser pulse and electrons or even gamma rays are counter-propagated.
When colliding highly relativistic electrons head-on with a intense laser pulse, the electric field of the laser will be boosted by a factor of $2\gamma$ in the rest frame of the electrons, where $\gamma$ is the relativistic Lorentz factor of the electrons. In this geometry the quantum dominated regime becomes accessible already with existing laser systems, enabling probing of (quantum) radiation reaction and the production of matter through a combination of the Breit-Wheeler \cite{Breit1934} and the Schwinger mechanism \cite{Schwinger1951}. However, also alternative approaches have been considered to create electron-positron pairs via the Breit-Wheeler mechanism relying on a hohlraum design \cite{Pike2014}.
\vspace{\baselineskip}

In the case of counter-propagating photons and particles the `quantumness' of a reaction, i.e. how dominant quantum effects are expected to be, is not only the ratio of the laser field to the Schwinger field but it gains an additional factor of $2\gamma$ as well, expressed in the parameter $\eta$ \cite{Blackburn2014}:
\begin{equation}
\eta = 2 \gamma \frac{E_L}{E_S},
\end{equation}
where $E_L$ is the electric field of the laser and $E_S$ the Schwinger field. Alternatively, this can be also expressed in terms of the laser intensity $I_L$ and the intensity of the Schwinger field $I_S$:
\begin{equation}
\eta = 2 \gamma \sqrt{\frac{I_L}{I_S}}.
\end{equation}

\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{quantumness_contour_grey.png}
\caption{The diagram shows the combination of laser intensities and electron energies required to reach a certain value of $\eta$, indicated by the coloured contour lines. The plot demonstrates how significantly higher values of $\eta$ can be achieved when using the same laser but relativistic counter-propagating particles.}
\end{figure}

\EliasComm{maybe a section on high-field phenomena: cascades, vacuum birefringence, radiation reaction, Breit-Wheeler.}





\section{Introduction to Plasmas}

\subsection{General Properties}

Plasmas are frequently referred to as the `4th state of matter' in addition to the more commonly in daily life encountered matter states solid, liquid and gaseous. Heating up a material at a constant pressure will take it from the solid to the liquid, then to the gas phase and finally will ionize the gas first partially, then completely. The gas is now charged and exhibits interesting collective behaviour due to the dominance of electromagnetic forces whilst upholding quasi-neutrality. This is called a plasma. Sometimes summarised in this definition: ``A quasi-neutral ionized gas that exhibits collective behaviour.'' However, setting `ionized gas' equal to `plasma' might be a too narrow description as the range of temperature and density scales is very wide for plasmas, including exotic states like the quark-gluon plasma. Although plasmas are probably the most common state of matter in the universe, they barely exists on earth outside of the laboratory environment.
\vspace{\baselineskip}
\EliasComm{Phase Diagram including plasma?}
...
\subsection{Laser Ionisation}

Ionisation effects can influence the dynamics of a system. In the context of the work the author presents high intensity lasers are used to drive waves in plasmas. The plasma is usually formed by filling a gas compartment or firing a gas jet with a neutral gas, assuming that the laser pulse ionises the gas fully.
\vspace{\baselineskip}
...
\end{equation}
where the position of the maximum potential is given by $r_{max} = (Ze/4\pi\epsilon E)^{1/2}$. Using this result and by setting the height of the barrier to the ionisation potential $V_e$, the electric field required comes to

\begin{equation}
E = \frac{\pi \epsilon_0 V^2_e}{Ze^3},
\end{equation} 

or in terms of intensity using $I = \frac{c\epsilon_0}{2}|E|^2$ in (near) vacuum:

\begin{equation}
I = \frac{c\epsilon^3 \pi^2 V^4_e}{2e^6 Z^2}.
\end{equation}
...
The plasma frequency $\omega_p$ is the natural frequency at which electrons will oscillate in a plasma when displaced and constitutes at the same time a typical time scale of the medium. The corresponding plasma wavelength $\lambda_p$ on the other hand constitutes a typical length scale.
\vspace{\baselineskip}

Assume a uniform and neutral plasma, i.e. $n_e = Z n_i = n_{e0}$, where $n_e$ is the electron number density, $Z$ the charge number of the ions, $n_i$ the ion number density, and the electric field $E = 0$.
...
\begin{equation}
E_x = \frac{1}{\epsilon_0} \int^x_0 \rho (x') dx' + E_x(0) = \frac{e n_{e0} X}{\epsilon_0}.
\end{equation}

\begin{equation}
E = \frac{e n_e X}{\epsilon_0}
\end{equation}
...
\begin{equation}
\frac{m_e \mathrm{d}^2 X}{\mathrm{d}t^2} = - \frac{e^2 n_e}{\epsilon_0} X = - \omega^2_p X,
\end{equation}

with the (plasma) frequency

\begin{equation}
\omega^2_p = \frac{n_{e0} e^2}{\epsilon_0 m_e}.
\end{equation}

\EliasComm{Add some typical numbers used in wakefield accelerators.}
...
Similarly to electrons in an insertion device (undulator or wiggler) the oscillations lead to the emission of radiation, collimated due to the relativistic forward momentum of the particles.
