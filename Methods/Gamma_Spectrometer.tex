...
A long array of scintillating crystals is placed in the beam path and the scintillation signal is measured with a CCD. The spectrum of gamma radiation passing through the crystals can be inferred by simulating the response of the array in GEANT4 and comparing it to the measured response in the experiment \cite{Behm2018_Gamma}.
...
\caption{Sketch of crystal orientations (cyan) in the spectrometer arrays relative to the incident gamma rays (green) for the RAL stack (left) and the alternating layers in the new long stack (right). The yellow faces indicate the open sides out of which the scintillation light (green) leaves the crystal.}
...
The specific steps vary from setup to setup depending on the imaging system. The open side of the crystals (see Figure \ref{Methods:Figs:GammaSpectrometer_Axes} were imaged close-to-normal to collect a large fraction of the scintillation light, which also removed the necessity of morphing the image as required for the magnetic spectrometer screen, for instance.

The images contain background noise from different sources that have to be removed. The background can roughly be divided into `permanent' background and on-shot noise. The permanent noise includes the intrinsic `dark noise' on the camera chip and counts related to ambient light.
This background can be characterised by taking a series of images at dark conditions and then subtracted from on-shot data.

On-shot additional noise from hard radiation or electrons directly hitting the CCD (`hard hits' or `hot pixels'), scattered laser light and a physical bremsstrahlung background is measured. The treatment of the physical bremsstrahlung background is more complicated and will be discussed later.
Scattered laser light can be mitigated by putting light shielding in place and adding bandpass filters matched to scintillation light to the imaging system. Hard hits, single pixels with high or saturation value, can be removed by a median filter which will also result in some blurring of the image.
