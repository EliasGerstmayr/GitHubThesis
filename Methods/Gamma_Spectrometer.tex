...
To infer the spectrum of the gamma radiation, a long array of scintillating crystals is placed in the beam path and the scintillation signal is measured with a CCD. By simulating the response of this scintillator array in GEANT4 and comparing it to the measured response in the experiment, the spectrum can be inferred \cite{Behm2018_Gamma}.
A conceptual sketch of the experimental setup is shown in Figure \ref{Methods:Figs:SketchGammaSpec}: a beam of gamma rays enters from the left side of the sketch, passes through a vacuum window (orange) into air, then traverses a lead collimator and enters the scintillator array. The response of the detector is imaged by a camera (bottom right corner). 
...
The individual scintillators are caesium-iodide crystals doped with thallium, with the dimensions $5 \times 5 \times 50\,\mathrm{mm}$. 
The long side of the crystals is oriented transversely to the propagation direction of the gamma rays ($z$), in the sketch shown in Figure \ref{Methods:Figs:SketchGammaSpec} this is the horizontal axis $x$. This maximises the light yield as the response of the detector is integrated along this axis ($x$) at cost of spatial resolution. The crystals are arranged in a two-dimensional grid of columns ($z$) and rows ($y$). Crystals along the longitudinal axis $z$ measure the decay of the radiation and can be used to infer the spectrum. The response in the $y$-axis encodes the vertical divergence of the radiation. 

The crystals are separated from each other by light-tight spacers such that each crystal is enclosed from all but one side, from which the scintillation light leaves the crystal. This crystal side faces in $x$ direction and the scintillation light is measured on a CCD. In all the experiments discussed in this thesis cooled 14-bit EMCCD cameras of type Andor iXon were used, which are very sensitive and allow for high gain but in turn also require efficient light shielding. The cameras were equipped with suitable objectives and bandpass filters that only transmit light in the bandwidth of the scintillation light at a wavelength of $546\pm10\nm$. A reflective casing for the crystals along with polishing and reflective coating of the crystals can increase the light output. The material and thickness of the dividers between the crystal columns determine how fast the incoming radiation decays which can be used to `tune' the response of the detector for the right spectral range. The separation and thickness of the crystals in the $y$-axis, on the other hand, determines the vertical spatial or with respect to the source angular resolution.
\vspace{\baselineskip}
...
\begin{figure}
\centering
\includegraphics[height=0.3\columnwidth]{GammaSpecCrystals_2.pdf}\includegraphics[height=0.3\columnwidth]{GammaSpecCrystals_3.pdf}
\caption{Sketch of crystal orientations in the spectrometer arrays for the RAL stack (left) and the alternating layers in the new long stack.}
...
\EliasComm{Change phrasing.}

Another new feature is the alternating orientation of the crystals (see Figure \ref{Methods:Figs:GammaSpectrometer_Axes}, right): the long side of the crystals is now not in every column aligned with the horizontal $x$-axis, but in every second layer instead rotated by $90$ degrees aligning with the $y$-axis (vertical). Each odd and even layer now integrates the response of the detector in a different transverse dimension. The preserved non-integrated spatial component then also alternates with each subsequent column. This is discussed in more detail at the end of this section.

In the setup used in the context of Chapter \ref{Chap:linICS} the top of the detector ($y$) was imaged directly and a long rectangular mirror was placed at 45 degrees next to the other side, reflecting the scintillating light of both integrated views onto the same CCD chip using the area of the rectangular CCD chip most efficiently. Due to the length of the stack ($z$) two Andor iXon cameras were used, one for the front section and one for the back. 
...
The specifically used GEANT4 code used in this work is based on previous work by K. Poder, J. Cole and K. Behm.
...
\end{figure}
\SMComm{There are photons on the simulation image. Mention.}

In this code $10^{6}$ individual photons or $10^5$ electrons are simulated, assuming collective or multi-photon/-particle processes are negligible, such that individual photon responses can be combined cumulatively. The specific experiment geometries are reconstructed in GEANT4 including the detector geometry and materials in the pathway as well as distances measured in the experiment. The electrons and gamma rays are emitted from a point source and low divergence, close to an idealised pencil beam. The GEANT4 representations of the two spectrometer designs are shown in Figure \ref{Methods:GSpec:GEANT4screenshots}.
The code measures the energy deposition in the scintillator material and does not model the physics of the scintillation process or the light transport. It is assumed that the energy deposited within the scintillator crystals is linearly related to the light yield of the scintillator, i.e. the number of fluorescence photons emitted, independent of the process of energy deposition (REF\addref)


\subsubsection{Image processing}

In this setup, the response of the detector is recorded as an image on a CCD and the images taken require some processing before analysing them.
The specific steps vary from setup to setup depending on the imaging system.

Ideally, the crystals should be imaged close-to-normal as a large fraction of the scintillation light will be emitted in forwards-direction and the collection efficiency will drop off off-normal.
\SMComm{There is always an angle,.. what makes it relative? Keep the sentence linked to previous one, which was
about imaging at normal...}
If there is a relative angle between the stack and the camera, a projective image transform can correct the perspective, similarly as described for the magnetic spectrometer setup previously (SIMILARLY TO THAT DESCRIBED IN SECTION XX\addref). For normal imaging a simple rotation of the image will suffice. 
In some cases there might be aberrations in the system which lead to image distortion. In principle, this can be corrected using appropriate algorithms. In order to correctly apply transformations to the images, it is helpful to take images as spatial references (for instance rulers or well-defined grids).
...
\SMComm{In which case is there any need for the discussion above??}
\vspace{\baselineskip}
...
\SMComm{There is quite a lot of detail on background subtraction. Not a problem but it would be nice to see some
images pre and post big subtraction, to show that it’s effective,}
\vspace{\baselineskip}
...
\SMComm{Has the image been background subtracted?}
...
To further reduce the impact of noise and to focus on the signal-relevant parts of the image, we select the regions of the individual crystals (see Figure XX\addref{} for an example) and extract the content of the crystals. As a result we obtain the pixellated response of the detector, only using its active parts.
...
\SMComm{overlay is not the best word.}
\vspace{\baselineskip}
...
Since the primary noise source is off-axis, efficient shielding of the direct line of sight reduces the measured signal significantly. The source of the remaining radiation is likely to originate from a similar direction as the signal we aim to measure and will then enter the stack from one defined direction like the signal radiation. This makes it impossible to shield this component without infringing on the signal itself. Since it enters the stack from one defined direction, just as the signal, we can characterise its spectrum similarly as for the signal (GRAMMAR).
Depending on the radiation that is being measured, this might be an underlying source of noise that can skew the spectral measurement. The signal-to-noise ratio is an important quantity to estimate whether this becomes an important factor. If the perturbation is larger than XX\addnum{} we will have to correct it.
\SMComm{what do you mean skew. I t can have a very specific meaning (making a symmetric distribution
asymmetric) but I think you mean something more generic here. I would use a different word.}

In Chapter \ref{Chap:BW} the only radiation produced and measured is bremsstrahlung. The electrons are efficiently converted in a high-Z foil. The signal is very bright and the background is much smaller, will not affect fitting procedure (REF). Background is XXX times lower than signal (NOT COMPLETE SENTENCE).

In Chapter \ref{Chap:RR15} a bright signal of inverse Compton scattering is produced. Since we only selected the brightest signals, the impact of the bremsstrahlung is low. Background is XXX times lower than signal (NOT COMPLETE SENTENCE).
\SMComm{Was it included? Keegan did but not Jason.}

In Chapter \ref{Chap:linICS} radiation from linear inverse Compton scattering is measured. The signal is bright but the energy is lower than in the previous cases. The bremsstrahlung noise contributes to a skewing (!!!!XXXXX!!!) of the distribution. Background is XXX times lower than signal.
\vspace{\baselineskip}
...
\SMComm{Remaining after what. Not removed but accounting for it in analysis.}
...
In case of linear ICS the electron spectrum is unperturbed and this method can be used. In case of non-linear ICS the electron spectrum is perturbed but the bremsstrahlung is produced after the interaction, so can estimate that this is also the right method, but the signal is relatively bright. 
...
\SMComm{Qgamma not defined.}
\SMComm{This paragraph is important but has ended up waffling a bit. Keep the reader with you. here is how I
might try and say the same thing...
...
1) Linear ICS (chapter x): The background was characterised by a series of null shots (I.e no scattering laser, but full electron beam). The spectral shape of the background was found to be stable and proportional to the total energy (Qg2) in the electron beam. As linear ICS does not affect the energy of the electron beam this allows the measured electron spectrum on full shots to be used to estimate the bremsstrahlung background on each shot.
2) non linear ICS (chapter y): The background was characterised on a series of null shots and was also found to be proportional to Qg2. Although the energy spectrum is affected by collision with the laser on full shots, the bremsstrahlung sources are likely to be from interaction of electrons with material downstream from the collision point. This means it is reasonable to assume that the background on full shots is determined by the post collision Qg2.
3) bremsstrahlung (chapter z). The background, characterised on shots without the converter in place was much lower than the signal measured on shots with the converter. The background therefore has a negligible affect on the measured spectrum when the converter was used.}

In \cite{Behm2018_Gamma} a series of example shots was used to characterise the background contribution. By sampling from this distribution the error on the fit was estimated.
...
\SMComm{I don’t think this is clear enough for the reader. At the start of each subsection it’s good to have a very short sentence explaining what’s going to be done. But this one is too short and doesn’t give enough useful information.
In fact this first paragraph doesn’t seem to be needed. The second one introduces the problem (crystal, variability) the third one says how you’d like to deal with that (known source) and the next says using a combination of bremsstrahlung and GEANT4 sims we can deal with it}
...
\SMComm{I would out this as the first paragraph in this section.}
...
\caption[Example of simulated bremsstrahlung spectrum using electron spectra measured in experiment.]{Left: Two examples of electron spectra measured in an experiment that are used as input for GEANT4 bremsstrahlung simulation. Right: Corresponding simulated bremsstrahlung spectrum produced using 1 mm of bismuth as converter material. The spectrum is normalised by the number of electrons simulated.}
...
\SMComm{I doubt it. Is 9mm is enough to stop 500 MeV electrons? Probably more that we lose the signal on
detectors due to scattering}
\SMComm{It’s good to state what the different conditions were for each experiment, but I found it hard to keep it all in my head. It might be better to say various background methods were used and then put the information in a table?}

In Chapter \ref{Chap:BW}, iron and a $4\,\mathrm{mm}$-thick tungsten target are used. Used for calibration thick tungsten for wide signal. The electron spectrum was as before characterised and an average response was used as simulation input.
\EliasComm{Check which one was used.}

In Chapter \ref{Chap:linICS}, 1.6 mm of PTFE is used as converter, which predominantly scatters the relativistic electrons and affects their spectrum very little. GEANT4 simulations have also confirmed that the impact on the e-beam is negligible. This means the electron spectra can be measured on-shot and the close-to-exact input for GEANT4 simulations. The radiation produced from this converter target is emitted in a narrow cone and covers only a small transverse part of the detector. Several shots are being taken to ensure sufficient coverage of the detector relying on the spatial jitter of the electron beam. Additional calibration data just confirms the stability.
The exact on-shot electron spectra can then be used in GEANT4 to obtain a detector response that is more accurate than the one from the average spectrum.
\SMComm{Check this section for grammar - quite a few sentences are almost bullet points not proper sentences}
\vspace{\baselineskip}

After obtaining the experimental detector response and the response from the related GEANT4 simulation, we can compare both.
In Figure XX\addref{} an example of an experimentally measured and a simulation result for one detector row is presented. We see that the simulated response is very smooth in comparison to the measured response. The peaks in the experimental data are results of the varying light efficiency of the crystals (intrinsic to the crystals and the setup). 
...
\end{figure}
\SMComm{I would love to see how reliable this is shot to shot. Have you calculated the correction factor from a
number of shots?}
...
\SMComm{It’s a bit pedantic, but you don’t mean in chapter 5, you mean in the experiments reported in chapter 5...
watch out for this}
...
Other work has shown that the gamma spectrum can be retrieved without assumption of the spectral shape if a second measurement is performed, for instance using a Compton or pair spectrometer (REF\addref{}).
...
For a stack of XX this XX acceptance angle XX angular resolution of XX\addnum.
...
\end{figure}
\SMComm{Can you add a 3D sketch of the 3 views?
It would be really cool if you could display the data in a quasi 3D way too...
}
\SMComm{Can you align these? (Image)}
\SMComm{The bremsstrahlung one is a really nice example of it working, but the betatron one is less so - it’s not
clear which is the bigger dimension on the spectrum images.}
\SMComm{Betatron looks wide in either case.}
...
Measuring the spatial profile of radiation is interesting for different applications. In the context of inverse Compton scattering,  it can be used as beam diagnostics in the linear case (KRAMER REF\addref) and to infer the intensity at the interaction in a model-independent way from the ellipticity in the non-linear case (REF BLACKBURN, UMSTADTER, HARM SHEMESH\addref{}). For betatron radiation produced from wakefield accelerators, for instance, the ellipticity is an indicator for the wiggler parameter. However, a plain spatial analysis of the profile is easier done using a scintillator with much better resolution.
