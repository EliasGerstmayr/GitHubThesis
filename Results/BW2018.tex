...
This Chapter relates to an experimental campaign aimed at measuring the linear Breit-Wheeler process \cite{BreitWheeler1934_BW} conducted at the \textsc{Gemini} laser system in spring 2018. Two high-intensity lasers are used to provide highly energetic gamma radiation and an X-ray source that combined exceed the production threshold of a centre-of-mass energy of $\sqrt{s} > 1$.
...
\begin{figure}
\centering
\includegraphics[width=.9\columnwidth]{BWOverlapSketch.png}
\caption{Overlap sketch.}
\end{figure}


\begin{figure}
\centering
\includegraphics[height=.3\columnwidth]{GeTarget1mmOffaxis.jpg}\hspace{1em}%
\includegraphics[height=.3\columnwidth]{GeTargetOnAxis.jpg}\hspace{1em}%
\includegraphics[height=.3\columnwidth]{TungstenBlockOnAxis.jpg}
\caption{Alignment X-ray target.}
\end{figure}
...
\begin{itemize}
\item Both laser pulses are temporally and spatially overlapped
\item the temporal overlap to few ps pulse duration and such that the laser pulse is around 20 ps before the other on
\item X-ray target is heated when the short gamma flash arrives
\item gamma beam is cut down and has a spatial dimension of XX
\item X-ray target is expanding at 4 pi, so that after 20 ps the sphere is of size XX
\item including jitter this is okay
\item including temporal jitter of order 10s fs and drifts in 100 fs region this is okay
\end{itemize}
...
The spatial overlap of both photon sources is hence guaranteed on each shot. The temporal overlap is also secured as the North beam is a $40\,\mathrm{ps}$ long heater and timing fluctuations at \textsc{Gemini} have been measured to be of order of 10's of $\mathrm{fs}$ \cite{Shalloo_GEMINIDRIFT}.}
...
The central angle between the two photon sources was set in the experiment to be $140^\circ \pm 10^\circ$. Due to the strong collimation of the gamma-ray beam to $< 10\,\mathrm{mrad}$ and pointing fluctuations of $< 2\mrad$ the maximum spread of angles is $\theta < 0.7^\circ$ and hence negligible. In this setting the main angular spread is then dominated by the angular distribution of the $\sim 4\pi$ X-ray emission. Due to the wide emission angle of the X-ray source the geometry is less sensitive to angular and spatial variations on a shot-to-shot basis or between shot days, for instance when the tape target is being replaced. Considering this factor and since the analysis of the directionality of the X-ray source is still ongoing, we will for now also ignore any angular variation within the spectrum and calculate the cross-sections at a fixed crossing angle, $\theta$, and investigate how the cross-section, $\sigma_{BW}$, varies for a deviation in $\theta$. 
In Figure \ref{BW:Figs:Xsec_2DRange} (top) the cross section, $\sigma_{BW}$, is shown as a function of the invariant mass, $s$: $\sigma_{BW}$ is zero for $s<1$, then increases up to a peak value at $s = 2$ after which it decays slowly. Since $s \sim E_1 E_2$ or in this scenario $s \sim E_X E_\gamma$, the energy of the most likely interaction partner for one photon to produce a Breit-Wheeler pair with changes with its energy. For instance, the higher the gamma-ray energy, $E_\gamma$, the lower the X-ray energy, $E_X$, of the partner photon that would be most likely to produce a pair in an interaction.
