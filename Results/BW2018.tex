...
\BKComm{Can provide error for spot-size. Measured radiation spot size seems much larger than spot. Add blackbody spectrum to emission.}

The X-ray source was diagnosed by a pinhole camera setup with two $25\,\mathrm{\upmu m}$ tantalum pinholes, measuring the flux and the source size at a magnification $M = 2.03\pm0.02$. One pinhole was filtered with $5\,\mathrm{\upmu m}$ of aluminium, the other one with $6\,\mathrm{\upmu m}$ of PTFE. The X-ray source size measured in the experiment was $240\,\mathrm{\upmu m} \times 255\,\mathrm{\upmu m}$. A crystal spectrometer using a thallium-hydrogen-phthalate (TlAP) crystal measured the spectrum of the X-rays in a spectral window spanning $\sim 700\,\mathrm{eV}$ from just under $1.3\,\mathrm{keV}$ to $2\,\mathrm{keV}$ including emission lines around $1.5\,\mathrm{keV}$. Both cameras are back-illuminated deep-depletion in-vacuum X-ray CCD cameras (Andor DX420-BR-DD). Both measurements were conducted from the side of the target that was heated by the laser and are hence not spectrally filtered by the Kapton tape. 
...
\BKComm{Combine datasets and make thicker lines.}
...
\BKComm{Varying plasma conditions can also change ratio of emitted lines (peaks). E.g. a hotter plasma might be ionised more and shift lines/probabilities. Laser intensity can play a role if our spot was slightly different.}
\vspace{\baselineskip}
\begin{figure}
\centering
...
\BKComm{But emission into 4 pi? (exception being the angular dependent transmission through plastic backing)}
...
\BKComm{What X-ray yields are used here? You have photons per keV a few pages above, but you must have to assume some number of photons per ev PER STERADIAN here?  Also do you account for the differing pulse lengths at all? 40ps vs 40 fs. Do we miss any of the X-ray volume when we send the gammas through?}
...
\BKComm{Estimate how many positrons make it through?}
...
Only the positrons were measured as the Breit-Wheeler cross section peaks at $s=2$ resulting together with the asymmetry of the photon energies in an unequal momentum distribution which makes a coincidence measurement more difficult and not feasible in this setup (see Figure \ref{Theory:Figs:BW_DiffXsec_CM_E_LAB} in Section \ref{Theory:Subsec:BW} ).
...
\BKComm{Maybe improve electron source using density gradients to increase stability or change spectra.}
\BKComm{Effect on background noise? Showed it scales with gamma yield.}
\EliasComm{I guess this would need checking whether more yield results in more signal than it does for noise.}
...
\BKComm{Flash coating of aluminium to suppress this?}
\section{Conclusion}
