...
For each target and accelerator setting 10 shots were taken. 
Figure \ref{BW:figs:Gprofile_materials_montage} shows a montage of the averaged gamma profile response at each configuration.
\vspace{\baselineskip}
...
In all configurations thicker targets produce a higher yield than their thin counterparts of the same material, as expected from their respective thickness in radiation lengths. 
...
The transmitted gamma yield estimated this way is shown in Figure \ref{BW:fig:abs_transmission}. 
...
We will now characterise the gamma spectra and its variations for the relevant shot days at the optimum conditions identified before.
...
The response of the detector is dominated by the spectrum below $400\,\mathrm{MeV}$ and is not able to clearly resolve a spectral change as induced by the variation in spectra shown. 
An example of an experimentally measured response curve is shown in Figure \ref{BW:fig:GSpec_GEANT} (bottom right). Due to experimental uncertainties of background noise and imperfections in the detector setup, we are also not able to measure the small difference in the responses shown in Figure \ref{BW:fig:GSpec_GEANT} (bottom left), which would have given indication of significant changes in the spectrum. 
...
Using the reference electron spectra for each shot day, the corresponding gamma spectra can be simulated in GEANT4 and, using in turn these simulated spectra, an average gamma spectrum can be determined for each day (see Figure \ref{BW:fig:GSpec_AverageSpecVariations}). 
...
Similarly as before, we will characterise the properties of the source on the three shot days \textsc{a}, \textsc{b} and \textsc{c}. The average spectra measured by the crystal spectrometer on the three days, flat-field and background corrected, are shown on a logarithmic $y$-scale in Figure \ref{BW:Figs:Xray_Sim_and_Exp} (bottom). The standard deviation of the shot-to-shot fluctuations from the mean is indicated in the shaded regions. The range of X-ray energies shown on the $x$-axis is representative of the spectral range measured effectively with the crystal spectrometer setup throughout the three shot days. The measured spectrum falls off sharply on either side of the spectral window presented here as the end of the crystal is reached. The height of the spectra is indicative of the relative X-ray yield on the different days. The distinct peaks are emission lines of the material and slight spectral shifts are due to smaller calibration and alignment errors. Note that these emission lines are not shown in the simulated spectrum as the code is based on an average atom model that does not resolve the electronic structure well enough.
The spectral shape on day \textsc{b} and \textsc{c} is very consistent, whereas on day \textsc{a} the spectral lines above $1.4\,\mathrm{keV}$ are less pronounced and the spectrum falls off. This is probably due to varying plasma conditions as a hotter or colder plasma will shift the emission lines and rates. The fall off is more consistent with the simulated spectrum than the spectral shape observed on days \textsc{b} and \textsc{c}, which indicates that the plasma conditions are closer related as well. The spectrum on day \textsc{a} is also cut off at around $1.33\,\mathrm{keV}$, which is due to misalignment of the crystal and not a physical phenomenon. In general, the shape of the X-ray spectra is consistent from shot-to-shot, whereas the shaded regions indicate that the total yield of photons can change significantly. 
\vspace{\baselineskip}
\begin{figure}
\centering
...
The central angle between the two photon sources was in the experiment set to be $140^\circ \pm 10^\circ$. Due to the strong collimation of the gamma-ray beam to $< 10\,\mathrm{mrad}$ and pointing fluctuations of $< 2\mrad$ the maximum spread of angles is $\theta < 0.7^\circ$ and hence negligible. In this setting the main angular spread is then dominated by the angular distribution of the $\sim 4\pi$ X-ray emission. Due to the wide emission angle of the X-ray source the geometry is less sensitive to angular and spatial variations on a shot-to-shot basis or between shot days, for instance when the tape target is being replaced. Considering this factor and since the analysis of the directionality of the X-ray source is still ongoing, we will for now also ignore any angular variation within the spectrum and calculate the cross-sections at a fixed crossing angle, $\theta = 140^\circ$, and investigate how the cross-section, $\sigma_{BW}$, varies for a deviation in $\theta \pm 20^\circ$. 
...
This section aims to place the results presented so far in the context of the overall effort of analysing the experimental data of this campaign\footnote{Brendan Kettle (Imperial College) was the experimental lead on this campaign and coordinates the analysis efforts.}. After having discussed the commission and performance of both high-energy photon sources, here we will now describe the installation of the last component, the detector system composed of a magnetic chicane and single-particle detectors. We will then indicate briefly how eventually all components worked together towards the main goal of this campaign, but will fall short of presenting details or results of the ongoing complex analysis as this is outside the scope of this work. 
...
In this section we will explore, based on the analysis presented in this Chapter, how a potential future measurement of the linear Breit-Wheeler process could be improved. For this discussion we will remain within the framework of the present experimental layout as we were able to field the three main components (gamma-ray source, X-ray source and single-particle detectors) successfully, and we demonstrated that the components worked individually and in tandem as designed. In the following we will discuss how to improve their performance, focusing on the topics of this Chapter, mainly the commission of the bremsstrahlung source but also in smaller parts its interplay with the X-ray source.
