...
In relativistic \textit{inverse Compton scattering} (ICS), sometimes also referred to as Compton Backscattering (CBS) or, in the classical limit, relativistic Thomson scattering, one (linear) or multiple (non-linear) photons scatter from a relativistic electron of energy $\epsilon$, with $\epsilon/m_e c^2 = \gamma \gg 1$. 
The scattered photons experience a relativistic Doppler-shift and are re-emitted in a narrow cone of divergence $\sim 1/\gamma$ in the direction of the electron propagation, carrying a higher energy based on the electron energy and the intensity of the laser field. 
The emitted photon energy, $\epsilon_{ph}'$, is maximised in a head-on collision: $\epsilon_{ph}' = 2 \gamma^2 (1-\cos\theta)\epsilon_{ph}$ (see e.g. \cite{Esarey1993_NT,Lau2003_NT} or Section \ref{Theory:Sec:LinearICS}), where $\epsilon_{ph}$ is the photon energy before the interaction and $\theta$ is the angle between the photon and the electron. 
Higher electron energies result in a stronger shift of the radiation towards higher energies, also called `hardening of the spectrum', and an increase of the corresponding energy loss the electrons experience. 
A more intense laser field ($a_0 > 1$), on the other hand, enables non-linear interactions and increases the number of photons interacting with an electron, resulting in the generation of $\sim a^3_0$ higher harmonics \cite{DiPiazza2012_ICS} but also redshifting and broadening of the spectrum due to the relativistic mass increase \cite{Sarachik1970_ICS,Esarey1993_NT,Lau2003_NT,Koga2005_ICS}. 
As a result, the rapidly rising number of higher harmonics blend together to form a broadband synchrotron-like spectrum and the characteristic energy of the spectrum, $\epsilon_{ICS} = 3 \gamma^2 a_0 \hbar \omega_0$, increases linearly with $a_0$.

ICS provides a promising route to generate bright beams of high energy radiation reaching 100's of keV or even MeV photon energy, suitable for medical \cite{Carroll2002_MEDICX,Devita2008_CANCER} and imaging applications (also of high-Z objects) \cite{Oliva2010_ICSIMAGING,Glinec2005_Brems} and to stimulate nuclear resonances \cite{Baldwin1948_GAMMA,Nakano2001_ICSNuclear}, even at small-scale facilities \cite{Albert2016_APP}.
It also provides a useful tool to measure beam properties \cite{Bosco2008_LW,Kramer2018_Gamma}, including polarisation \cite{Baylac2002_POL,Barber1993_POL}.
In addition, the high photon energies offer the opportunity to directly measure the effect of \textit{radiation reaction} on the beam dynamics itself and to observe energy loss, if the emitted photon carries an energy comparable to the energy of the electron $\gamma^2 a_0 \frac{2 r_e \omega}{3c} \sim 1$ \cite{Albert2016_APP}, where $r_e$ is the classical electron radius. 
The attempt to solve radiation reaction self-consistently within classical electrodynamics exhibits unphysical predictions like self-acceleration, resulting in runaway solutions, or an unbounded emission spectrum, i.e. energy loss higher than the initial energy of the electron is allowed \cite{Blackburn2019_RRReview}. A universally accepted practicable description of radiation reaction has not been found yet.

\subsubsection{Inverse Compton Scattering using Laser Wakefield Accelerators}

In laser wakefield acceleration (LWFA), the intense laser driver is intrinsically synchronised to the relativistic electron beam that trails behind it and the electron source size is small, allowing complete overlap with a tightly focused laser pulse. This makes LWFA well suited to study inverse Compton scattering at high intensities.
...
\label{RR:Figs:Gmax_Eta_ICS}
\end{figure}

Colliding-pulse or ICS experiments using LWFA have been successfully performed by different groups over the past years, gradually improving the control over the process, increasing the laser intensities and the energies of the electrons involved, and subsequently of the generated radiation \cite{TaPhuoc2012_ICS,Chen2013_ICS,Liu2014_ICS,Powers2014_ICS,Sarri2014_ICS,Khrennikov2015_ICS, Yan2017_ICS, Shaw2017_ICS} (see Figure \ref{RR:Figs:Gmax_Eta_ICS}). 
The energy and the tunability of the radiation produced in these experiments are good indicators for their continuing progress. 
More specifically of interest for measuring radiation reaction is the quantum non-linearity parameter, $\eta = 2\gamma E_L/E_{crit}$ \cite{Ritus1985_QRR,Bell2008_PairsEta}, that indicates the strength of the electric field of the laser, $E_L$, in the rest frame of the electron relative to the critical electric field of QED, $E_{crit} = m^2_e c^3/\hbar e$, also called the \textit{Schwinger field} \cite{Sauter1931_ECRIT,Heisenberg1936_ECRIT}.

\subsubsection{Inverse Compton Scattering using a plasma mirror}

A single intense laser pulse can be used to accelerate electrons to relativistic energies via LWFA and to scatter the electrons after backreflecting the laser from a close-to-normal plasma mirror \cite{Kapteyn1991_PM}, e.g. tape \cite{TaPhuoc2012_ICS} (see Table \ref{RR:tables:ICS_with_PM} for an overview of recent experimental results achieved in this geometry). In this geometry, the laser beam and the electrons are timed intrinsically. In \cite{TaPhuoc2012_ICS} electrons of an energy $\sim 100\,\mathrm{MeV}$ were collided at a laser intensity of $a_0 \approx 1.2$ (weakly non-linear). The radiation measured was a broadband X-ray spectrum and reached up to $100`\mathrm{s}\,\mathrm{keV}$ photon energies. In \cite{Tsai2015_ICS} a low energy spread electron beam of up to $90\,\mathrm{MeV}$ was scattered at a laser intensity $a_0 \approx 0.9 - 1.2$, producing an X-ray spectrum peaked at $180\,\mathrm{keV}$. In \cite{Yu2016_ICS} a laser of intensity $a_0$ was scattered from electrons of $400\,\mathrm{MeV}$ energy emitting gamma rays of up to $2\,\mathrm{MeV}$ energy.
By now the highest recorded gamma-ray energies from ICS in LWFA using this scheme were reported in \cite{Shaw2017_ICS}, reaching photon energies of up to $85 \,\mathrm{MeV}$ in a collision with electrons of energies up to $2\,\mathrm{GeV}$.
...
\hline
\multicolumn{6}{|c|}{\textbf{All-optical experiments with plasma mirror}} \\
\hline \hline
\textbf{Name} & \textbf{Journal} & \textbf{Year} & $\bm{\epsilon}$ \textbf{[MeV]} & $\bm{a_0}$ & $\bm{\epsilon_\gamma}$ \textbf{[MeV]}\\ \hline \hline
Ta Phuoc et al. & Nat. Photonics \textbf{11} & 2012 & 100 & 1.2 & 0.1\\
Tsai et al. & Phys. Plasmas \textbf{22} & 2015 & 90 & 1.2 & 0.18\\
Doepp et al. &  PPCF \textbf{58} & 2016 & 150 & 1.0 & 0.05\\
Yu et al. & Sci. Rep. \textbf{6} & 2016 & 400 & 1.0 & 2\\
Shaw et al. & arXiv:1705.08637 & 2017 & 2000 & 0.5 & 85\\
\hline
\end{tabular}
}
\caption[All-optical inverse Compton experiments using a plasma mirror.]{Overview of all-optical inverse Compton experiments using a plasma mirror, colliding electrons of energy $\epsilon$ and a laser pulse of peak intensity $a_0$ reaching maximum gamma energies of $\epsilon_\gamma$.}
\label{RR:tables:ICS_with_PM}
\end{table}
Whilst this technique avoids issues with timing and overlap, the intensity of the laser pulse is limited as it is partly depleted when interacting with the electron bunch after driving a wake \cite{Shadwick2009_nlDepletion} and is typically not tightly focused due to diffraction. 
At the same time, the acceleration can not be pushed to its depletion limit as the laser pulse has to remain intense enough for a suitable interaction. Further problems might arise as controlling or measuring the wavefront of the depleted laser pulse is challenging. The electrons will also produce radiation from bremsstrahlung when passing through the plasma mirror \cite{Yu2016_ICS}, which overlays the ICS signal, although in \cite{Tsai2015_ICS} the measured signal was claimed to be insignificant.
...
If two separate lasers are available or maybe one very powerful laser pulse can be split into two parts, one can be used to accelerate electrons via LWFA and one to scatter off them. 
An overview of recently published experimental results in this geometry are shown in Table \ref{RR:tables:ICS_with_2lasers}. Electrons produced from LWFA are typically very short in duration $\sim 10`s \,\mathrm{fs}$ \cite{Lundh2011_BUNCH} and only few microns small \cite{Weingartner2012_BUNCH} when leaving the accelerating cavity. This allows focusing the second laser very tightly to reach high intensities and to interact with a large fraction of the electron bunch at comparable intensities without a large background from linear ICS. 
Using two laser pulses opens the gateway to combine higher intensities and electron energies at the interaction point, and gives more control over the interaction itself. However, it is challenging to overlap the micron-sized electron bunch with the ultra-short tightly-focused laser pulse, compressed in time and space, and to maintain this alignment over an extended period of time \cite{Samarin2017_RR}.
...
\begin{tabular}{|r|r|r|r|r|r|}
\hline
\multicolumn{6}{|c|}{\textbf{All-optical experiments with two lasers}} \\
\hline \hline
\textbf{Name} & \textbf{Journal} & \textbf{Year} & $\bm{\epsilon}$ \textbf{[MeV]} & $\bm{a_0}$ & $\bm{\epsilon_\gamma}$ \textbf{[MeV]}\\ \hline \hline
Chen et al. & Phys. Rev. Lett. \textbf{110} & 2013 & 250 & 0.4 & 4\\
Powers et al. & Nature Photonics \textbf{8} & 2014 & 300 & 0.3 & 0.1\\
Liu et al. & Optics Letters \textbf{39} & 2014 & 450 & 0.1 & 9\\
Sarri et al. & Phys. Rev. Lett. \textbf{113} & 2014 & 400 & 2 & 18\\
Khrennikov et al. & Phys. Rev. Lett. \textbf{114} & 2015 & 50 & 0.9 & 0.04\\
Yan et al. &  Nature Photonics \textbf{8} & 2017 & 200 & 12 & 20\\
\hline
\end{tabular}
}
\caption[All-optical inverse Compton experiments using two lasers.]{Overview of all-optical inverse Compton experiments using two lasers, colliding electrons of energy $\epsilon$ and a laser pulse of peak intensity $a_0$, reaching maximum gamma energies of $\epsilon_\gamma$.}
\label{RR:tables:ICS_with_2lasers}
\end{table}

Tunability at narrow electron energy spread was demonstrated at $300\,\mathrm{MeV}$ and tens of $\mathrm{MeV}$ electron energies and laser intensities $a_0 < 1$ in \cite{Powers2014_ICS} and \cite{Khrennikov2015_ICS}. In \cite{Chen2013_ICS} relativistic electrons were scattered at an angle of $10$ degrees and up to $4\,\mathrm{MeV}$ gamma rays were produced. In \cite{Liu2014_ICS} an electron beam of $\epsilon = 450\,\mathrm{MeV}$ was scattered with a second harmonic laser at $\epsilon_{ph} = 3\,\mathrm{eV}$  reaching up to $9\,\mathrm{MeV}$ gamma energies.
...
Several PW laser systems have already or are soon to commence their operation, with some existing laser systems being upgraded to the PW-level as well (for instance ELI-NP \cite{Gales2018_ELINP}, ELI-Beamlines \cite{Weber2017_ELIBeamlines}, EPAC \cite{EPAC_Website}, HERCULES \cite{Yanovsky2008_HERCULES} and ZEUS \cite{Maksimchuk2019_ZEUS}, Apollon \cite{Zou2015_Apollon}, SULF \cite{Li2017_SULF}, SEL \cite{Cartlidge2018_SEL}, BELLA \cite{Leemans2013_BELLA,Toth2017_BELLA}, XCELS \cite{Bashinov2014_XCELS}, CALA \cite{Kessel2018_PWMPQ} and many more \cite{Danson2019_PWLASERS}). At this point, QED studies are already considered for the 100 PW regime \cite{Shen2018_SULF,Cartlidge2018_SEL}. 
...
The combination of both, highly relativistic electron beams and highly intense lasers, opens up a wide variety of research topics in the strongly non-linear quantum regime probing radiation reaction\cite{Blackburn2014_QRR}, photon-photon scattering \cite{Enterria2013_PhPh} and pair production from the Breit-Wheeler mechanism \cite{Pike2014_BW,Blackburn2017_pairs} or by reaching supercritical field strengths \cite{Blackburn2019_SUPER}. The high laser intensities will also enable studies of vacuum birefringence \cite{King2016_VB}.
...
At XFELs, for instance, this is interesting for the study of warm-dense-matter states and high-energy-density physics \cite{Vinko2012_HEDX,Schropp2015_SHOCKX,Harmand2015_XAS}, involving pump-probe measurements, where X-rays from an insertion device probe matter structures and changes induced in them in the interaction with an intense laser pulse.
...
Another measurement of radiation reaction was performed at CERN \cite{Wistisen2018_RR}, however using positrons and planar channeling in crystals \cite{Artru1994_CHANNELING,Katkov1998_CHANNELING} as tool to probe radiation reaction \cite{DiPiazza2017_CrystalRR} instead of ICS.
An overview over the high field QED projects currently being undertaken can be found in \cite{SFQEDOverview2019}.

\section{Chapter Outline}

The work presented in this Chapter relates to an experiment aimed at measuring radiation reaction through inverse Compton scattering (ICS). It was performed at the \textsc{Gemini} laser facility in late 2015. Our experimental team succeeded in colliding electrons of energy $\epsilon \approx 550\,\mathrm{MeV}$ at a laser intensity of $a_0 \approx 10$, reaching critical gamma-ray energies, $\epsilon_{crit}$, in excess of $30\,\mathrm{MeV}$. This was the highest gamma-ray energy from an all optical ICS source published at that point and consists the first published measurement of radiation reaction in an LWFA setup \cite{Cole2018_RR}. 

This Chapter includes significant contributions to this publication, in particular devising a method to identify successful collisions and characterise the electron spectra. As a result, the reader will find several parallels and in some instances a similar line of arguments between \cite{Cole2018_RR} and this Chapter.
\vspace{\baselineskip}
...
After an outline of the experimental setup in Section \ref{Chap:RR:Sec:ExpSetup}, such a method to identify successful collisions is presented in Section \ref{Chap:RR:Sec:SuccessfulColl}. 
For this purpose, the yield on the gamma detectors is correlated with the energy in the electron beam on a shot-to-shot basis. 
Without the second scattering beam, the measured radiation is produced from bremsstrahlung as dispersed electrons interact with the walls of the vacuum chamber. It is assumed that the second laser adds ICS as an additional source of radiation and that particularly intense interactions produce a significant excess signal on the gamma detector. 

The Chapter continues in Section \ref{Chap:RR:Sec:CharacElec} with an analysis of the electron spectra measured in this dataset. The electron spectra consistently exhibit a distinct spectral feature in shape of a sharp edge-like fall-off in charge at $\sim 550\,\mathrm{MeV}$ (see Figure \ref{RR15:Fig:Cole_espec_example_EdgeShotsNull}). The intrinsic fluctuations of the electron source are characterised in a statistical analysis after removing correlations.

Then key results from the analysis of the gamma spectra, performed by Jason Cole (Imperial College London) and Keegan Behm (University of Michigan), are introduced. The spectra are inferred by fitting the simulated response of the detector, a stack of scintillating caesium-iodide (CsI) crystals, to the signal measured in the experiment. Details of this procedure can be found in \cite{Behm2018_Gamma,Cole2018_RR} or Section \ref{Chap:Methods:subsec:GammaSpec} of the \nameref{Chap:Methods}.
...
Based on a sketch made by J. Cole (Imperial College) for \cite{Cole2018_RR}, adapted for this work and for \cite{Behm2018_Gamma}. 
From left to right: a high intensity laser beam (red) focused with an $f/40$ spherical mirror generates $\mathrm{GeV}$-scale electrons (blue) in a gas jet (LWFA). A second laser beam (red) is focused down tightly at the edge of the gas jet by an $f/2$ off-axis parabola (OAP) to scatter the electron beam shortly after it leaves the jet. The electrons are dispersed by a dipole magnet and detected on a scintillating lanex screen (grey). Finally, gamma rays (green) emitted in the interaction propagate through a Kapton vacuum window (orange) and a lead aperture onto a stack of scintillating CsI crystals acting as gamma detector.
}
\label{RR15:figs:setup_sketch}
\end{figure}


The first part of the experiment is set up to produce a relativistic electron bunch from laser wakefield acceleration (LWFA): a laser beam, linearly polarised in the horizontal plane, is focused down by an $f/40$ spherical mirror with $6\,\mathrm{m}$ focal length onto the leading edge of a $15\,\mathrm{mm}$ conical supersonic helium gas jet target. 
The top edge of the gas jet is positioned $6\,\mathrm{mm}$ below the laser axis in order to avoid damage to the nozzle from the second, more divergent laser beam. 
Electrons are accelerated via LWFA and propagate further downstream, where they are dispersed by a permanent dipole magnet of integrated field strength $\int B(x) \mathrm{d}x = 0.4\,\mathrm{Tm}$ onto a scintillating standard Lanex screen, which is imaged by a cooled 16-bit CCD camera (Andor Neo) to measure their spectrum. 
The typical \textsc{fwhm} focal spot of the driving laser pulse measures $37 \times 49 \,\mathrm{\upmu m}$ with an energy on target of $(8.6 \pm 0.6)\,\mathrm{J}$, which corresponds to a normalised vector potential $a_0 = 1.9 \pm 0.1$. The electron density of the target was interferometrically measured to be $(3.7 \pm 0.4) \times 10^{18} \,\mathrm{cm}^{-3}$. 
\vspace{\baselineskip}

The second laser beam, linearly polarised in the vertical plane, is focused down tightly onto the opposite edge of the gas jet, at 180 degrees from the driver laser, using an $f/2$ off-axis parabola (OAP). 
This laser is used to scatter from the electron bunch accelerated through LWFA to generate a bright flash of gamma rays from inverse Compton scattering (ICS). The OAP is fitted with a central hole, $21\,\mathrm{mm}$ in diameter, to enable propagation of the electrons, gamma rays and the remaining laser light of the $f/40$ wakefield driver beam. In addition, a plastic ring of $28\,\mathrm{mm}$ radius around the hole protects the optics and the laser chain upstream from potential driver laser light scattered in an interaction with the plasma. The combined loss of reflective surface leads to a decrease in intensity of the flat-top beam of around $16\%$. 
The energy on target was typically $(10 \pm 0.6)\,\mathrm{J}$ focused into a spot of $2.4 \times 2.8\,\mathrm{\upmu m}$ \textsc{fwhm}, corresponding to a peak normalised vector potential of $a_0 = 24.7 \pm 0.7$. 
The peak value of the quantum non-linearity parameter, $\eta$, in a head-on collision is $\eta = 2\gamma a_0 \hbar \omega_0/m_e c^2$.
...
%\caption[Montage of gamma spectrometer and electron spectrometer images for collision and null shots.]{Montage of raw images from the gamma ray spectrometer (upper row) and electron spectrometer (bottom row) at shots with the colliding laser beam off (left) and on (right), using the same colour scale. Some shots with the colliding beam show bright signals on the scintillator array and indicate a successful collision.}
%\label{Results:Figs:NullColl:Montage}
%\end{figure}

\begin{figure}[h]
\centering
\includegraphics[trim={5.4cm 1cm 8cm 0.8cm}, clip, width=0.8\columnwidth]{Example_RR15Cole_CsI_Montage.png}

\includegraphics[trim={5.4cm 1cm 8cm 0.8cm}, clip, width=0.8\columnwidth]{Example_RR15Cole_Espec_Montage.png}
\caption[Montage of gamma spectrometer and electron spectrometer images for collision and null shots.]{Montage of images from the gamma ray spectrometer (upper row) and electron spectrometer (bottom row) at shots with the colliding laser beam off (left) and on (right), using the same colour scale. Some shots with the colliding beam show bright signals on the scintillator array that indicate a successful collision.}
\label{Results:Figs:NullColl:Montage}
\end{figure}


To identify successful collisions, we compare the radiation yield from the electron beams with and without scattering laser, as measured with the gamma detector (see Figure \ref{Results:Figs:NullColl:Montage}).
The electrons will emit broadband bremsstrahlung with photon energies extending up to the maximum electron energy when interacting with matter in their trajectory \cite{Jackson}. 
Especially high-Z materials such as the vacuum chamber walls are efficient converters. 
This radiation is in a spectral range comparable to the expected ICS signal and is also measured by the gamma-ray detectors. 
The electrons are dispersed upwards and collide with the roof of the aluminium vacuum chamber producing the main source of bremsstrahlung. 
It is located off-axis and a large fraction is shielded efficiently by blocking the direct line of sight with sufficient amounts of lead. 
This reduces the total background signal on the detector and improves the signal-to-noise ratio, but also allows a spectral retrieval of the bremsstrahlung component as it now only enters the stack from one defined side.
\vspace{\baselineskip}
...
The total energy emitted through bremsstrahlung by a relativistic electron is proportional to its energy squared (e.g. \cite{Jackson} and Section \ref{Theory:Sec:Bremsstrahlung}). 
The total energy deposited in the caesium-iodide crystals of the gamma spectrometer is assumed to convert linearly into scintillation light \cite{Frlez2000_CsI} at an efficiency of $\approx 5 \times 10^4 \,\mathrm{photons}/\mathrm{MeV}$. 
The total yield of the detected bremsstrahlung signal on the camera chip, $S_{BG}$, should then follow this relation:
\begin{equation}
...
\end{equation}
where $c_{BG}$ is a constant in units of [pixel counts $\mathrm{pC}^{-1}\,\mathrm{MeV}^{-2}$] that encapsulates the complicated details of the interaction and the experimental setup that remain the same over the course of the shots, such as the conversion efficiencies of electron energy to bremsstrahlung photons, photons depositing their energy in the detector crystals, number of photons emitted from the scintillator per deposited energy, viewing angle of the camera, collection efficiency of the imaging system, and quantum efficiency of the camera. 
$\mathrm{d}N_e/\mathrm{d}\gamma$ is the charge distribution of the measured electron spectrum, $Q = \int (\mathrm{d}N_e/\mathrm{d}\gamma) \mathrm{d}\gamma$ the total charge and $\gamma$ is the relativistic Lorentz factor of the electrons. 
Other sources of background (dark field, stray light on the camera etc.) are removed efficiently and do not need to be included in this equation. By measuring the total counts (yield) on the gamma detector and the electron spectrum, one can then determine $c_{BG}$ experimentally.
\vspace{\baselineskip}
...
Figure \ref{Results:Figs:NullColl:CsIVsE2Coll} shows the relation of the experimentally measured $Q \left\langle\gamma^2\right\rangle$ to the total number of pixel counts (yield) on the gamma-ray detector for 10 shots without the scattering beam (blue). Figures \ref{RR15:Fig:Cole_espec_example_EdgeShotsNull} and \ref{fig:Cole_gamma_example} show examples of an electron spectrum and the gamma detector. The data points follow a linear trend with a slope corresponding to $c_{BG}$ at a linear correlation coefficient of $0.71$. 
\vspace{\baselineskip}

Successful collisions of the electron bunch and the scattering beam produce a beam of gamma rays from inverse Compton scattering (ICS). At the detector, the signal is then a combination of bremsstrahlung as without the scattering beam, following the same relation with slope $c_{BG}$, and the ICS signature. The emitted energy of the ICS radiation and hence the resulting detector signal, $S_{ICS}$, is also proportional to $\gamma^2$, similarly as for the bremsstrahlung process, but also scales with the normalised vector potential $a_0$ in interaction with an electron with energy $\gamma$, valid for $\gamma a_0^2 < 5.5 \times 10^5$ \cite{Thomas2012_LL,Corde2013_Rad}:

\begin{equation}
\boxed{S_{ICS} = c_{ICS} \int  a_0^2 \left[\mathrm{d}N_e /\mathrm{d}\gamma\right]\, \gamma^2 \mathrm{d} \gamma,}
\end{equation}

where now $c_{ICS}$ similarly encapsulates all the complex physics of coupling constants, the imaging system, cross sections and conversion efficiencies. 
In this case, the value of $a_0$ at the interaction is not constant from shot to shot as a varying overlap of the laser pulse and the electron beam will result in changing interaction conditions. 
$a_0$ can also vary throughout the interaction, if the duration of the interaction is comparable to the time it takes for the laser pulse to pass through its focus, either due to a short Rayleigh length or long electron bunches \cite{Harvey2016_ICSFOCUS}. 
The better the overlap and the higher the intensity at the interaction, the stronger the ICS signal and the easier will it be to distinguish from the background bremsstrahlung. In addition, whilst we expect all of the electrons to produce bremsstrahlung, only a variable fraction of electrons will interact with the laser pulse. If the laser pulse is larger than the electron beam, the variation in intensity the electrons experience will be reduced.

\begin{figure}
\centering
\includegraphics[width=0.9\columnwidth]{ElecQ2_CsI_Correlation.pdf}
\caption[Gamma-ray yield as a function of the electron beam energy squared for null and collision shots.]{Yield on the gamma detector in pixel counts (y-axis) as a function of the squared electron beam energy (x-axis). The reference shots without the scattering beam are indicated in blue with a regression line, where the gradient corresponds to $c_{BG}$. The shaded region indicates the 95\% confidence interval. In orange shots with both laser beams on.}
...
In Figure \ref{Results:Figs:NullColl:CsIVsE2Coll}, the shots with the scattering beam (orange) are more spread than the shots without the colliding beam (blue). Some of the data points with the colliding beam follow the linear relationship determined for the reference data relatively well. This indicates poor overlap of the electron bunch with the scattering beam. A few shots exhibit a much higher detector signal than predicted by the fit for an electron spectrum of comparable charge and energy, signalling good overlap.

\begin{figure}
\centering
\includegraphics[width=0.9\columnwidth]{DCsI_sigma_with_errorbars.pdf}
\caption[Gamma-ray signal above expected background signal.]{Deviation from the expected background gamma signal in units of the standard deviation of the background, $\sigma_{BG}$, as a function of the shot number (order data was taken) without the scattering beam on (blue) and with both laser beams (orange). The shaded blue area indicates a 2 sigma interval for the background centred around zero (blue line). The orange line at $5\sigma_{BG}$ and the shaded area above it indicates the parameter space satisfying our criterion for successful collisions, here including 4 shots. The absence of any successful collisions after shot 18 indicates a drift in alignment.}
...
Figure \ref{Results:Figs:NullColl:DeltaCsIVsSigmaE2+CsIVsE2} presents the shots in the order their data was taken in the experiment. The normalised signal above expected background, $S_{ICS, norm}$, is indicated on the y-axis, which now encapsulates the information of both the gamma signal and the electron spectrum.
As we can see, most of the shots follow the expected trend within 2 standard deviations (shaded blue area), with some exceeding this. 4 shots show a signal more than 5 standard deviations higher than expected from background measurements (shots in shaded orange area). 
Based on a normal distribution, the probability for one shot to be as bright as 5 standard deviations above the mean background is 1 in 3,500,000. 
This is very unlikely and hence a suitable confirmation that these are successful collisions. 

\begin{equation}
\boxed{
S_{ICS, norm} [\sigma_{BG}] 
\begin{cases}
\text{if } \geq 5 &\longrightarrow \mathrm{Successful\,Collision}\\
...
In the previous section, the comparison of the measured gamma-ray signal with the expected bremsstrahlung noise produced by the electron beam enabled the identification of successful collisions. The next step is to quantify whether there was a measurable lower energy (potential energy loss) visible in the electron spectrum on these collisions and to estimate the statistical significance of this.
...
\includegraphics[trim={4.8cm 0 2.5cm 0}, clip, width=0.85\columnwidth]{Example_RR15Cole_V3A.png}

\includegraphics[trim={4.8cm 0 2.5cm 0}, clip, width=0.85\columnwidth]{Example_RR15Cole_V3B.png}

\includegraphics[trim={4.6cm 0 5cm 0}, clip, width=.85\columnwidth]{ElecEdge_Example_slim.png}
\caption[Example of an electron spectrum measured on the radiation reaction campaign.]{Top/centre: Example of a typical electron spectrum measured in the experiment. The x-axis indicates the energy, the y-axis the divergence of the electrons. The spectrum is integrated in the dispersion and divergence axis and shown on the respective axes. The colour scale indicates the charge per divergence and energy (pC per mrad and MeV) scaled to the maximum value in the image (top) or saturated by a factor 50 to show the low-charge high-energy tail of the e-beam (centre).
Bottom: Lineout of an electron spectrum (grey) normalised to its peak charge as a function of electron energy. The first derivative of the spectrum (blue) exhibits a sharp peak at the edge-like spectral feature around $600\,\mathrm{MeV}$ (dashed blue line). This peak is used to identify the spectral feature on a shot-to-shot basis.} 
%The blue line is the first derivative of the spectrum with a sharp peak at the edge-like spectral feature around $600\,\mathrm{MeV}$, indicated by a dashed line
...
%
The characteristic electron spectrum consists of two components (see Figure \ref{RR15:Fig:Cole_espec_example_EdgeShotsNull}): one is a low charge but high energy tail reaching $800-1000\,\mathrm{MeV}$ (centre), the other part contains a high charge at lower energy that falls off rapidly at an edge-like spectral feature at around $550\,\mathrm{MeV}$ (top). The position of this spectral feature is identified by finding a maximum in the derivative of the integrated spectrum (bottom). The electron spectrum is cut off at $250\,\mathrm{MeV}$ due to limitations in the spectrometer setup. 
The high energy component resembles spectra from self-injection \cite{Bulanov1997_SELF} measured at \textsc{Gemini} before (see for instance \cite{PoderThesis}). 
The high charge component with the distinct spectral feature is consistent with shock injection \cite{Schmid2010_SHOCK} measured at \textsc{Gemini}, e.g. see Chapter \ref{Chap:linICS}, potentially caused by either the nozzle design or potential damage to it. 
The optical image seen in Figure \ref{RR15:figs:prettypic} indicates some damage to the nozzle.
%The shock fronts are visible on an optical image seen in Figure \ref{RR15:figs:prettypic} along with the damage on the nozzle. 
...
Note that the electron beams observed at \textsc{Gemini} are qualitatively different from these results and the high quality properties commonly associated with the term `shock injection' in literature.
\begin{figure}
\centering
%\includegraphics[width=0.4\columnwidth]{20151217r001s009_PrettyPic_annotated.JPG}\includegraphics[width=0.4\columnwidth]{20151217r002s003_PrettyPic_crop_V2.jpg}
\includegraphics[width=0.4\columnwidth]{20151217r002s018_PrettyPic_annotated.JPG}\includegraphics[width=0.4\columnwidth]{20151217r002s003_PrettyPic_crop_V2.jpg}
\caption[Photograph of the plasma channel and plasma self emission in a supersonic helium gas jet.]{Photograph of the plasma channel and plasma self emission taken with a Canon DSLR camera. A supersonic helium gas jet is traversed by the wakefield driver beam from left to right forming a plasma channel (left image). A second divergent beam arriving from the right side is used to scatter off electrons accelerated in the wakefield, producing a bright and divergent cone of emission on top of the channel (right image). The gas nozzle was scorched by the divergent beam and rotated by 180 degrees. Subsequently, the nozzle was lowered to avoid further damage. Apparent structuring in the plasma channel (left, zoomed in) could indicate density perturbations or could be an optical effect.}
%Structuring in the plasma channel indicates density perturbation which could result in shock injection.}
...
A small sample size like this (10) is not sufficient to draw reliable conclusions about the general character of the fluctuations of the accelerator, i.e. more reference data is required. 
In order to gain access to a larger pool of reference shots, we now consider a second dataset that was taken immediately after the small dataset discussed so far. 
Here the accelerator was operated at comparable conditions, i.e. the general properties of the electron beam are the same, but the scattering beam was strongly defocused, such that interactions only occurred at low intensities ($a_0 < 1$) without measurable effect of radiation reaction.
The scattering beam was active on all shots and translated relative to the electron beam, such that now radiation from linear ICS was produced at varying overlap. In order to obtain `clean' reference shots without any significant interaction between the scattering beam and the electron bunch, we only consider shots with a gamma-ray yield within 1 standard deviation of the expected background. This additional set of references consists of 89 shots, and will help to investigate correlations with other experimental parameters and to characterise the `intrinsic' fluctuations of the accelerator statistically.
%We now use the gamma-ray signal to identify the subset of shots \textit{without} any , we use again the gamma-ray signal but this time. Using the gamma-ray signal as indication for significant overlap as developed in the previous section, we now identify shots that very closely align with the characterised bremsstrahlung background. Shots that are within 1 standard deviation of the background are identified as non-collision shots and are included as reference data. This new dataset consists of additional 89 shots, and is used to investigate correlations with other experiment parameters and to characterise statistical fluctuations. 
...
\caption[Laser energy as a function of time, and correlation of electron edge energy and laser energy.]{Left: On-shot driver laser energy as a function of elapsed time. The dataset is truncated earlier than in Figure \ref{fig:Cole_drift_raw}, since this diagnostic failed about 2 hours into the measurement. Right: Correlation of laser energy with spectral feature for the core dataset.}
\label{RR15:Figs:laser_elec_corr}
\end{figure}

Despite the lack of conclusive evidence to explain the origin of the drift, we can measure and account for it when characterising the intrinsic fluctuations of the electron spectrum.
...
After removing the slow drift from the data, we can check if the remaining fluctuations correlate with the on-shot laser energy and the injected charge. There appears to be a correlation but only at a coefficient of $0.35$ ($0.22$ with drift) across the entire dataset and even lower for the potential high-intensity dataset. The same holds for the injected charge at a correlation coefficient of $0.39$ ($0.38$ with drift). The gradient of the regression is small compared to the shot-to-shot fluctuation in the laser energy and the $95\%$ confidence interval indicates no significant correlation (see Figure \ref{RR15:Figs:laser_elec_corr}, right). We will hence ignore this factor in our analysis.

\subsubsection{Intrinsic statistical fluctuations of the accelerator}

Any deviation in energy $\Delta \epsilon$ from this slow drifting mean now represents a characteristic intrinsic fluctuation of the accelerator. Figure \ref{RR:fig:ElecDEE_histo} shows a histogram of relative differences in energies, $\Delta \epsilon/\epsilon$, along with an estimate of the underlying density distribution using a Kernel Density Estimate (KDE) and a Gaussian fit. The Gaussian fit agrees with the $99\%$ confidence interval of the KDE, but there seems to be a slight skew in the distribution.

\begin{figure}[h]
\centering
\includegraphics[width=0.8\columnwidth]{ElecEdge_DE_drift_histo_errors.pdf}
\caption[Distribution of electron edge energies and KDE.]{Histogram of relative deviation of the spectral edge energy from the expected slowly varying mean ($\Delta \epsilon/\epsilon$). The total distribution is overlaid with a KDE, its $95\%$ confidence intervals and a Gaussian distribution (orange). The shaded area indicates a 2 sigma (95\%) confidence interval assuming a normal distribution.}
...
Having now characterised the fluctuations of the energy of the spectral feature in the electron beam, we can estimate how likely it is that we measure low energies by chance or at what confidence this could be due to energy loss from radiation reaction. The collision dataset was taken over the course of $\sim 20$ minutes, such that the slow drift identified in the previous section becomes negligible and is not included in the following analysis.


\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{DCsI_sigma_v_ElecEdge_with_errorbars.pdf}
\caption[Gamma-ray signal as function of electron edge energy.]{Gamma ray signal above the expected radiation background (y-axis) in units of standard deviation from the reference mean as a function of the electron energy of the spectral edge feature (x-axis). Reference shots without the scattering beam (`beam off') are shown in blue, dual-beam shots (`beam on') are indicated in orange. The 2 sigma interval of the reference shots is shaded in blue, whereas the parameter space for successful collisions is shaded orange.}
\label{Results:Figs:Edge:PosVsCsI}
\end{figure}



In the previous section, we identified 4 successful collisions. The corresponding electron spectra on those four shots exhibit a lower energy for the edge-like spectral feature than on most of the other shots. Figure \ref{Results:Figs:Edge:PosVsCsI} shows a plot of the electron energy of the spectral feature and its relation to the excess signal on the gamma detector.

For a normal distribution of mean $550\MeV$ and a standard deviation $\sigma_{Cole} = 7.7\%$, the cumulative probability to observe a spectral feature below 500 MeV on one shot is $12\%$. To observe this on all four successful collisions is then $(12\%)^4 = 0.02\%$. We conclude that the lower energies measured on these shots are due to energy loss in the successful collision and statistically significant.
\vspace{\baselineskip}

To estimate whether the energy loss relates to a realistic intensity at the interaction, we use the analytic expression for energy loss from the classical Landau-Lifschitz radiation reaction force for a plane wave with a Gaussian temporal envelope, valid for $\gamma a_0 \ll 1.6 \times 10^5$ \cite{Thomas2012_LL}:

\begin{equation}
\frac{\Delta \gamma}{\gamma_0} = \frac{\sqrt{\pi/2} \tau_0 t_L \omega^2_0 \gamma_0 a^2_0}{1+ \sqrt{\pi/2} \tau_0 t_L \omega^2_0 \gamma_0 a^2_0},
\end{equation}
where $\tau_0 = e^2/6 \pi \epsilon_0 m_e c^3 = 6 \times 10^{-24}\,\mathrm{s}$, the pulse duration $t_L = 45\,\mathrm{fs}$, the wavelength be $\lambda = 800\,\mathrm{nm}$.
...
For an $80\pm10\,\mathrm{MeV}$ energy loss from the mean of $550\pm20\,\mathrm{MeV}$ down to $470\pm10\,\mathrm{MeV}$, for instance, we obtain $a_0 = 8.9\pm1$. In a quantum model, on the other hand, the required peak intensity to effect the same energy loss is $10\pm2$ as electrons lose energy at $\approx 75\%$ the rate as in a classical model \cite{Ridgers2017_QRR}.
...
The two laser pulses were synchronised in vacuum using spatial interferometry to an estimated accuracy of $30\,\mathrm{fs}$. This timing is only valid if both laser pulses travel through vacuum. In a shooting scenario, however, the driving laser pulse propagates through plasma to accelerate electrons via LWFA. The propagation in a medium reduces the group velocity of the laser pulse and delays its arrival relative to the vacuum timing.
...
For this purpose we can use Equation \eqref{Methods:Eqns:BeamLaser_Timing_deltaz} derived in Section \ref{Methods:Sec:Beam_Laser_Timing} to determine the distance $\delta z$ from the intended collision point \cite{Cole2018_RR}:
\begin{equation}
\boxed{\delta z = \frac{3d}{4} \frac{n_e}{n_c} + N \frac{\lambda_0}{2}\sqrt{\frac{n_c}{n_e}},}
\end{equation}
where $d$ is the distance of the injection point from the front of the gas jet, $n_c$ the critical density, $n_e$ the density of the plasma, $\lambda_0$ the wavelength of the laser, and $N$ the number of plasma wavelengths the electron bunch is trailing behind the laser pulse ($N = 1/2$ at the dephasing limit).

For a normal distribution of $\pm 30$ fs of the relative temporal jitter and by sampling from a uniform distribution for the injection point $d$ between $0$ and $10\,\mathrm{mm}$, the intensity expected at the interaction averaged over an area of $10\,\mathrm{\upmu m}^2$, the estimated size of the electron beam and its spatial shot-to-shot fluctuations, reaches $a_0 = 12 \pm 1$ \cite{Cole2018_RR}.
...
The gamma spectra were inferred by analysing the signal on the gamma detector, a stack of scintillating caesium-iodide crystals (see \nameref{Chap:Methods}), and comparing the shape of the energy deposition throughout the detector with the detector response simulated in GEANT4 \cite{GEANT4}.
An example of the experimentally measured raw detector signal is shown in Figure \ref{fig:Cole_gamma_example}.
\vspace{\baselineskip}
\begin{figure}
\centering
\includegraphics[width=0.5\columnwidth]{CsIFit_ColePaper.pdf}\includegraphics[width=0.5\columnwidth]{OnOff_Temp_DCsI_with_errorbars.pdf}
...
On some shots with both beams, the contribution of bremsstrahlung and ICS might be of similar level, so that the inferred characteristic energies take an intermediate level.
\vspace{\baselineskip}

In simulations of the inverse Compton scattering process\footnote{performed by Tom Blackburn (Chalmers) and Jason Cole (Imperial College)}, it was found that the spectral shape of the expected radiation is well parametrised by 
\begin{equation}
\frac{\mathrm{d}N_\gamma}{\mathrm{d}\epsilon_\gamma} \propto \epsilon^{-2/3}_\gamma e^{-\epsilon_\gamma/ \epsilon_{crit}},
\end{equation}
where we used an exponential spectrum with a critical energy $\epsilon_{crit}$ as approximation to the high energy component of the synchrotron-like non-linear ICS spectrum. In this parametrisation about half of the energy is radiated below and above $\epsilon_{crit}$, with a mean photon energy of $\epsilon_{crit}/3$.
\vspace{\baselineskip}

Figure \ref{Results:Figs:Gamma:OnOff_OnOffCole} (left) shows two examples of fits using this spectral shape: a single beam shot where we characterise only bremsstrahlung (blue) and a successful collision (orange).
The successful collision is, as defined by our identification criterion, brighter than the bremsstrahlung shot. In addition, the critical energy of the fitted spectrum is lower for the collision than for bremsstrahlung only.

In Figure \ref{Results:Figs:Gamma:OnOff_OnOffCole} (right), we now see the critical energies for all shots in this run, but this time with the yield on the detectors on the $x$-axis. We see that all shots with one beam only have high critical energies fitted to their detector response. The energies on shots with both beams that have low signal are also quite high. All shots that have a higher yield, however, decrease in critical energy and all identified shots are $< 50$ MeV.
\vspace{\baselineskip}
%\begin{figure}
%\centering
...
Figure \ref{RR15:Figs:RRmodels} considers how the critical energy of the fitted spectra changes with the energy of the measured electrons:
...
Using the LL equation we estimated that the intensity at the interaction point must have been around $a_0 \sim 9\pm1$ (or $10\pm2$ in a quantum model) which slightly undervalues the experiment conditions of $a_0 \approx 12\pm1$ suspected due to a mismatch in the timing (Section \ref{Chap:RR:Sec:Intensity}).
...
Figure \ref{RR15:Figs:RRmodels} shows the 4 successful collisions with the fitted critical energies for the gamma spectra on the $y$-axis and the measured electron energy of the spectral feature on the $x$-axis. We will now use different models for this interaction to determine where in this parameter space events would be most likely. For this purpose, we sample experimental parameters from distributions incorporating the experimental fluctuations and uncertainties we characterised (energy stability, overlap, fitting errors), and calculate the energy loss of the electron beam with mean energy $\epsilon = 550\pm 40\,\mathrm{MeV}$ and the generated radiation spectra in the respective model for intensities ranging from $a_0=4$ to $20$. The respective $1\sigma$ contours are then plotted onto Figure \ref{RR15:Figs:RRmodels} as comparison to the experimentally measured data.

First, we consider a classical description. The orange contour represents the results for an interaction described by the Lorentz force and the classical LL radiation reaction force, only considering the dominant term as higher order terms are suppressed by at least $1/2\gamma^2 \ll 1$.
The red contour is generated using a semi-classical model. Here the equation of motion of the particles is governed by a classical description and is deterministic, but the emission power is reduced to match the quantum description, using the Gaunt factor which in the limit of low-intensity and low-energy interactions approaches 1 \cite{Baier1991_GAUNT,Ridgers2017_QRR}. 
Due to the reduced emission spectrum, the predicted critical energies are lower than for the classical model (orange) for the same final electron energy, which means that the purely classical description systematically underestimates $a_0$ relative to the semi-classical model.

The blue contour indicates the parameter space most likely to be occupied by interactions correctly described by a quantum model, which includes the stochastic nature of emissions in the interaction, derived in the locally constant field approximation \cite{Ritus1985_QRR}. 
In this model, the electrons follow a classical trajectory between discrete emission events. 
The contour closely aligns with the semi-classical model as the average emission power is by definition matched. However, at lower final energies, i.e. more intense interactions, the contour starts to separate towards higher critical energies. 
The finite radiation lifetime allows electrons to propagate unperturbed between different radiation events and to release the equivalent energy within one single emission event. 
In the astrophysical context, this phenomenon is referred to as \textit{`energy straggling'} \cite{Shen1972_STRAGGLING}. 
This only occurs when the stochastic nature of the process is considered and leads then to a spectral hardening of the emitted radiation.
Finally, we consider a model without energy loss. The electrons follow an equation of motion governed by the Lorentz force and the emitted radiation is calculated as in the quantum case, but the electrons do not lose energy in the process. This is indicated as a green contour, centred on the initial mean energy.
...
On a $1\sigma$ level, it appears that the data points are in better agreement with models including quantum corrections (semi-classical and quantum model, red and blue) than with the purely classical LL description (orange). 
To improve the confidence, we would require more data points, interactions at higher intensities and electron energies or use a different set of observables that is more sensitive to differences in the models \cite{Ridgers2017_QRR}.
A technique to determine $a_0$ at the interaction on-shot \cite{HarShemesh2012_INTENSITY,Yan2017_ICS} would also help to constrain the experimental conditions further, in particular if the value of the inferred intensity were model-independent \cite{Blackburn2019_ModelIndependentLaser} separating experimental from model uncertainties.

\section[Electron beam stability and model distinction]{Electron beam stability and model distinction for future measurements}
\label{Chap:RR:Sec:StabilityStatisticsModel}
...
In addition, the data seems to be more consistent with the models including quantum corrections at the $1\sigma$ level. 
However, due to the low number of shots and the intrinsic fluctuations of the setup, a more definite discrimination of radiation reaction models is not possible at this point. 
More data, stability, higher electron energies, laser intensities and additional observables could enable this as part of a future precision measurement of radiation reaction.

The following section will investigate the stability of gas targets used in two measurements of radiation reaction at \textsc{Gemini} under similar experiment conditions. One is the already presented data from a gas jet (\textit{Cole et al.} \cite{Cole2018_RR}), the second is from a gas cell target (\textit{Poder et al.} \cite{Poder2018_RR}). For this purpose, the electron spectra of the second experiment undergo a statistical analysis similarly as described before.
...
\begin{figure}[h]
\centering 
\includegraphics[width=0.8\columnwidth]{ExpSetup_Poder.pdf}%\includegraphics[width=0.9\columnwidth]{ExpSetup_GasJet_MagnetOnly.pdf}
...
By factoring out known correlations of the spectra, scaling them accordingly [slow drift for gas jet data (\textit{Cole et al.}) and laser energy for gas cell data (\textit{Poder et al.})], and by normalising the spectra to their total charge, it is possible to compare the spectra. Expected intrinsic or non-attributed fluctuations can be characterised and taken into consideration.

\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{Comparison_ESpectra_insetVar.pdf}
\caption[Averaged electron spectra for Cole and Poder, along with their variance.]{Averaged and scaled electron spectra for data from Cole (blue) and Poder (green). The spectra are normalised to a total charge of 1. The error bars indicate the energy dependent variance of the spectra. Inset: Energy-dependent variance of the average spectra, following the same colour scheme. The total variance for Cole et al. was $1.5 \times 10^{-7}$ and $1.98 \times 10^{-8}$ in the case of Poder et al.}\label{fig:comparison_especs_InsetVarLog}
\end{figure}

Figure \ref{fig:comparison_especs_InsetVarLog} shows the scaled and averaged spectra from the two experiments. 
The spectra were processed as outlined before and then averaged over all available data shots. The lineouts are normalised by their total charge such that the integral of each spectrum is set to 1. The typical energy reached in the gas cell from \textit{Poder et al.} is significantly higher, by at least a factor two for the majority of the charge distribution. 
The higher electron energy also enables a potentially higher $\eta$ parameter in an interaction as $\eta \propto \gamma$. The shaded region around the scaled spectra indicates the standard deviation of the averaged spectrum at that particular energy. 
The energy-dependent variances are also shown in the inset of Figure \ref{fig:comparison_especs_InsetVarLog} on a logarithmic $y$-axis. 
The variations for the data taken with the gas jet (blue) are in particular strong around the important position of the spectral edge and the variance reaches a 10-times higher peak and total value.

\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{Comparison_Histo_errors.pdf}
\caption[Distribution of relative energy deviations from mean electron energy for Cole and Poder.]{Distribution of the relative energy deviations from the mean ($\Delta \epsilon/\epsilon$ for Cole et al. (blue) and Poder et al. (green) after scaling. The overlaid lines are kernel density estimates (KDEs). The shaded areas indicate the $\pm2$ sigma intervals assuming a normal distribution.}\label{fig:comparison_histo}
\end{figure}
...
Based on the statistical analysis of the electron spectra from both experiments, we want to investigate now how these fluctuations affect the confidence in the identification of radiation reaction effects in the electron spectra.

For this purpose, we sample electron energies from two normal distributions with the same mean energy $\epsilon = 550\,\mathrm{MeV}$ and the measured standard deviations $\sigma_{Cole} = 7.7\%$ and $\sigma_{Poder} = 3.5\%$. 
The sampled energies are shown in Figure \ref{RR15:Fig:Sampled_noRR_RR} in blue for a normal distribution with $\sigma_{Cole}$ and in green with $\sigma_{Poder}$ as previously.
...
If we assume a fixed relation of number of successful collisions and reference shots, we can estimate the number of shots we require to distinguish a `no RR' from a `LL-RR' distribution at a confidence of $Z$ sigma. 
In the case of an equal number of successful collisions and reference shots, the minimum number of shots required is given by

\begin{equation}
\boxed{N_{min} = Z^2 \frac{\sigma_1^2 + \sigma_2^2}{(\overline{\mu}_1-\overline{\mu}_2)^2}.}
\end{equation}

In an experiment, we also have to consider that only 1 in $X$ shots with both beams will result in a successful collisions. In this case, it was estimated to be a 1 in 3 chance, so the total number of shots would then be $4 \times N_{min}$ or in general terms $(1+X)N_{min}$ shots. We assume that we can identify successful collisions using the method described at the beginning of this Chapter, and that we know the intensity at the interaction which for now remains constant throughout the dataset. 
...
At $a_0 = 8$ and $\epsilon = 550\,\mathrm{MeV}$, the number of collisions shots required to reach $Z = 5$ is then $n = 15.7$ for $\sigma_{Cole}$ and $n = 3.4$ for $\sigma_{Poder}$, for $Z = 3$ we obtain $n = 5.6$ and $n = 1.2 $, respectively. A single shot level $n=1$ still requires a proper characterisation of the reference (`no RR') distribution.

At a $0.05$ Hz repetition rate like at the \textsc{Gemini} laser system (1 shot/20 s), we would then take at least 21 minutes to reach $5\sigma$ for $\sigma_{Cole}$, given no failed shots and stable conditions.
...
Using this method, we can easily determine $N_{min}$ for a range of standard deviations and electron energies. 
Figure \ref{RR:fig:a0_8_20_matrix} shows the minimum shot numbers for $Z= \lbrace 5, 3\rbrace$ and $a_0 = \lbrace 8, 20\rbrace$ for standard deviations between 1-10\%, and 100 to 2500 MeV electron energy, spanning a realistic parameter space of electron properties at \textsc{Gemini}. 
The blue and green markers now indicate the conditions of \textit{Poder} and \textit{Cole et al}. 
If we include the higher electron energy measured by \textit{Poder et al.}, the number of successful collisions required to distinguish the cases no-RR and RR (LL) falls to 1, i.e. one successful collision suffices as long as the unperturbed electron spectrum is characterised well enough. 
This is consistent with the findings in \cite{Poder2018_RR}.
...
If the laser focus is larger than the electron beam, the interaction can be reduced to a 1D problem which simplifies the modelling process. Gas jets have in the past produced very stable electron beams with narrow energy spread \cite{Faure2006_STABLEJET} and have also reached energies beyond $1\,\mathrm{GeV}$ \cite{Mirzaie2015_GEVJET,Kim2013_GEV,Mangles2016_LPA_Review}, which indicates that the performance of this target was maybe not representative of what is achievable to-date. 
...
It is assumed that successful collisions can be identified, for instance using the method described at the start of this Chapter, and that the laser intensity remains fixed in all interactions. 
Fluctuations or uncertainties are only considered for the electron beam in terms of energy, and for both laser and electron beam for the spatio-temporal jitter, but are not extended to other laser or electron parameters.

We also only considered the binary scenario where either the entire electron beam or nothing interacts. 
This might be approximately true in the case where the waist of the laser pulse is at the interaction wider than the electron beam, but is not the case if the opposite is true. 
In this scenario, the exact on-shot laser profile and wavefront becomes more important, and the radiation reaction signature is hidden under the remaining, unperturbed electron beam. 
This could represent an avenue to achieve an on-shot reference on upcoming more powerful laser systems \cite{Baird2019_RR}.

This analysis also neglects effects that could become important for longer interactions and tight focusing, such as change in intensity due to focusing effects and a chirp in the electron beam energy \cite{Harvey2016_ICSFOCUS}.

Furthermore, we used the Landau-Lifschitz description to calculate the post-interaction distribution. It would be of interest, in particular for future precision studies, to discriminate different radiation reaction models, for instance the full quantum model including stochasticity and the semi-classical model introduced in the previous section. Limitations of the local constant field approximation (LCFA) \cite{Ritus1985_QRR}, which is basis for the models including quantum corrections, are also an active field of research and require experimental validation \cite{DiPiazza2018_LCFA}.
...
Successful high-intensity collisions were signalled by bright bursts of gamma radiation recorded on the gamma detectors.
The expected radiation background from bremsstrahlung was found by correlating the electron spectra and the signal on the gamma-detector on shots without the scattering laser. Over the course of 13 shots, this method identified 4 successful collisions with an excess signal of more than 5 standard deviations above background, i.e. at a probability of 1 in 3,500,000 to occur by chance on one shot, assuming a normal background distribution. 
\vspace{\baselineskip}

On the 4 identified collisions, the energy of the spectral feature was lower than the average of the shots without the scattering laser. 
Considering a larger dataset, the statistical fluctuations of the spectral feature without the scattering laser were characterised and correlations to other experiment parameters explored. Based on this statistical analysis, the combined probability of measuring 4 spectral features at such low energies is about $0.3\%$, rendering this a statistically significant measurement of energy loss.
\vspace{\baselineskip}
...
The characteristic energy of the gamma spectrum is higher on shots without the scattering laser as the detector response is dominated by bremsstrahlung, whilst exhibiting a lower yield. The characteristic energies of the 4 identified collisions are anti-correlated with the electron energy, all below $500\,\mathrm{MeV}$, which is not consistent with radiation produced after measuring the energy of the electron beam.
