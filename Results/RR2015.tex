...
In relativistic \textit{inverse Compton scattering} (ICS), sometimes also referred to as Compton Backscattering (CBS) or, in the classical limit, relativistic Thomson scattering, one (linear) or multiple (non-linear) photons scatter from a relativistic electron of energy $\epsilon$, with $\epsilon/m_e c^2 = \gamma \gg 1$. The scattered photons experience a relativistic Doppler-shift and are re-emitted in a narrow cone of divergence $\sim 1/\gamma$ in the direction of the electron propagation, carrying a higher energy based on the electron energy and the intensity of the laser field. The emitted photon energy, $\epsilon_{ph}'$, is maximised in a head-on collision: $\epsilon_{ph}' = 2 \gamma^2 (1-\cos\theta)\epsilon_{ph}$ (see e.g. \cite{Esarey1993_NT,Lau2003_NT} or Section \ref{Theory:Sec:LinearICS}), where $\epsilon_{ph}$ is the photon energy before the interaction and $\theta$ is the angle between the photon and the electron. Higher electron energies result in a stronger shift of the radiation towards higher energies, also called `hardening of the spectrum', and an increase of the corresponding energy loss the electrons experience. A more intense laser field ($a_0 > 1$), on the other hand, enables non-linear interactions and increases the number of photons interacting with an electron, resulting in the generation of $\sim a^3_0$ higher harmonics \cite{DiPiazza2012_ICS} but also redshifting and broadening of the spectrum due to the relativistic mass increase \cite{Sarachik1970_ICS,Esarey1993_NT,Lau2003_NT,Koga2005_ICS}. As a result the rapidly rising number of higher harmonics blend together to form a broadband synchrotron-like spectrum and the characteristic energy of the spectrum, $\epsilon_{ICS} = 3 \gamma^2 a_0 \hbar \omega_0$, increases linearly with $a_0$.
...
In addition, the high photon energies offer the opportunity to directly measure the effect of \textit{radiation reaction} on the beam dynamics itself and to observe energy loss, if the emitted photon carries an energy comparable to the energy of the electron $\gamma^2 a_0 \frac{2 r_e \omega}{3c} \sim 1$ \cite{Albert2016_APP}, where $r_e$ is the classical electron radius. The attempt to solve radiation reaction self-consistently within classical electrodynamics exhibits unphysical predictions like self-acceleration, resulting in runaway solutions, or an unbounded emission spectrum, i.e. energy loss higher than the initial energy of the electron is allowed \cite{Blackburn2019_RRReview}. A universally accepted practicable description of radiation reaction has not been found yet.
...
The total energy emitted through bremsstrahlung by a relativistic electron is proportional to its energy squared (e.g. \cite{Jackson} and Section \ref{Theory:Sec:Bremsstrahlung}). The total energy deposited in the caesium-iodide crystals of the gamma spectrometer is assumed to convert linearly into scintillation light \cite{Frlez2000_CsI} at an efficiency of $\approx 5 \times 10^4 \,\mathrm{photons}/\mathrm{MeV}$. The total yield of the detected bremsstrahlung signal on the camera chip, $S_{BG}$, should then follow this relation:
\begin{equation}
...
On some shots with both beams the contribution of bremsstrahlung and ICS might be of similar level the inferred characteristic energies take an intermediate level.
\vspace{\baselineskip}
...
The blue contour indicates the parameter space most likely to be occupied by interactions correctly described by a quantum model which includes the stochastic nature of emissions in the interaction, derived in the locally constant field approximation \cite{Ritus1985_QRR}. In this model, the electrons follow a classical trajectory between discrete emission events. The contour closely aligns with the semi-classical model as the average emission power is by definition matched. However, at lower final energies, i.e. more intense interactions, the contour starts to separate towards higher critical energies. The finite radiation lifetime allows electrons to propagate unperturbed between different radiation events and to release the equivalent energy within one single emission event. In the astrophysical context this phenomenon is referred to as \textit{`energy straggling'} \cite{Shen1972_STRAGGLING}. This only occurs when the stochastic nature of the process is considered and leads then to a spectral hardening of the emitted radiation.
