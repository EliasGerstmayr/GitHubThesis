...
Following this Chapter the author will introduce some \nameref{Chap:Theory} and \nameref{Chap:Methods} underpinning this work.

The main experimental results of this thesis will follow in Chapters \ref{Chap:linICS}, \ref{Chap:RR15} and \ref{Chap:BW}:

\begin{enumerate}
\setcounter{enumi}{4}
\item \textbf{Linear Inverse Compton Scattering and Beam Profile Diagnostic:}\\
Highly relativistic electrons from a wakefield accelerator were collided with a defocused laser pulse at $a_0 \sim 0.3$ generating sharply peaked gamma radiation from linear inverse Compton Scattering in the 10s $\mathrm{MeV}$ range. A novel gamma-spectrometer design that allows measuring the response of the detector from two sides was fielded for the first time. The application of linear ICS as electron beam profile diagnostic is explored.
...
A dual laser setup is used to produce a $\mathrm{keV}$ X-ray source and a 100s $\mathrm{MeV}$ gamma beam from bremsstrahlung in an attempt to measure the elusive linear Breit-Wheeler process. The converter material and the electron source are optimised to maximise the yield of gamma rays at the interaction while finding the lowest noise.
\end{enumerate}
...
The conclusion is followed by supplementary material in the Appendix.
