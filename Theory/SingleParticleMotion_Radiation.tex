...
\EliasComm{Add regimes from Tom's review here. Add plot.}
...
\EliasComm{Structuring: either QRR in general and then signatures, or maybe explain QRR generally and then have one section just one signatures (relying on Chris Ridger's bits).}
...
\EliasComm{Add some bits here about the time scsale of the laser. Tom compares the duration of the laser field with the time interval.}
...
\subsubsection{Signatures of radiation reaction}
...
Classical
\begin{equation}
\left( \frac{\dif \langle p \rangle}{\dif t}\right)_{cl} = - \frac{\langle P_{cl} \hat{p}}{c}
\end{equation}
Modified
\begin{equation}
\left( \frac{\dif \langle p \rangle}{\dif t}\right)_{mod cl}- \frac{\langle g P_{cl} \hat{p}}{c}
\end{equation}
