...
There are different parameters to indicate the significance of radiation reaction effects and to signal the appropriate description that describes the dynamics most accurately in this regime.

One such parameter is the classical radiation reaction parameter, $R_c$ \cite{DiPiazza2010_QRR}:
\begin{equation}
\boxed{R_c = \frac{\alpha a^2_0 \gamma_0 (1+\beta_0) \omega_0}{m},}
\end{equation}
...
\EliasComm{Explanation what it means.}
...
The \textit{quantum nonlinearity parameter}, on the other hand, indicates the strength of the electric field (in the rest frame) relative to the critical field of QED:
\begin{equation}
\eta = E_{RF}/E_{crit},
\end{equation}
...
\end{equation}
with quantum effects dominating when $\eta \rightarrow 1$, which can be achieved by high intensities and electron energies.
In terms of $a_0$ and $\gamma_0$ quantum effects become significant at
\begin{equation}
a_0 \gamma_0 > \frac{m_e c^2}{2 \hbar \omega_0}
\end{equation}
This indicates when we have to consider quantum radiation reaction.

Figure \ref{Theory:Figs:RR:RegimesOfRR} shows a map of $\eta$ (there called $\chi$), effectively $\gamma$, and $a_0$, along with interesting regimes ($\chi = 1$) and $R_c$, as well as previous measurements of radiation reaction phenomena. Circles indicate colliding-pulse experiments, whereas the triangle shows the parameter of an experiment using aligned crystals. The red dashed line at $\chi = 0.1$ indicates when quantum effects become significant. The solid red line indicates the quantum dominated regime. The shaded regions show different values of $R_c$. The black line indicates the position on the plane when increasing the intensity of the laser $a_0$ in an interaction with an electron beam of fixed energy $\gamma = 1000$.
...
\caption[Overview of radiation reaction regimes and experiments.]{Overview of radiation reaction regimes and experiments. Circles indicate experiments colliding an electron beam and a laser pulse. The triangle denotes an experiment based on planar channeling. Reprinted from \cite{Blackburn2019_RRReview}.}
\label{Theory:Figs:RR:RegimesOfRR}
\end{figure}

\subsubsection{Locally constant crossed field approximation}

An important assumption is that the electric field remains constant over the formation time of the photon. A common approximation including this is the so-called locally constant crossed field approximation (LCFA).

The formation length can be estimated geometrically by
\begin{equation}
...
\end{equation}
LCFA holds if the ratio is small, at high intensities, i.e. large $a_0$. LCFA also requires $a_0 \gg 1$ and $a^3_0 \gg \chi$ large, i.e.
...
\begin{figure}
\centering
\includegraphics[height=.4\columnwidth]{DiracSea.png}
\caption{Feynman diagram for Volkov state equivalent to infinite series of diagrams}
\label{Theory:Figs:Schwinger_DiracSea}
\end{figure}
...
At extreme field strengths electron-positron pairs can be created directly from the vacuum: here the virtual electron-positron pairs, quantum fluctuations, are accelerated to their rest mass within a Compton wavelength. This process is comparable to a tunneling process where now the virtual particles tunnel out of the Dirac sea into real space. This is shown in Figure \ref{Theory:Figs:Schwinger_DiracSea}. This process is called Schwinger pair production \cite{Schwinger1951_VACPAIRS}, with the Schwinger field \cite{Sauter1931_ECRIT,Heisenberg1936_ECRIT}

\begin{equation}
\boxed{E_S = 1.36\times 10^{18}\,\mathrm{V m^{-1}}}
\end{equation}
\EliasComm{Add physical units.}
...
Production rate is something like DiPiazza2012REF\addref{}
\begin{equation}
R (Y_0) = 
\begin{cases}
...
\EliasComm{Add physical diagrams of multi ionisation and tunnel ionisation}