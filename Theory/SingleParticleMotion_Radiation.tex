...
Whilst the highest significant order increases at $a^3_0$, the energy of the emitted radiation is red-shifted at $E_f \propto 1/a^2_0$ for $a_0 \gg 1$, such that the net energy gain is linear in $a_0$.
...
The \textit{classical radiation reaction parameter}, $R_c$, for instance, indicates how much energy is radiated in a single cycle relative to the electron energy in the average rest frame, $f = E_{rad}/(\gamma' m) = 4\pi R_c/3$. $R_c$ is in natural units and in terms of experimental variables given by \cite{DiPiazza2010_QRR,Blackburn2019_RRReview}:
\begin{subequations}
\begin{empheq}[box=\widefbox]{align}
R_c &= \frac{\alpha a^2_0 \gamma_0 (1+\beta_0) \omega_0}{m} \\ \nonumber
&\approx 0.13 \left(\frac{E_0}{500\MeV}\right) \left( \frac{I_0}{10^{22}\,\mathrm{W/cm^2}}\right)\left(\frac{\lambda}{\mathrm{\mu m}}\right),
\end{empheq}
\end{subequations}
where $\alpha$ denotes the fine-structure constant, $a_0$ the normalised vector potential,  $\omega_0$ is the laser frequency and $m$ is the electron mass, $\lambda$ is the laser wavelength, $I_0$ the peak intensity and $E_0$ the initial energy of the electron. $\gamma' = (1+a^2_0)^{1/2}$ and $\beta' = a_0 (1+ a^2_0)^{-1/2}$. 
...
Considering the special case of one high-energy photon colliding with $n$ identical low-energy photons \cite{Bula1996_RR,Burke1997_RR}, for instance optical photons from a laser, the corresponding centre-of-mass energy can simply be written as  $s = (E_{CM}/m_e c^2)^2 = E_{\gamma} n E_{\gamma_n} (1-\cos\phi)/2 m^2_e c^4$. 
