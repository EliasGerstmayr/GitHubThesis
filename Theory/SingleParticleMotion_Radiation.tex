...
\end{equation}
including the effect of the mass increase ($m_e \rightarrow m^\ast = m_e \sqrt{1+a^2_0}$) the energy of the radiation in the laboratory frame is then given by \cite{Esarey1993_NT}
\begin{equation}
...
The emission/scatter rates for the $n$-th harmonic depend on the laser polarisation \cite{Ritus1985_QRR,BlackburnThesis}. The scattering rate (spin-averaged and unpolarised states in/out) for $n$ scattered photons for circular polarisation is given by a series of Bessel functions
\begin{equation}
\frac{\dif W_n}{\dif x} = \frac{\alpha m^2}{4 E}\left[ -4 J^2_n + a^2_0 \left(1-x+ \frac{1}{1-x}\right) (J^2_{n-1} + J^2_{n+1} - 2 J^2_n)\right],
\end{equation}
where we have to evaluate $n$ differential cross sections to reproduce the right spectrum. The highest significant order scales as XXX. However, we see that even at moderate intensities we already have to evaluate a high number which becomes unfeasible. Alternatively, use the quantum synchrotron function (see next section).
