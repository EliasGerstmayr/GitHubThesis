...
\end{equation}
including the effect of the mass increase ($m_e \rightarrow m^\ast = m_e \sqrt{1+a^2_0}$) on the energy of the radiation in the laboratory frame is given by \cite{Esarey1993_NT}
\begin{equation}
\boxed{E_f \approx \frac{2 \gamma^2 (1- \cos \theta) n E_i}{1+a_0^2/2 + (\gamma \theta)^2},}
\end{equation}
which for higher $a_0$ leads for a fixed harmonic $n$ to a redshifting of the radiation $E_f \propto 1/a^2_0$. This already becomes significant for $a_0 \sim 0.1$. The contribution $(\gamma \theta)^2$ leads to a fall-off of the photon energy off-axis, so that it reaches $E_f/e$ at about $\theta \sim 1/\gamma$ and the emitted radiation is confined to a cone of that opening angle.
