...
In the following we will consider the production of radiation through the processes that are relevant in the course of this work.
First, inverse Compton scattering or, in its classical limit, relativistic Thomson scattering is considered, where one (linear) or multiple photons (nonlinear) are scattered off a relativistic electron to produce highly energetic radiation. This is followed by a brief discussion of the bremsstrahlung process. 

There are two main routes to calculate a radiation spectrum for a specific process.
In a classical picture we first derive the equation of motion of the charged particle and analyse it in terms of relativistic Lienard-Wiechert potentials. Solving the corresponding radiation integrals through a Fourier analysis then provides the classical answer. Similar equation of motions then provide similar radiation spectra which then enables identifying similar behaviour. An example is, for instance, the figure-of-eight motion (see Section \ref{Theory:Sec:SingleParticle:FigOfEight}), which is common to betatron radiation, synchrotron and undulators. Also for Compton scattering.

In a quantum picture the the cross sections and spectra are obtained by solving the Feynman diagrams related to the process. The evaluation of a matrix element, even a simple fundamental tree-level diagram, is relatively tedious (see Section \ref{Appendix:QEDDeriv_KleinNishina}), but processes with a similar Feynman representation can be related to each other (crossing symmetry) and the small coupling constant, $\alpha \approx 1/137$, allows a perturbative treatment (contrast QCD for instance) such that more complicated processes contribute only little. However, at high field strengths the number of contributing diagrams increases significantly, e.g. at $a^3_0$ for nonlinear Compton scattering, which makes the evaluation of standard Feynman diagrams not very feasible and computationally challenging. To restore an easy perturbative treatment `dressed states' are being introduced, which replaces, for instance, an electron interacting with many photons with one dressed electron, i.e. an electron in a background field. The dressed state uses the exact classical description of the background field, assuming that the quantum nature is not in the field itself. LCFA? This is referred to as the Furry picture. In this picture one dressed or Volkov state is equivalent to the infinite sum of standard Feynman diagrams.
\vspace{\baselineskip}
...
The quantum nonlinearity parameter, on the other hand, indicates the strength of the electric field (in the rest frame) relative to the critical field of QED:
\begin{equation}
\eta = E_{RF}/E_{crit},
\end{equation}
...
Figure \ref{Theory:Figs:RR:RegimesOfRR} shows a map of $\eta$ (there called $chi$), effectively $\gamma$, and $a_0$, along with interesting regimes ($\chi = 1$) and $R_c$, as well as previous measurements of radiation reaction phenomena. 
...
\caption[Overview of radiation reaction regimes and experiments.]{Overview of radiation reaction regimes and experiments. From \cite{Blackburn2019_RRReview}. Circles indicate experiments colliding an electron beam and a laser pulse. The triangle denotes an experiment based on planar channeling.}
...
Finally, when $\eta$ approaches unity we also have to consider prolific pair production and subsequent QED cascades that might deplete the laser pulse.
...
In a colliding pulse experiment we expect to see two signatures in the electron spectrum. One is the energy loss, the second is the variance of the electron spectrum. The signatures are different for different models and can be used to discriminate models. 
References Ridgers  \cite{Ridgers2017_QRR} and Arran \cite{Arran2019_RR_PPCF,Arran2019_RR_SPIE}

Arran investigates this in a detailed model to find optimum conditions to distinguish models \cite{Arran2019_RR_PPCF}.
...
More details on the Gaunt factors are found in the Appendix \ref{Appendix:Sec:QuantumSynchrotron}.

Considering Figure \ref{Theory:Figs:RidgersEspec} again we see that the classical model reduces the width of the electron beam the most, whereas the modified model also reduces the width but by less as the emission power is reduced. The stochastic model, on the other hand, shows a strong broadening, which for longer laser pulses reduces again (see Ridgers \cite{Ridgers2017_QRR}).
