...
The evaluation of a matrix element, even a simple fundamental tree-level diagram, is relatively tedious (see Appendix \ref{Appendix:QEDDeriv_KleinNishina}), but processes with a similar Feynman representation can be related to each other (crossing symmetry) \cite{PeskinSchroeder} and the small coupling constant, $\alpha \approx 1/137$, allows a perturbative treatment such that more complicated processes contribute only little overall and can be neglected. 
...
\end{equation}
where $S$ is a measure of stochasticity and is given in Appendix \ref{Appendix:Sec:QuantumSynchrotron}.
