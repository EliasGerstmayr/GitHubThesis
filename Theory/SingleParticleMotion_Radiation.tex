...
In the following we will consider the production of radiation through the processes Compton scattering, in different regimes, and bremsstrahlung.
...
the energy spread of the electron beam, $\Delta \gamma$, will translate to an energy spread in the ICS spectrum, $\Delta E_f$.
Short pulse lasers intrinsically require a certain bandwidth are hence not monochromatic, so that the spread of photon energies, $\Delta E_{ph}$, modifies the spectrum similarly.
...
\iffalse
or in terms of $a_0$ and $\gamma_0$ 
\begin{equation}
...
\end{equation}
This description only holds for length scales larger than the Compton wavelength, $L \gg \lambda_C$, and electric field strengths much smaller than the critical field strength, $E \ll E_{cr}/\alpha$.
\vspace{\baselineskip}
...
\caption{Electron with different models. From \cite{Ridgers2017_QRR}.}
\end{figure}
...
At high photon densities multiple photons ($n>1$) can interact with one high energy photon to produce a single electron-positron pair.
...
\end{cases}
\end{equation}
$\chi = \hbar \omega E_L/2 m_e c^2 E_S$ is the quantum parameter, with the electric field of the laser, $E_L$, and the Schwinger field, $E_S = 1.38 \times 10^{18} \,\mathrm{V m^{-1}}$.
