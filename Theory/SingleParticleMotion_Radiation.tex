...
In a \textit{quantum} picture, the cross sections and spectra are obtained by calculating the matrix element from the Feynman diagrams related to the process. The evaluation of a matrix element, even a simple fundamental tree-level diagram, is relatively tedious (see Appendix \ref{Appendix:QEDDeriv_KleinNishina}), but processes with a similar Feynman representation can be related to each other (crossing symmetry) \cite{PeskinSchroeder} and the small coupling constant, $\alpha \approx 1/137$, allows a perturbative treatment such that more complicated processes contribute only little and can be neglected. However, at high field strengths the number of contributing diagrams increases significantly, e.g. at $a^3_0$ for nonlinear Compton scattering \cite{DiPiazza2012_ICS}, which makes the evaluation of standard Feynman diagrams not very feasible and computationally challenging. To restore a perturbative treatment \textit{`dressed states'} are being introduced, which replaces, for instance, an electron interacting with many photons with one dressed electron, i.e. an electron in a background field. The dressed state uses the exact classical description of the background field, assuming that the quantum nature of the interaction does not originate in the field itself and that there is no backreaction of the particles to the field. This is referred to as the \textit{Furry picture} \cite{Furry1951_FURRY}. In this picture one dressed or \textit{Volkov state} \cite{Wolkow1935_VOLKOV} is equivalent to the infinite sum of standard Feynman diagrams \cite{SeiptThesis}.
...
\end{empheq}
\end{subequations}
where $\alpha$ denotes the fine-structure constant, $a_0$ the normalised vector potential,  $\omega_0$ is the laser frequency and $m$ is the electron mass. $\gamma' = (1+a^2_0)^{1/2}$ and $\beta' = a_0 (1+ a^2_0)^{-1/2}$. 
...
In the context of the collision of a relativistic electron beam of energy $\gamma$ with a laser pulse of intensity $a_0$ the relative magnitude of the Landau-Lifschitz radiation reaction force (see Section \ref{Theory:Sec:ClassRR:LL}), for instance, can be estimated by \cite{Thomas2012_LL}
\begin{equation}
...
Beyond these classical descriptions, we can also find parameters to indicate how `quantum' an interaction will be. The \textit{quantum nonlinearity parameter}, for instance, indicates the strength of the electric field (in the rest frame) relative to the critical field of QED:
\begin{equation}
\eta = E_{RF}/E_{crit},
\end{equation}
with the critical field for quantum electrodynamics $E_{crit} = 1.38 \times 10^{18}\,\mathrm{V m^{-1}}$ \cite{Sauter1931_ECRIT,Heisenberg1936_ECRIT}.
...
In terms of $a_0$ and $\gamma_0$ quantum effects dominate for
\begin{equation}
...
Figure \ref{Theory:Figs:RR:RegimesOfRR} shows a map of $\eta$ (there called $\chi$) and $a_0$, that indicates interesting regimes for the parameters $\eta$ and $R_c$, as well as previous experimental measurements of radiation reaction. Circles indicate colliding-pulse experiments, whereas the triangle shows the parameter of an experiment using aligned crystals. The red dashed line at $\chi = 0.1$ indicates when quantum effects become significant, whereas the solid red line indicates the quantum dominated regime. The shaded regions show different values of $R_c$. The black line indicates the position on the plane when increasing the intensity of the laser $a_0$ in an interaction with an electron beam of fixed energy $\gamma = 1000$, which corresponds to moving diagonally on the plane. Upwards means more quantum, sideways more nonlinear. Diagonal more radiation reaction.
...
If we also include the requirement that $a^3_0 \gg \eta$, this approximation is then called the locally constant crossed field approximation (LCFA).
This approximation requires $a_0 \gg 1$ and $a^3_0 \gg \chi$ large, i.e.
the particle has to be ultrarelativistic and background is weak compared to critical field of QED.
LCFA allows the classical treatment of the background field and, as a result, the particle trajectories between emission events that are governed by QED can be determined classically, enabling in a semi-classical treatment of the process overall.
...
\end{cases}
\end{equation}
$\Delta t$ is given by $\Delta t = \langle \omega \rangle / P$, where P is the emission power $P_q = 2 \alpha m^2 \eta^2 g(\eta)/3$ and $\omega$ is the average photon energy.
...
The Gaunt factor can be analytically approximated by
\begin{equation}
...
The second observable we consider is the evolution of the variance or the energy spread of the electron spectrum. Classical models are deterministic and as such yield one-to-one predictions of the final energy of an electron undergoing an interaction with an intense laser pulse. Since higher electron energies result in a higher energy loss, the energy spread of an electron beam experiencing a classical radiation reaction force always results in a lower final energy spread.
...
Whereas the deterministic models predict a decrease in variance in all circumstances, we see that this equation consists of two terms with opposite signs such that the variance can increase or at least decrease at a slower rate if the stochasticity is significant.
For a Gaussian electron beam the crossover point below which stochastic broadening dominates is for $\sigma_{\epsilon_i} \ll \langle \epsilon_i \rangle$ given by \cite{Arran2019_RR_PPCF}
\begin{equation}
...
Returning to Figure \ref{Theory:Figs:RidgersEspec} \cite{Ridgers2017_QRR} again, we now also consider the shape of the electron distributions. We see that the width of the distribution has narrowed for both the classical (black solid) and the modified classical (red) models as they are deterministic models, where the classical model has narrowed more. This is again consistent as the emission power is for the modified classical model reduced by the Gaunt factor. The model including stochasticity (blue) has, on the other hand, widened significantly whilst losing on average the same amount of energy as the electron beam experiencing the modified classical radiation reaction force. 
...
At extreme field strengths electron-positron pairs can be created directly from the vacuum, where virtual electron-positron pairs are created an anihilated in quantum fluctuations. This process is called \textit{Schwinger pair production} \cite{Schwinger1951_VACPAIRS} or sometimes nonperturbative pair production, with the \textit{Schwinger field} or \textit{critical field of QED} \cite{Sauter1931_ECRIT,Heisenberg1936_ECRIT}:
\begin{equation}
\boxed{E_S = \frac{m^2_e c^3}{e \hbar} \approx 1.36\times 10^{18}\,\mathrm{V m^{-1}}},
\end{equation}
where we see that by rearranging that
\begin{equation}
e E_S \lambda_C = m_e c^2,
\end{equation}
which gives an intuitive explanation of the field: $E_S$ is the static electric field required to accelerate a charged particle to an energy $m_e c^2$, the rest mass of an electron, within a Compton wavelength, $\lambda_C$.
The corresponding Feynman diagram is the virtual electron-positron pair in a loop in the presence of a strong background field Figure \ref{Theory:Figs:Schwinger_FeynmanDiag}.


\begin{figure}
\centering
\includegraphics[height=.25\columnwidth]{SchwingerDiagram.pdf}\includegraphics[height=.2\columnwidth]{feyn_Schwinger.pdf}
\caption{Nonlinear Breit-Wheeler as multiphoton ionisation process and Schwinger as tunneling. Feynman diagram for the Schwinger process.}
\label{Theory:Figs:Schwinger_FeynmanDiag}
\end{figure}

Another interpretation of the Schwinger process is to imagine it as something comparable to a tunneling process where now the virtual particles tunnel out of the Dirac sea into real space. This is shown in Figure \ref{Theory:Figs:Schwinger_FeynmanDiag} (centre).
Production rate is something like \cite{DiPiazza2012_ICS}
\begin{equation}
R (Y_0) = 
\begin{cases}
m Y^2_0 \exp\left(-\frac{\pi}{Y_0}\right) \,\, & Y_0 \ll 1,\\
m Y^2_0 \,\,& Y_0 \gg 1.
\end{cases}
\end{equation}
(Same in static and oscillating fields)
Static field \cite{Schwinger1951_VACPAIRS}
Oscillating (Brezin and Itzykson 1970 and Popov 1971/1972\addref)
This rate has a similar shape to ionisation.

Here the Keldysh parameter $\gamma_K = 1/a_0$ and $l_f \ll \lambda$, so static field
$\gamma_K = \omega \sqrt{2mI_p}/eE$, where we then identify $I_p \sim mc^2$.
If we can understand this as a tunneling process, then nonlinear BW can be understood as multiphoton ionisation.


This field is incredibly large such that we can not reach this in the laboratory. The field is, however, boosted in the presence of an electron and using the previously introduced quantity $\eta$ we then reach this quantity here.
However, even in the rest frame of an electron it is challenging to reach these high values as the production of pairs from the nonlinear BW process rises exponentially at $\eta \rightarrow 1$ and the electron loses energy drastically due to radiative effects. Cascades will deplete the laser pulse, such that the wavefront has to be tailored or we require very highly energetic electron beams. See \cite{Blackburn2019_SUPER}.
