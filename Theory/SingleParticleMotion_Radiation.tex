...
There are two main routes to calculate the spectrum for a specific radiative process.
In a \textit{classical} picture we first have to derive the equation of motion of the charged particle and then analyse its trajectory in terms of relativistic Lienard-Wiechert potentials \addref. Solving the corresponding radiation integrals through a Fourier analysis then provides the classical emission spectra \addref. Similar particle motions then provide similar radiation spectra which allows identifying similar behaviour in analogue scenarios. An example is, for instance, the figure-of-eight motion (see Section \ref{Theory:Sec:SingleParticle:FigOfEight}), which can be found in the context of electrons in (alternating) magnetic fields, e.g. synchrotrons and insertion devices, but similarly in an electron in a laser field\addref. As a result, we identify a wiggler parameter, $K$, which in one case is related to the magnetic field strength and in the other is the normalised vector potential, $a_0$. Similarly, the emitted radiation has a similar shape in the limits of $K$.

In a \textit{quantum} picture the the cross sections and spectra are obtained by calculating the matrix element from the Feynman diagrams related to the process. The evaluation of a matrix element, even a simple fundamental tree-level diagram, is relatively tedious (see Appendix \ref{Appendix:QEDDeriv_KleinNishina}), but processes with a similar Feynman representation can be related to each other (crossing symmetry) and the small coupling constant, $\alpha \approx 1/137$, allows a perturbative treatment (contrast QCD for instance) such that more complicated processes contribute only little and can be neglected. However, at high field strengths the number of contributing diagrams increases significantly, e.g. at $a^3_0$ for nonlinear Compton scattering, which makes the evaluation of standard Feynman diagrams not very feasible and computationally challenging. To restore an easy perturbative treatment \textit{`dressed states'} are being introduced, which replaces, for instance, an electron interacting with many photons with one dressed electron, i.e. an electron in a background field. The dressed state uses the exact classical description of the background field, assuming that the quantum nature is not in the field itself. This is referred to as the \textit{Furry picture} \addref. In this picture one dressed or \textit{Volkov state} is equivalent to the infinite sum of standard Feynman diagrams \addref.
\vspace{\baselineskip}
...
\end{cases}
\end{equation}
Stochastic effects are most significant when $\omega_0 \Delta t \geq 1$, when total number of emissions small but $\chi$ large. $\Delta t$ is the typical timescale between emissions $\Delta t = \langle \omega \rangle / P$, where P is the emission power $P_q = 2 \alpha m^2 \chi^2 g(\chi)/3$ and $\omega$ is the average photon energy.
\vspace{\baselineskip}
...
Here we will now consider the signatures or observables of radiation reaction in the electron spectrum in a colliding beam experiment, using \cite{Ridgers2017_QRR}. The two key signatures are the change in the momentum, predominantly witnessed through energy loss, the second is the variance of the electron spectrum. The evolution of these two properties is characteristic for different models of radiation reaction, such that these are suitable observables in an experiment to probe the process and to discriminate the models. The optimum conditions to distinguish different models using the descriptions presented in the following is, for instance, investigated in more detail in \cite{Arran2019_RR_PPCF,Arran2019_RR_SPIE}.

\subsubsection{Evolution of momentum or energy loss}

The average energy loss is given by the synchrotron power and the momentum operator \cite{Ridgers2017_QRR}, where the classical momentum function is the classical Larmor power
\begin{equation}
\left( \frac{\dif \langle p \rangle}{\dif t}\right)_{cl} = - \frac{\langle P_{cl} \hat{p}\rangle}{c}
\end{equation}
whereas in a quantum model the average emission power is reduced (see quantum synchrotron in Appendix \ref{Appendix:Sec:QuantumSynchrotron}) by the Gaunt factor which is equal for a deterministic and stochastic model (on average)
\begin{equation}
\left( \frac{\dif \langle p \rangle}{\dif t}\right)_{mod,cl} = - \frac{\langle g P_{cl} \hat{p}\rangle}{c},
\end{equation}
where $g$ is the Gaunt factor analytically approximated by
\begin{equation}
g(\eta) = [1 + 4.8(1+\eta)\ln(1+1.7\eta) + 2.44\eta^2]^{-2/3}.
\end{equation}
The Gaunt factor takes values from $0$ to $1$ such that the emission power is in most cases reduced.
See more Gaunt factor details in Appendix \ref{Appendix:Sec:QuantumSynchrotron}.

Figure \ref{Theory:Figs:RidgersEspec} shows the simulated energy loss of an electron beam interacting with a laser pulse. The different colours indicate different radiation reaction models. We see that the classical model shifts the electron spectrum down the furthers to about half of its initial energy, whereas the modified classical (red) and the stochastic model (blue) are shifted to about 67 percent of its initial energy instead.

As a result, the strength of the energy loss is a good indicator to distinguish a purely classical and a model including quantum corrections.
This is similarly resolved in the gamma spectrum (compare those). See Arran \cite{Arran2019_RR_PPCF} for model overlap discussion.

\subsubsection{Evolution of the variance or energy spread}

The second observable is the variance of the electron spectrum. In a classical model the energy loss is deterministic. At a fixed intensity the energy loss is larger for higher energies so that the variance decreases in a classical model. Considering the reduced emission power in the quantum correction by $g$ the reduction of variance is mitigated. Finally, considering stochasticity we have a second term that increases the variance. As a result, the variance is a potential observable to distinguish the deterministic and the stochastic models.
...
\end{figure}
The definition of the stochasticity term $S$ is given in the Appendix\addref{}.
We see that quantum characteristics are captured in the evolution of the variance \cite{Ridgers2017_QRR}, which makes this a suitable observable to identify the stochastic quantum nature as discrimination to effective deterministic, hence intrinsically classical models.
For a Gaussian the crossover point below which stochastic broadening dominates for small $\sigma_{\epsilon_i} \ll \langle \epsilon_i \rangle$ \cite{Arran2019_RR_PPCF}
\begin{equation}
\frac{\sigma_{\epsilon_i}}{\langle \epsilon_i \rangle} \approx 0.7 \sqrt{\langle \chi_e \rangle}.
\end{equation}
More details on the Gaunt factor and the explicit form of $S$ are found in the Appendix \ref{Appendix:Sec:QuantumSynchrotron}.

Considering Figure \ref{Theory:Figs:RidgersEspec} again we see that the classical model reduces the width of the electron beam the most, whereas the modified model also reduces the width but by less as the emission power is reduced. The stochastic model, on the other hand, shows a strong broadening, which for longer laser pulses reduces again as the effective $\eta$ varies throughout the interaction due to the energy loss (see Ridgers \cite{Ridgers2017_QRR}).
An analysis of the impact of stochasticity on different shapes, see \cite{Niel2018_RRStochastic}
