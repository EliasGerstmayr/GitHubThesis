...
High energy radiation in the $\mathrm{keV}$ range can be produced from highly relativistic electrons, for instance at XFELs and synchrotron sources using bending magnets and insertion devices, and equivalently in plasma accelerators through betatron radiation. Another approach is to sufficiently heat solid targets, for instance hohlraums \cite{PikeThesis} or burn-through foils \cite{Edwards1990_BURNTHROUGH}, with a laser.

Directed energetic gamma radiation can be generated by passing electrons through solid material to produce bremsstrahlung \cite{Glinec2005_Brems} or relying on inverse Compton scattering \cite{Sarri2014_ICS,Yan2017_ICS}.
\vspace{\baselineskip}
...
In this experiment, the North beam was temporally detuned to a pulse duration of $40\,\mathrm{ps}$ \textsc{fwhm} in order to effectively heat a solid target. The beam was focused by an f/2 off-axis parabola (OAP). After optimising the OAP a phase plate was inserted in the collimated beam which results in a smoothing of the spot and the spot is increased to an ellipse of size $75\,\mathrm{\mu m} \times 210\,\mathrm{\mu m}$, with a characteristic speckled profile \cite{Kato1984_PHASEPLATE}. The target is a $25\,\mathrm{\mu m}$ thick Kapton tape with etched holes reduced to $5\,\mathrm{\mu m}$ thickness. The thinned out parts of the tape are coated with $100\,\mathrm{nm}$ thick germanium (Ge) dots\footnote{Target and fabrication techniques were developed by the Target Fabrication Division at the Central Laser Facility, in particular Sam Astbury and Chris Spindloe.}. When the germanium is heated, it produces a plasma plume that faces the focusing optic and emits X-rays with distinct emission lines around $1.5\,\mathrm{keV}$(REF SIMULATED BY MORTON, using \cite{Roberts1980_AWECODE}). To reach the interaction region the X-rays have to pass through the tape which filters some of the low energy components. The motion of the tape drive is motorised\footnote{The tape drive was designed and built by Brendan Kettle (Imperial College).} and a fresh Ge target is provided on each shot. 
...
\caption{X-ray spectra. Top: X-ray spectra measured with the crystal spectrometer for different shot days. Bottom: X-ray spectrum simulated by Morton REF USING \cite{Roberts1980_AWECODE} (blue) and filtered by plastic (red).}
\end{figure}
