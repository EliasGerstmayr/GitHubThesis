...
\EliasComm{Maybe make structure: Characterisation of Electron Spectra for 2 regimes. Then commission gamma source, then characterise those electrons and then gamma source?}
...
\EliasComm{Do I need to provide examples of electron spectra for the high and low density shots?}
\begin{figure}
...
The key quantities of interest to characterise the electron beam in this context are charge, energy, divergence and beam pointing as well as the stability of these properties from shot to shot as well as their variation between the days.
...
\EliasComm{Check coherent story: bremsstrahlung will depend on electron beam and on converter material. We want to optimise our source according to the parameters useful yield and noise on the single-particle detectors. Explain how we define useful yield by showing the collimator. Then introduce the materials and the electron properties (need to have electron properties for high and low density).}
\EliasComm{Add brems equations to support those comments.}
The properties of the gamma-ray beam depend strongly on the properties of the electron beam in terms of energy, brightness, divergence and pulse duration, but is also shaped by the choice of the converter material (REF)\addref.
...
\end{figure}
\EliasComm{Discussion size and overlap in time and space?}
\EliasComm{GEANT sim of collimator?}
Despite its low divergence the collimated gamma beam will be around $6.2\pm 0.1\,\mathrm{mm}$ wide at the interaction point, $865\pm 4.5\,\mathrm{mm}$ downstream from the converter target. The X-ray source is much closer to the interaction point and will be smaller at around XX MM NUMBER\addnum.
...
\begin{itemize}
\item Electron spectra
\item Show simulation of spectra
\item show response does not change
\item show experimental response also does not change
\item use measured Gamma spec to show yield/show gamma profile and gamma spec yield are equivalent
...
\caption[Example electron spectra and corresponding in GEANT simulated gamma spectra.]{Left: Representative extreme examples of electron spectra from one day. Right: Resulting gamma spectrum based on GEANT simulations. Maybe show an average with shaded error bars instead. }
\end{figure}
...
\EliasComm{Exp data error bar, can not measure response. Show simulated response and experimental does not change for the extreme examples. Use electron spectrum to characterise things.}
...
\EliasComm{Replace this by now using Cary's raw data, wait for this... Error bars and content will change as a result.}
...
In order to produce and measure electron-positron pairs from the Breit-Wheeler process, we require two suitable photon sources and a detector system that is capable of measuring the pairs.



\begin{itemize}
\item Overall for the BW experiment we require two photon sources and a single-particle detector.
\item In order to measure BW pairs we need to be able to identify particles from BW from other noise and identify ideal conditions. Characterise noise and then find out how to measure excess.
\item We have shown the commissioning of the gamma-ray source and implied from Section XX that we have a working X-ray source
\item In Section XX implicitly showed that we have some single-particle detectors
\item we are using a magnetic chicane, TimePix3 and a CsI detector 
\item we mapped the magnetic field of the magnets (Dominik Hollatz) and measured its position accurately
\item we used a second pair of magnets to check the fields work as expected using the main electron beam
\item we also produced positrons from the Bethe Heitler process to measure positrons
\item we were able to use the TimePix in these datasets as tracking layers and were able to distinguish particles from radiation
\item we were able to distinguish high-energy leptons on the CsI detector
\item in summary, we commissioned two photon sources and single-particle detectors, so everything working in principle
\item we have three days of data taking where we are following an alignment procedure and attempt to measure the process
\item overall, things are more complicated than just the number of pairs, there is also a varying noise background of Bethe-Heitler pairs 
\item the alignment was the best on day A showing a difference in number of pairs (Bethe-Heitler plus?) on shots with two photon sources at once than with only one
\item analysis still in progress, so not more to say at this point but can consider improvements
\item very challenging
\end{itemize}

\subsubsection{Single-particle detectors}

Only produce few pairs, so need a way to separate the particles from noise.
A magnetic chicane and shielding at multiple points was used to effect that only particles with certain properties reach the detector plane.

First, other particle types.
If X-rays etc. still make it to the detector, we need to be able to distinguish particles from X-rays. The TimePix tracking layers are able to track particles and distinguish them from X-ray noise.

The CsI detector is through its thickness able to distinguish by shape a lepton from other particles.

Second,
there will be same type of particles from Bethe-Heitler or angles. Design chicane accordingly and reduce matter near-axis. Also, characterise noise. We need to be able to see and quantify when there is an excess of particles when we collide successfully.
