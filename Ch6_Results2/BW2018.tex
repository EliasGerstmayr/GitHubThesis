...
The X-ray source was diagnosed by a pinhole camera setup with two $25\,\mathrm{\mu m}$ tantalum, filtered with $5\,\mathrm{\mu m}$ aluminium and $6\,\mathrm{\mu m}$ PTFE, respectively, measuring the flux and the source size. A crystal spectrometer measured the spectrum of the X-rays in a spectral window spanning $\sim 700\,\mathrm{eV}$ from just under $1.3\,\mathrm{keV}$ to $2\,\mathrm{keV}$ including the emission lines around $1.5\,\mathrm{keV}$. Both cameras are back-illuminated deep-depletion in-vacuum X-ray CCD cameras (Andor DX420-BR-DD). Both measurements were conducted from the side of the target that was heated by the laser and are hence not spectrally filtered by the Kapton tape. 
...
Relate measured quantities to the cross section for the BW process (REF).
...
The number of pairs produced from BW scales something like this (REF)
...
If the full X-ray spectrum (for instance Morton simulations REF) are included then a large pool of X-rays are available at lower energies to be paired up with the high energy photons of 600-800 MeV. We see the effect of this in Figure XX showing the fluctuation of the number of expected pairs on the days based on different angles. The differences flatten out.
