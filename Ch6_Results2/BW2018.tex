...
In 2014, Pike et al. \cite{Pike2014_BW} proposed to measure the linear Breit-Wheeler process by combining a low-divergence ($\theta \sim ln(\gamma)/\gamma$) bremsstrahlung source, consisting of a GeV-class electron beam generated by a high-intensity laser and a gold foil, with a blackbody radiation field formed in a hohlraum which is heated by a high-energy laser. For $1\times 10^9$ electrons at 2 GeV, a 5 mm thick gold foil and a 1 cm hohlraum at 400 eV, the researchers predicted up to $1\times10^5$ pairs to be generated from the linear Breit-Wheeler process. The asymmetry of the energies produced by the two photon sources results in an emission of pairs in the direction of the higher energetic gamma ray beam. This is referred to as pair beaming \cite{Ribeyre2018_BW}, which in principal allows the extraction and measurement of all pairs produced, better separation of signal and noise, and using smaller detectors.
...
The experiment presented here is based on this proposed layout \cite{Pike2014_BW,PikeThesis}: two lasers are employed to provide simultaneously an X-ray and a gamma-ray source to overcome the mass threshold and to produce electron-positron pairs from the linear Breit-Wheeler process. The gamma rays are produced, as proposed, by bremsstrahlung from LWFA electrons \cite{Glinec2005_Brems}, but in this instance typically at electrons energies $\epsilon_e < 1\,\mathrm{GeV}$. As converter the higher-Z metal bismuth (Bi) is used which allows reducing the material thickness with respect to the proposed gold foil at comparable conversion efficiency. Instead of using a high-energy laser and a hohlraum, a replenishable burn-through foil germanium target in conjunction with another high-intensity laser is used, producing an X-ray spectrum with emission lines peaked around $1.5\,\mathrm{keV}$. This balances out the lower gamma-ray energy and a narrow bandwidth spectrum constrains the interaction conditions.
...
By removing the X-ray target from the beam axis we also reduce the production of Bethe-Heitler pairs of gamma rays interaction with solids.

\section{Chapter Outline}

This Chapter relates to an experimental campaign aimed at measuring the linear Breit-Wheeler process conducted at the Astra Gemini laser system in Spring 2018. Two high-intensity lasers are used to provide provide a highly energetic gamma radiation and an X-ray source that combined exceed the production threshold of a centre-of-mass energy of $\sqrt{s} = 1$.
...
The X-ray data has been analysed by Cary Colgan (Imperial College). Data related to the single particle detectors has been analysed by Brendan Kettle (Imperial College) and Guillermo Marrero Samarin (QUB) with significant contribution from Dominik Hollatz (Jena) for the magnetic chicane.
\vspace{\baselineskip}

In this experiment, the gamma-ray source is produced by bremsstrahlung from LWFA electrons traversing a solid target.
The properties of electrons from LWFA determine the properties of the radiation produced, but LWFA electrons underlie intrinsic fluctuations and properties vary from experiment to experiment. Hence, the electron properties are first characterised in general and more specifically its variation in performance for three shot days. The properties of interest are charge, electron energy, divergence and pointing stability.

Secondly, the electron beam is used to produce radiation from bremsstrahlung. The properties of radiation that the electron beam can produce are investigated for two different density regimes of the accelerator and two converter materials at different thicknesses. The performance of the source in its different configurations is scrutinised and optimised in terms of `useful' yield and signal-to-noise ratio.

Based on the conclusions of the bremsstrahlung survey and its optimisation, the gamma source is characterised for the three shot days. The characterisation of the electron spectra for these shot days are used to validate the response of the gamma spectrometer.

Then, the measured spectra of the X-ray source, analysed by C. Colgan (Imperial College) and its performance for the shot days are presented.

Combining the characterisation of the gamma-ray source and the X-ray source the impact of the varying performance on the total Breit-Wheeler cross section for the three shot days is analysed and discussed.

Then, briefly the overall status of the analysis is summarised to give context.

Finally, as conclusion the results of this analysis are used to suggest improvements for a future measurement.

\section{Experimental Setup}

The experiment was performed at the dual 300 TW Ti:Sa Gemini laser at the Central Laser Facility, UK, in early 2018.
The aim of this setup is to produce and to measure electron-positron pairs from the linear Breit-Wheeler process. For this purpose, high-energy bremsstrahlung gamma-rays produced by LWFA electrons were collided with X-rays from a heated burn-through foil. The South arm of the Gemini laser provided the gamma source, the North beam the X-rays.
A sketch of the experimental setup is shown in Figure \ref{BW:fig:exp_sketch}.

\SMComm{Use past tense.}

\begin{sidewaysfigure}
\centering
\includegraphics[width=1.0\columnwidth]{BW2018_render_V4_annotated.png}%{BW2018_render_V3_annotated3.png}
\caption{Sketch of the experimental setup indicating its main components. From left to right: The first laser pulse (left, red) is focused by an f/40 OAP into a gas cell target (blue), producing an electron beam (blue) via LWFA leaving the cell on the right side. The remainders of the laser pulse are removed after the cell by a kapton tape acting as plasma mirror (orange). The electron beam passes through the tape traverses a metal converter target producing gamma radiation from bremsstrahlung (green) in forward-direction. A tungsten (W) collimator removes the divergent components of the radiation and a thick W block shields the direct line of sight to the X-ray target further downstream. When the converter (and the collimator) are not in the path of the electron beam, it is characterised by a magnetic spectrometer using a permanent dipole magnet. A second laser (red) is focused onto a kapton tape with germanium (Ge) dots (orange) act as source of X-rays (green) that are characterised by a crystal spectrometer and a pinhole camera (turquoise). Positrons produced in the interaction of the X-rays and gamma-rays propagate downstream and are directed through a magnetic chicane and onto single particle detectors (brown). The gamma-rays are measured by a gamma profile detector and a gamma spectrometer (gold).}
\label{BW:fig:exp_sketch}
\end{sidewaysfigure}

\subsubsection{North beam and X-ray source}

In this experiment, the North beam was temporally detuned to a pulse duration of $40\,\mathrm{ps}$ \textsc{fwhm} in order to effectively heat a solid target. The beam was focused by an f/2 off-axis parabola (OAP) with a phase plate (REF) in the collimated beam that smooths and also increases the size of the focal spot to an ellipse of about $75\,\mathrm{\mu m} \times 210\,\mathrm{\mu m}$. The target is a $25\,\mathrm{\mu m}$ thick kapton tape with etched holes reduced to $5\,\mathrm{\mu m}$ thickness. The thinned out parts of the tape are coated with $100\,\mathrm{nm}$ thick germanium (Ge) dots\footnote{Target and fabrication techniques were developed by the Target Fabrication Division at the Central Laser Facility, in particular Sam Astbury and Chris Spindloe.}. When the germanium is heated, it produces a plasma plume that faces the focusing optic and emits X-rays with distinct emission lines around $1.5\,\mathrm{keV}$(REF). To reach the interaction region the X-rays have to pass through the tape which filters some of the low energy components. The motion of the tape drive is motorised\footnote{The tape drive was designed and built by Brendan Kettle (Imperial College).} and a fresh Ge target is provided on each shot. 
\SMComm{Focal spot size North?}

The X-ray source is diagnosed by a pinhole camera measuring the flux and the source size. A crystal spectrometer measures the spectrum of the X-rays that do not pass through the tape in a $\sim 700\,\mathrm{eV}$ window from just under $1.3\,\mathrm{keV}$ to $2\,\mathrm{keV}$ including the emission lines around $1.5\,\mathrm{keV}$. Both cameras are back-illuminated deep-depletion X-ray in-vacuum CCD cameras (Andor DX420-BR-DD).
...
\centering
\includegraphics[trim={2cm 0 5cm 0}, clip, width=.9\columnwidth]{2018QED_ElecSpecs_average_V2.png}
\caption{Average spectra for the relevant shot days (A, B and C). The spectra are normalised such that the total charge is 1 for each spectrum. The shaded areas indicate 1 standard deviation from the average spectrum.}
...
... large energy spread as typical for ionisation injection at Gemini (REF), no quasi-monoenergetic features (REF)
...
Material & Z & Density [g/cc] & Radiation Length [mm] & Thickness [mm] & Thickness [X0] \\ \hline \hline
...
\caption{Montage of gamma profile images for different materials and accelerator configurations. The images on the top row are on the same colour scale, the images on the bottom respectively as well (with a maximum value of 1/4).}
...
\caption{Total measured gamma yield of the gamma-ray beam at two accelerator configurations (low and high electron densities) for tungsten (W, red) and bismuth (Bi, yellow). The values for the thin and the thick targets are shown by the thin and thick bars, respectively.}
\end{figure}
...
\caption{FWHM divergence of the gamma-ray beam at two accelerator configurations (low and high electron densities) for tungsten (W, red) and bismuth (Bi, yellow). The values for the thin and the thick targets are shown by the thin and thick bars, respectively.}
\end{figure}
...
\caption{Pointing of the gamma-ray beam at two accelerator configurations (low and high electron densities) for tungsten (W, red) and bismuth (Bi, yellow). The values for the thin and the thick targets are shown by the thin and thick bars, respectively.}
\label{BW:figs:converter_pointing}
\end{figure}
...
\caption{Estimated transmitted yield through the collimator assuming a field of view of 7.14 mrad at two accelerator configurations (low and high electron densities) for tungsten (W, red) and bismuth (Bi, yellow). The values for the thin and the thick targets are shown by the thin and thick bars, respectively.}
...
\begin{figure}
\centering
\includegraphics[trim={4.9cm 0 5cm 0}, clip, width=.9\columnwidth]{2018QED_Converter_YieldvNoise_V2.png}
...
\begin{figure}
\centering
\includegraphics[trim={4.5cm 0 5cm 0}, clip, width=.9\columnwidth]{2018QED_YieldvNoise_CollIN_V2.png}
...
\caption{Total measured gamma yield at two accelerator configurations (low and high electron densities) for tungsten (W, red) and bismuth (Bi, yellow). In turquoise the gamma yield measured using the thick bismuth target and the collimator in the beam path. The values for the thin and the thick targets are shown by the thin and thick bars, respectively.}
...
\caption{Fitted gradient for Noise/Yield from Figure XX at two accelerator configurations (low and high electron densities) for tungsten (W, red) and bismuth (Bi, yellow). In turquoise the gradient fitted for the data using the thick bismuth target and the collimator in the beam path.}
...
\begin{figure}
\centering
\includegraphics[trim={4.9cm 0 5cm 0}, clip, width=.9\columnwidth]{2018QED_GSGP_Corr.png}
\caption{Gamma profile and spectrometer yield correlation for an example run.}
\end{figure}
...
\includegraphics[width = 0.9\columnwidth]{2018QED_GEANT_AvGSpec_V2.png}
...
\EliasComm{Absolute numbers for photons? Absolute calibration for yield?}
...
\caption{X-ray spectra. Top: X-ray spectra measured with the crystal spectrometer for different shot days. Bottom: X-ray spectrum simulated by Morton REF (blue) and filtered by plastic (red).}
\end{figure}

\begin{figure}
\centering
\includegraphics[trim={4.9cm 0 5cm 0}, clip, width=.9\columnwidth]{BW_XraySpec_AvDay_Yield.png}
\caption{Total X-ray yield as measured by the crystal spectrometer for the shot days.}
\end{figure}
...
\begin{equation}
N_{BW, day}|_{theta} \sim \int \frac{N_{ph, X}}{\mathrm{d}E_{X}} \frac{N_{ph, \gamma}}{\mathrm{d}E_{\gamma}} \sigma_{BW} (E_{X}, E_{\gamma}, \theta) \mathrm{d}E_{\gamma}\mathrm{d}E_{X}
\end{equation}
