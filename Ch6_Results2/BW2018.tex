...
The aim of this setup was to produce and to measure electron-positron pairs from the linear Breit-Wheeler process. For this purpose, high-energy bremsstrahlung gamma-rays produced by LWFA electrons were collided with X-rays from a heated burn-through foil. The South arm of the Gemini laser provided the gamma source, the North beam the X-rays.
...
In this experiment, the North beam was temporally detuned to a pulse duration of $40\,\mathrm{ps}$ \textsc{fwhm} in order to effectively heat a solid target. The beam was focused by an f/2 off-axis parabola (OAP). After optimising the OAP a phase plate was inserted in the collimated beam which results in a smoothing of the spot and the spot is increased to an ellipse of size $75\,\mathrm{\mu m} \times 210\,\mathrm{\mu m}$, with a characteristic speckled profile (REF KATO 1984 PRL, PEPLAR 2005 PHASE PLATE). The target is a $25\,\mathrm{\mu m}$ thick kapton tape with etched holes reduced to $5\,\mathrm{\mu m}$ thickness. The thinned out parts of the tape are coated with $100\,\mathrm{nm}$ thick germanium (Ge) dots\footnote{Target and fabrication techniques were developed by the Target Fabrication Division at the Central Laser Facility, in particular Sam Astbury and Chris Spindloe.}. When the germanium is heated, it produces a plasma plume that faces the focusing optic and emits X-rays with distinct emission lines around $1.5\,\mathrm{keV}$(REF). To reach the interaction region the X-rays have to pass through the tape which filters some of the low energy components. The motion of the tape drive is motorised\footnote{The tape drive was designed and built by Brendan Kettle (Imperial College).} and a fresh Ge target is provided on each shot. 

The X-ray source was diagnosed by a double pinhole camera, filtered with XX and XXX, respectively, measuring the flux and the source size. A crystal spectrometer measured the spectrum of the X-rays in a $\sim 700\,\mathrm{eV}$ window from just under $1.3\,\mathrm{keV}$ to $2\,\mathrm{keV}$ including the emission lines around $1.5\,\mathrm{keV}$. Both cameras are back-illuminated deep-depletion X-ray in-vacuum CCD cameras (Andor DX420-BR-DD). Both measurements were conducted from the side of the target that was heated by the laser. The radiation propagating to the interaction point on the laser axis traverses the kapton tape and is spectrally filtered.
...
\caption{Example of an electron spectrum measured on the experiment.}
...
\caption[Average electron spectra for the shot days.]{Average spectra for the relevant shot days (A, B and C). The spectra are normalised such that the total charge is 1 for each spectrum. The shaded areas indicate 1 standard deviation from the average spectrum.}
...
\caption[Average electron charge, energy, divergence and pointing measured on the relevant shot days.]{Average electron charge, energy, divergence and pointing measured on the relevant shot days. The error bars indicate 1 standard deviation from the average value.}
...
\end{tabular}
\caption{Overview of electron beam properties for the relevant shot days. The maximum energy $E_{max}$ is defined as the energy when the spectral intensity falls to 10 percent of its peak value. $\theta$ is the \textsc{fwhm} divergence angle and $\sigma_X$ is the beam pointing fluctuation in the horizontal, non-dispersion axis.}
...
\begin{equation}
Bremstrahlung Energy \sim Z^2 N(E) E^2
\end{equation}
\begin{figure}
\centering
...
\begin{figure}
\centering
\includegraphics[trim={3.8cm 0 5cm 0}, clip, width=.9\columnwidth]{2018QED_YieldvNoise_CollIN_V2.png}
...
\EliasComm{Absolute numbers for photons? Absolute calibration for yield? Not possible for the spectrometer. Only for profile screen.}
