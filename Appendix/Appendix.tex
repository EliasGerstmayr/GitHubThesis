...
In LWFA betatron radiation typically reaches the soft X-ray regime of a few to tens of keV\addref.


\chapter{Radiation Length}
\label{Appendix:Sec:RadiationLength}

The radiation length, $X_0$, is a material-specific quantity that indicates how much energy a high-energy particle converts into radiation and secondary particles. The energy of a particle passing through a material of thickness $x$ and radiation length $X_0$ loses energy at a rate \cite{Jackson}:
\begin{equation}
...
The emission/scatter rates for the $n$-th harmonic depend on the laser polarisation \cite{Ritus1985_QRR,BlackburnThesis}. The scattering rate (spin-averaged and unpolarised states in/out) for $n$ scattered photons for circular polarisation is given by
\begin{equation}
...
\end{equation}
The frequency of the incoming photons is $\omega$ and the scattered photon carries an energy $\omega' = xE$, i.e $x$ is the fraction of energy the photon carries away. $a_0$ is the normalised vector potential of the laser pulse.
...
Alternatively, in the high-field limit the rates can be related to the (quantum) synchrotron function, such that the differential probability rate is given by \cite{BlackburnThesis}
\begin{equation}
