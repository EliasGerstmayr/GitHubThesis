...
$\mathcal{E},\epsilon$ & Energy, e.g. $\epsilon_\gamma$ gamma energy\\
...
\end{minipage}
\caption[Overview of symbols.]{Overview of symbols used in this work.}
\end{table}
...
PWFA & Plasma wakefield accleration\\
...
\caption[List of abbreviations.]{List of abbreviations used in this work.}
\end{table}
\clearpage
...
\caption[List of notations.]{List of notations used in this work.}
\end{table}
...
\caption{Summary of relevant fundamental constants.}
\end{table}

Most of the thesis uses SI units. In a few exceptions, natural units ($\hbar = c = \epsilon_0 = 1$) are used, which is then explicitly indicated.
...
with the betatron frequency, $\omega_\beta$, which is related to the plasma frequency, $\omega_p$, by $\omega_\beta = \omega_p/\sqrt{2\gamma}$. The plasma frequency is derived explicitly in Section \ref{Chap:Theory:Sec:PlasmaFreq}.
...
\section[Elec. in a cylindrically symmetric E field with axial field]{Electron in a cylindrically symmetric electric field with axial field}
...
\begin{equation}
\omega_c = \frac{3}{2} \gamma^3 \frac{c}{r_L},
\end{equation}
where $r_L$ is the bending radius, e.g. the Larmor radius.
The doubly differential spectral intensity reads
\begin{equation}
\frac{d^2 I}{d\epsilon d\Omega} = \frac{3e^2}{16\pi^3 \hbar c \epsilon_0} \gamma^2 \frac{\epsilon^2}{\epsilon_c^2} \left(1+\gamma^2 \theta^2\right)^2 [K_{2/3}^2(\psi) + \frac{\gamma^2 \theta^2}{1+\gamma^2 \theta^2} K_{1/3}^2 (\psi)],
\end{equation}
with
\begin{equation}
\psi = \frac{\epsilon}{2\epsilon_c}(1+\gamma^2 \theta^2)^{3/2}
\end{equation}
...
A set of alternating magnets can be used to harvest and direct the radiation of an accelerated relativistic electron more efficiently. The electron oscillates and emits radiation preferentially at the turning points as the acceleration is the strongest there. The radiation is emitted in a forwards-pointing cone with a diameter proportional to $K/\gamma$ and $1/\gamma$ \cite{Jackson} in and out of plane of the oscillation, respectively.

The rapid change of direction in the field and hence forced trajectory of the electron in the transverse results in a change of effective Lorentz factor in the forward direction.
...
Unsurprisingly, the equations describing both processes are very similar.
...
\end{equation}
where $k_\beta = \omega_\beta / c$ is the wavenumber and $r_\beta$ is the betatron radius, the amplitude of the oscillations. 
If $K \gg 1$, it is called the undulator parameter.

The generated radiation follows the on-axis synchrotron radiation with a critical energy
\begin{equation}
\boxed{\epsilon_c = \frac{3}{4} \hbar \gamma^2 \omega^2_p r_\beta / c,}
\end{equation}
...
The radiation length, $X_0$, is a material-specific quantity that indicates how much energy a high-energy particle converts into radiation and secondary particles when passing through a material. A particle passing through a material of thickness $x$ and radiation length $X_0$ loses energy at a rate \cite{Jackson}:
\begin{equation}
\frac{\dif \epsilon}{\dif x} = -\frac{\epsilon}{X_0},
\end{equation}
with $\epsilon(x) = \epsilon_0 e^{-x/X_0}$ and $X_0$ is given by \cite{Tsai1974_PairsBrems,Tanabashi2018_PDGReview}:
\begin{equation}
...
\end{equation}
where $\hbar \omega$ is the energy of the emitted photon, $Z$ is the atomic number and $A$ the atomic weight and $N_A$ the Avogadro number. $y= \hbar\omega/\epsilon$, i.e. the fraction of the emitted photon energy relative to the initial electron energy.


\chapter{Nonlinear Compton Scattering}
...
\EliasComm{Needs thinking what to include.}
Photon rules:
...
\section{Derivation of the Klein-Nishina equation}
...
%\section{Nonlinear Compton scattering cross section}
...
\begin{equation}
\frac{\dif W_n}{\dif x} = \frac{2 \alpha m^2}{\pi \epsilon}\int^{\pi/2}_0 \left[-A^2_0 + a^2_0 \left(1-x+ \frac{1}{1-x}\right) (A^2_1 - A_0 A_2)\right]\dif \varphi.
\end{equation}
The energy of the incoming photons is $\omega$ and the scattered photon carries an energy $\omega' = x\epsilon$, i.e $x$ is the fraction of energy the photon carries away. $a_0$ is the normalised vector potential of the laser pulse.
$J_n$ is shorthand for $J_n(z)$, and $A_i = A_i(n,a,b)$, with
\begin{align}
z &= \frac{2 a_0}{\sqrt{1+a^2_0}}\frac{\sqrt{x[nu - (1+nu)x]}}{u(1-x)},\\
u &= \frac{2k \cdot q}{q^2} = \frac{4 \epsilon \omega}{m^2 (1+a^2_0)}.
\end{align}
...
The quantum synchrotron function, $F$, is given as a function of the two invariants, $\eta$ and $\chi$, by \cite{BlackburnThesis}
\begin{equation}
...
\end{equation}
where $\chi = \gamma b, \eta = \hbar \omega b/2m_e c^2$ are quantum parameters with $b = |\mathbf{E}_\perp + \mathbf{v}\times \mathbf{B}|/E_S$, and $E_S$ is the Schwinger field. $K_{2,3}, K_{1/3}$ are Bessel functions of the second kind, and $\delta$ is given by
\begin{equation}
...
The emission power in terms of $\chi$ for an electron of energy $\epsilon$ reads
\begin{equation}
\frac{\dif P}{\dif \chi} = \frac{2 \epsilon \chi}{\eta} \frac{\dif W_\gamma}{\dif \chi} = \frac{\sqrt{3\alpha}}{\pi} \frac{m c^2}{\tau_c} F(\eta, \chi).
\end{equation}
The total emission power of the classical synchrotron emission and the quantum description are related by the \textit{Gaunt factor}, $g(\eta)$:
\begin{equation}
P = \frac{2 \alpha}{3} \frac{mc^2}{\tau_c} \eta^2 g(\eta),
\end{equation}
were $\tau_C = \hbar/mc^2 = 1.29\times10^{-21}\,\mathrm{s}$ is the Compton time and $g(\eta)$ is defined as
\begin{equation}
...
\end{cases}
\end{equation}
or by an analytic approximation that holds over the entire parameter space to within few percent \cite{Baier1991_GAUNT}
\begin{equation}
...
The Landau-Lifschitz radiation reaction force, $\mathbf{F}_{LL}$, can be written in terms of 3-vectors as follows \cite{Bulanov2011_LADLL}:
\begin{align*}
\mathbf{F_{LL}} = \frac{2e^3}{3m_e c^3 \sqrt{1-\frac{v^2}{c^2}}} \left\lbrace \left( \frac{\partial}{\partial t} + (\mathbf{v} \cdot \nabla) \right) \mathbf{E} + \frac{1}{c} \left[ \mathbf{v} \times \left( \frac{\partial}{\partial t} + (\mathbf{v} \cdot \nabla) \right) \mathbf{B} \right] \right\rbrace +\\
...
\end{align*}
where $c$ is the speed of light, $e$ is the electron charge, $m_e$ its mass, $\mathbf{v}$ its velocity and $v = |\mathbf{v}|$. $\mathbf{E}$ and $\mathbf{B}$ are the electric and magnetic field, respectively.
\clearpage

\section{Stochastic Operator}
\label{Appendix:Sec:RR:StochasticOperator}

The stochastic operator, $S$, in Section \ref{Theory:Sec:Signatures} is given by \cite{Ridgers2017_QRR}
\begin{equation}
S(\eta) = \frac{55 \alpha c}{24 \sqrt{3} \bar{\lambda}_c} m^2_e c^4 \eta^4 g_2(\eta),
\end{equation}
where $\alpha$ is the fine-structure constant, $c$ the speed of light, $m_e$ the electron mass and $\bar{\lambda}_c$ is the reduced Compton wavelength. $b = |\mathbf{E}_\perp + \mathbf{v}\times \mathbf{B}|/E_S$, i.e. the relative magnitude of the Lorentz force, and $E_S$ is the Schwinger field. $\eta$ is the quantum nonlinearity parameter. $g_2(\eta)$ is similarly defined as the Gaunt factor, $g$, (see Appendix \ref{Appendix:Sec:QuantumSynchrotron}) in terms of the quantum synchrotron function, $F$, and the classical synchrotron function, $F_{cl}$:
\begin{equation}
...
For an electron beam with a Gaussian energy distribution colliding with a plane wave in the limits $\eta \ll 1$ and $\sigma \ll \langle \gamma \rangle$, i.e. the width is much smaller than the mean of the Gaussian, the evolution of the variance can be written as:
\begin{equation}
\left(\frac{\mathrm{d}\sigma^2}{\mathrm{d}t}\right)_{st} \approx \frac{\alpha  b^2}{\bar{\lambda}_c}\left(\frac{55b}{24\sqrt{3}}\left\langle\gamma\right\rangle^4 - \frac{8}{3}\sigma^2 \left\langle\gamma\right\rangle\right).
\end{equation}
...
Alternatively, in the high-field limit the rates can be related to the (quantum) synchrotron function (App. \ref{Appendix:Sec:QuantumSynchrotron}), such that the differential probability rate is given by \cite{BlackburnThesis}
\begin{equation}
...
The differential cross section of the Bethe-Heitler pair production process is given by crossing symmetry from bremsstrahlung (compare with Appendix \ref{Appendix:Brems_Born}) \cite{Ruffini2010_PAIRSASTRO}:
\begin{align}
...
where $r_e$ denotes the electron radius, $u_\pm$ the momentum of the positron and electron, respectively, and $\alpha$ is the fine-structure constant. $\gamma_\pm$ is the energy of the electron and positron, and $c$ is the speed of light.
\vspace{\baselineskip}

Similarly as for bremsstrahlung (see Appendix \ref{Appendix:Sec:Brems_RadiationLength}), the differential cross section can in the ultrarelativistic regime be approximated by a simplified equation in terms of the radiation length, $X_0$ (see Appendix \ref{Appendix:Sec:RadiationLength}) \cite{Tsai1974_PairsBrems,Tanabashi2018_PDGReview}:
\begin{equation}
\frac{\dif \sigma_{\gamma Z}}{\dif x} = \frac{A}{X_0 N_A} \left[ 1 - \frac{4}{3}x(1-x)\right],
\end{equation}
with $x = \epsilon/k$, where $k$ is the incident photon energy and $\epsilon$ the energy of the produced electron or positron. $A$ is the atomic weight and $N_A$ the Avogadro constant.
