...
Some flaw in alignment (Note in lab book stating prism cam was misplaced, changed angle in prism, camera position and maybe did not follow procedure to the end).

The following allows re-establish the conditions and finding the overlap in any case (similar as used for aligning gas cell targets).

In laser wire techniques the electron beam is fixed in space (due to imaging setups) and the laser is moved to establish the size of the electron beam.

Here we moved the laser beam to establish the size of the laser beam at the interaction to find the relative position (centre) to then move onto non-linear ICS and radiation reaction studies. The overlap is easier with large beams and at shallow angles (high f-numbers).

We used the transverse diagnostics (shadowgraphy and top view) to monitor the general overlap and drifts.

For a fixed relative timing position, we translate the beam using the OAP translation in 50 micron steps. We take 5 shots at each position. If two positions in a row show no collisions (identified by integrating over the gamma profile image, in experiment visual confirmation) we accept this is the end of the beam.

See MONTAGE.

We see that the extent of the beam is of order 400 microns plus minus 50 microns, the step size. This could be refined but since we were aiming for high-intensity interactions we did not do this.

At 400 microns diameter the hole in the beam is of order 50 microns. Is this visible? Could also be misaligned in the other axis.

Ideally would do a scan in X- and Y to establish the centre of the beam (map out the hole). Then move a step in time and repeat the procedure. This is a method that requires high repetition rate systems but especially at late nights this method is a visual confirmation and is better than a blind raster scan and the binary mode. Also know conditions much better.

After moving a step in time confirm whether the beam is larger or smaller, this then should get you straight away to the right position. Ideally a high magnification shadowgraphy could help with the alignment (short pulse duration or chirped probe?).

Unfortunately, in this experiment a lens in the beam expander upstream in the laser chain of the scattering beam broke and this procedure was not completed.

In addition, we have to consider that especially low f-number OAPs as this the offset potentially introduces astigmatism and the spot has to be optimised if the position deviates much from the optimised position.

This allows us to estimate the size of the beam, somewhat inverse laser wire where we scan the laser which is smaller than the electron beam to establish its size.
