...
Using the simulated result as a lookup-table for the energy deposition and relating the photon energy to the electron energy, the total energy deposition for a given electron spectrum can be calculated as follows:
\begin{equation}
...
\section{Photon numbers to estimate beam size}

We can use the simulated response of the detector to calibrate the gamma profile screen with a radioactive source. Here we used caesium-137 which predominantly emits gamma-rays at $661.7\,\mathrm{keV}$ photon energy, for which the GEANT simulation estimates an energy deposition of $\sim 90\,\mathrm{keV}$ per photon. By estimating the energy deposited on-shot with the photon numbers estimated in Equation \eqref{linICS:Eqs:Edep_YIELD}, we can compare the on-shot number of photons measured with the estimate provided by Equation \eqref{Chap:linICS:Eqn:NX_UsefulUnits}.
