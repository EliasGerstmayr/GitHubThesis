...
We followed an alignment procedure to overlap the two laser beams in space and time using a photodiode and spatial interferometry in a low power mode. The scattering beam is focusing into the plasma and is visible on the transverse (horizontal plane) shadowgraphy diagnostic (see Figure \ref{linICS:Figs:ShadowgraphyRaster}, left side) also showing the plasma channel from the wakefield driver beam and the shock leading to the injection of electrons into the wakefield accelerator. Another camera is used to image the self-emission of the plasma from the vertical plane (top view).
...
To establish the transverse size of the scattering laser pulse at the interaction point, we translated the beam in one transverse axis by translating the position of the $f/2$ OAP while keeping the parameters for the wakefield driver beam fixed. At each OAP position we took 5 full-power shots with both laser beams firing and an electron beam being measured. More shots were taken if one of the lasers failed to shoot or no electron beam was measured. The response of the gamma profile screen was recorded and the number of visible bright signals was noted down. The OAP was then translated by 50 microns and the procedure was repeated. In Figure \ref{linICS:Figs:GammaProfileRaster} a montage of the gamma profile screens at each position of a horizontal scan in the OAP position is shown. The relative position from the centre is shown on the x-axis. If there were no bright signals observed on the gamma profile screen for two consecutive OAP positions, this was deemed to be the end of the beam and defined the total extent of the laser beam with an error including the step size of the position scan and the fluctuation of the relative pointing.
\begin{figure}
\centering
\includegraphics[trim={8cm 2.5cm 6cm 2.5cm}, clip, width=1.0\columnwidth]{linICS_RasterScan_GammaProfile_Montage.png}
\caption{Montage of raster scan Gamma Profile.}
\label{linICS:Figs:GammaProfileRaster}
\end{figure}



We assume that we can position the centre of the scattering beam onto the centroid of the electron beam position by moving the OAP to the central position between the outermost OAP positions with measured signal on-shot. We can then repeat the procedure in the second transverse dimension to make sure that the electron beam and the scattering laser are centred onto each other in two dimensions. To increase the accuracy of this procedure we could repeat a scan in the first transverse dimension afterwards, which follows a similar procedure as the alignment procedures used by the group to align the pinhole in a gas cell target to the wakefield driver beam. This `raster scan' also follows a similar idea as the `laser wire' where a scattering beam is focused onto an electron beam that is larger than the focus at the plane of the interaction. By translating the laser in the a transverse axis and by measuring an X-ray signal from inverse Compton scattering or the absence of it, the size of the electron beam at the plane along with other beam properties can be determined. In this case, the electron beam is much smaller than the laser beam. Even though we translate the laser pulse here as well, the electron beam is now the probe and reveals the size of the laser beam at the interaction plane.

In the raster scan presented in Figure \ref{linICS:Figs:GammaProfileRaster} the interaction region and hence, following our argument, the combined size of the laser beam and the electron beam along with variations in the relative beam pointing is $400\pm50 \,\mu m$. At a on-target energy of XX this amounts to an $a_0$ and $a_0$ without the hole. For a $f/2$ geometry this also indicates that the interaction occurs at XX distance from the focal plane. Since we originally placed the interaction of both laser pulses by spatial interferometry at the edge of the gas jet (left hand side in Figure \ref{linICS:Figs:ShadowgraphyRaster} we assume that we are in a plane before the focal plane. In either case the interaction occurs in the plasma, so we can assume that the electron beam is only $\sim \mu m$ in size and pointing variations are mainly due to betatron oscillations $r_\beta \sim \mu m$ and global fluctuations in the laser pointing. Since the $f/2$ beam is very stable in terms of beam pointing, the raster scan mainly reflects the dimensions of the scattering beam.

The gas jet expands from its nozzle opening diameter of 15 mm to a length of approximately 17.3 mm (measured by the length of the plasma channel on the shadowgraphy). This means based on this scan the interaction point is defined as intended by the alignment procedure, the inner edge of the gas nozzle, but the gas jet has expanded. In addition, the focal plane of the laser has changed. We have to keep in mind that translating the OAP in longitudinal direction does not change the intensity of the interaction at first order as we shift the interaction plane along with the intensity plane when doing so. If we want to change the intensity at the interaction we have to change the relative delay of the two laser pulses.
...
\caption{Raster scan idea.}
\label{linICS:Figs:RasterTemporalTranslationIdea}
\end{figure}

By identifying the centre of the beam and its relative position in the focusing beam we can translate the relative timing such that we move the interaction from this plane to a different plane, for instance close to the focus by the estimated distance to the focal plane. The relative timing of the beams is here changed by removing or adding delay in a double-pass delay stage in the laser area (SAD). This idea is shown in Figure \ref{linICS:Figs:RasterTemporalTranslationIdea}. By repeating the raster scan procedure we can establish if we moved the relative timing in the right direction or if we are on the wrong side of the focal plane. The consistency of interactions is also an indicator of the size of the beam.

Unfortunately, at this point the final step of overlapping the interaction region with a region of high-intensity in the laser pulse a lens in the beam expander telescope of the scattering beam broke and we were not able to continue the campaign with dual-beam shots and collisions.

Nonetheless, this technique has worked very well in this scenario and we were able to show consistent overlap in space and time and a good control over the timing and spatial translations. Based on this raster scan we were also able to determine the intensity of the laser pulse at the interaction.
...
\begin{equation}
E_{X} = E_{ph}\frac{2(1-\cos \theta)\gamma^2}{1+a^2_0/2 + \gamma^2 \theta^2_0} = \frac{4\gamma^2}{1+a^2_0/2 + \gamma^2 \theta^2_0},
\label{linICS:eqns:full_linICS}
\end{equation}
where $\theta$ the angle between the electron and the incoming photon, $a_0$ is the normalised vector potential and $\theta_0$ is XX.
...
\end{equation}
which indicates that radiation reaction effects are negligible and the interaction with the laser pulse does not perturb the electron beam significantly \cite{Albert2016_APP}.
...
\caption[Example of an electron spectrum with corresponding calculated ICS spectrum.]{Example of an electron spectrum. The to the electron energies corresponding gamma-ray energies are indicated in a second x-axis on the top. CHANGE ENERGY SCALE ON TOP AND ADD A SECOND ELECTRON BEAM WITH WIDER ENERGY SPREAD.}
...
In this case the number of photons scattered in an interaction or the number of produced gamma-rays, $N_X$, is simply given by the Thomson scattering cross-section \cite{Albert2016_APP} REF
\begin{equation}
...
However, considering the experimental parameters it becomes clear that a linear Compton source can be achieved in a much more economical way by scaling down the energy and the beam size at the interaction whilst still preserving a permanent overlap within the shot-to-shot fluctuations of the laser and the accelerator. We can achieve the same number of scattered photons by keeping the photon density fixed. Moving from a beam radius of $200$ to $25\,\mathrm{\mu m}$, a factor of 8, the corresponding energy in the scattering beam could be reduced by a factor of 64, so around $150\,\mathrm{mJ}$. Using an $f/2$ optic as before, this plane would be more than 10 Rayleigh lengths, $z_R$, away from the focal plane and we might still be in the near-field of the laser, where the profile is flat and smooth, such that any change in response reflects the electron beam property. At the same time this keeps the beam large enough to interact at all times with the micron sized electron beam also considering relative pointing fluctuations. 
...
We calibrate the yield of the detector with a radioactive source using caesium 137 (Cs137). Based on the Equation above and this calibration we estimate $2.6 \pm 1.4 \times 10^{7}$ photons above 1 MeV for electrons above 220 MeV (what energy range, which spectrum XX WHICH SHOT).
...
... constant or consistent overlap 

... no radiation reaction

... non-invasive

... this makes it a suitable diagnostic of the beam
...
For the same dataset this gives us an electron divergence of $2.2 \pm 1.4\,\mathrm{mrad}$ in the dispersion direction. In comparison it is $2.2 \pm 0.6\,\mathrm{mrad}$ in the other axis, which indicates that the electron beam is on average symmetrical in terms of divergence (ellipticity of 1). 
...
\caption[]{Ellipticity (X over Y). Only use gamma profile.}
\end{figure}
...
The FWHM spatial features are well resolved and the spatial calibration is likely to be right. Somehow in general the pointing seems to be non-correlated. This is odd as the FWHM matches and indicates that we interact with the entire beam. The lack of correlation does not change by weighting the detector response by the energy deposition as described before.
...
Beam pointing only correlated by -0.15, which is only very weakly, which indicates that since the interaction occurs within the plasma that the beam is still evolving and has a transverse beam component that only freezes later in the channel. The negative value indicates there could be an inversion of the signal, but this will be investigated later. There is also the possibility that the beam also oscillates which should be visible by comparing spatial features.
...
EITHER beam is there and emits

OR beam is on other side and is pointing downwards
...
\begin{figure}
\centering
\includegraphics[width=0.6\columnwidth]{linICS_Inversion_Sketch.pdf}
...
Due to shock injection we might be in a fixed phase (downwards) so we are measuring the momentum component.

Hard to say as there are not as many shots with clear features like these.

Result: definitely interact outside of the plasma if want to measure properties of the frozen beam. Otherwise consider scanning more in time to see any developments. Shock injection (if fixed phase) could be a useful method to scan through the oscillation (if unperturbing).

\EliasComm{Add density profile from Cary, maybe increase in density leads to focusing of the beam, so we are inverted.}
