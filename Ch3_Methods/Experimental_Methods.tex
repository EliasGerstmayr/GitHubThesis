...
The following chapter introduces a selection of experimental methods and components relevant in the context of the LWFA experiments presented in the course of this work. The author aims to cover the three main components of an LWFA experiment: high-intensity lasers, gas targetry and diagnostics for particles and secondary radiation.

The chapter starts by introducing high-intensity lasers, the main tool to drive and probe the plasma wave, how to diagnose them in experiment and some basic terminology. It continues with a section on the technique of chirped-pulse amplification (CPA) \cite{Strickland1985}, a key development that enabled the realisation of high-intensity lasers now reaching peak intensities in excess of $10^{22}\,\mathrm{W}\,\mathrm{cm}^{-2}$. A breakthrough Strickland and Mourou were awarded the Nobel Prize in physics in 2018 for.
...
Finally, the author discusses techniques used to diagnose the plasma, the interaction of laser and plasma with each other as well as particles and radiation generated in the process.


\section{Laser Systems}

Since the experimental realisation of the maser (REF), shortly followed by the laser in 1960 \cite{Maiman1960} lasers have taken an indispensable role in a wide range of fields of communication, construction, navigation, medicine, machining, defense and numerous applications in science and innovation. 
...
Top hat profile/Flat top?

B-Integral


The research in wakefield acceleration involves the work with lasers as driver for LWFA, scatterer or heater, or probe to diagnose the plasma. A good understanding of optics and beam propagation is essential for setting up imaging systems, aligning and estimating properties like intensity, focal spot sizes and the guiding of the beam.

The following section will briefly introduce a few basic terms used to describe the properties of those lasers later in this work and some equations from Gaussian optics that are useful in this context as well.
\vspace{\baselineskip}
...
\paragraph{The divergence} of the beam $\theta$ several Rayleigh lengths from focus is approximately given by the ratio of the waist to the Rayleigh length
\begin{equation}
...
We can use these equations now, for instance, to estimate the conditions to expect at the Astra Gemini laser. Gemini delivers roughly $15\,\mathrm{J}$ on target at a pulse duration of $45\,\mathrm{fs}$, i.e. a power of a bit over $300\,\mathrm{TW}$. The standard setup LWFA uses a focusing optic with an F-number of $F = 40$ ($f=6\,\mathrm{m}$,$D=0.15\,\mathrm{m}$) at a central wavelength of $\lambda = 0.8\,\mu m$.

This gives a focal spot waist of $w_0 = XX \mathrm{\mu m}$ and a peak intensity $I_0 = XX \times 10^{20} \,\mathrm{W}\,\mathrm{cm}^{-2}$ and $a_0 \approx XX$

A typical focusing optic for the second Gemini beam as scatterer has an F-number of $F = 2$, which in is by a factor $10$ smaller, leading to a $10$ times smaller focal spot with $2 w_0 \approx 2\,\mathrm{\mu m}$, a $100$ times higher peak intensity $I_0 = XX\times 10^{20}\,\mathrm{W}\,\mathrm{cm}^{-2}$ and finally a $10$ times higher peak normalised vector potential $a_0 \approx XX$.
\vspace{\baselineskip}
...
\EliasComm{Real lasers are typically not Gaussian.}

\EliasComm{Wavefront}

\EliasComm{Radius of Curvature}

\EliasComm{Guoy Phase}
...
In laser amplification a laser pulse (single pass) passes once or several times (multipass) through an amplifier crystal, called the laser gain medium. To generate ultrashort pulses a large bandwith gain medium is required in order to amplify continuously over a large frequency domain. A popular choice are lasers based on titanium-doped sapphire (Ti:Sa) as gain medium. At these short pulse durations and high intensities non-linear effects such as relativistic self-guiding start to occur, leading to a local increase of the field strength. At very high field strengths the gain medium starts to ionize and in turn to take damage: amplification of these short pulse lasers is limited by this breakdown. Further amplification can then only be achieved if the beam is expanded for which then a larger gain medium becomes necessary. This limitation was overcome or at least pushed further by the invention of chirped-pulse amplification (CPA) \cite{Strickland1985} based on a technique employed in radar technology.
\vspace{\baselineskip}
...
CPA exploits this fact and stretches the pulse in time using sets of refraction gratings: depending on the frequency of the light it will be refracted differently, extending or shortening the path relative to other frequencies. In the pioneering work of Strickland and Mourou a long glass fibre was used to stretch the pulse \cite{Strickland1985}. A pair or a couple of grating pairs can in this way stretch the pulse with a dependence on wavelength: the pulse is `chirped'. The energy density is now much lower than before and the pulse in total can be amplified to much higher intensities without damaging or saturating the gain medium. In the next step, a set of gratings reverse the process and compress the pulse, achieving a high intensity but also an ultrashort pulse duration that is ideal for LWFA.
\vspace{\baselineskip}
...
\subsection{Wavefront Sensor}

Shack-Hartmann (?) sensor. HASO.

Array of micro lenses

Temporally integrated wavefront profile of the laser pulse

In combination with an adaptive optic (technique from astrophysics) used to flatten the wavefront to enable reaching the diffraction limited focus.
\EliasComm{Add picture.}

\subsection{Pointing Diagnostics}

Monitoring shot-to-shot fluctuations and long-term drifts.


\subsection{Focal Spot Camera}

The focal spot camera is a diagnostic to characterise the focal spot of the laser. A special microscope objective magnifies and images the focal spot onto a camera chip. The energy distribution, shape and size of the focal spot has been shown to be a crucial factor to couple energy from the laser pulse to the wake. Also the position of the focus, i.e. the point of highest intensity, is important to know in order to reach the non-linear regime and to correctly analyse the situation in experiment realistically.
In the context of colliding-pulse experiment the shape of the focal spot of the scattering laser is also of interest as a precise knowledge of the wavefront and intensity distribution is required for an accurate model of the interaction.
...
\subsection{Dazzler and FROG}

Measure the pulse duration of the laser and the shape. Control over the shape of the laser pulse (rise, compression).

\subsection{Timing Diagnostics}

Relative timing of laser beams, for probe and main beam or dual beam experiments.

\subsubsection{Diode Timing}

To time laser pulses to an accuracy of few picoseconds fast silicon photodiodes combined with fast scopes are an appropriate tool.
...
In experiment the fast diode EOT.... with a rise time of ... ps and a fast WaveMaster ... was used. This limits the resolution of the signal to about ... ps. The accuracy of the difference measurement is per measurement is a few picoseconds. If several measurements at different separations (using a delay stage to delay one laser arm) are taken the error can be reduced to an accuracy of ....
...
\subsection{Moire-Deflectrometry}

Based on ray tracing. Measure density gradients. 

\subsection{Wavefront sensor}

PHASICS. Wavefront changes can be used to estimate density of plasma channel.


\subsection{Self emission of the plasma}

Measuring the self emission of the plasma channel. Pretty Pic. Top view on channel. Less quantitative or ultrashort diagnostic.
...
Imaging plate and overlap with lanex measurement.
Account for imaging system and collection angle.

ICT Turbo and EMP problems.
...
One simple but effective way to measure the source size of an X-ray source is using a pinhole camera. An X-ray pinhole camera works the same way as its optical counterpart: the X-ray sensitive detector e.g. a CCD and the source are separated by an opaque wall only with a small hole (smaller than the source) to let light pass through. The magnification M is determined by the ratio of distances detector to pinhole and pinhole to source:

\begin{equation}
M = (a + b)/a,
\end{equation}

where $a$ is the distance source to pinhole and $b$ the distance pinhole to detector.

Measuring these distances carefully one can then derive the source size and from shot-to-shot fluctuations in position and photon flux.

To shield the CCD from ambient light the chip is typically covered with a thin beryllium window which is transparent to most X-rays but opaque to visible light. Depending on the range of X-rays one is interested the pinhole can be covered with plastic or other low-Z material to filter out low energy radiation.

The commonly used cameras for experiments at the Central Laser Facility (UK) are Andor DX420 with a ... thick beryllium window.


\subsection{X-ray Crystal Camera}

To measure the spectrum of X-rays there are typically two routes: one is measuring the transmission of the radiation through different materials and derive its energy from this. The second is diffracting the X-rays on a crystal based on the Bragg condition and measuring the diffracted components.

Latter method allows a measuring a narrow energy range at high resolution bound to available crystal lattices. Towards higher X-ray energies around tens of keV it becomes more difficult to use this method as crystals with the right structure and Bragg condition are rare.

\subsection{X-ray Filter Pack}

When reaching higher X-ray energies the transmission through materials becomes a more feasible and flexible tool to measure the X-ray spectrum at high energy and broad bandwith.
The composition of an array of different Z materials and thicknesses is sometimes referred to as `filter pack'. Each component has a distinct transmission profile for X-rays with sharp edges around electron absorption edges (K-edge). When selecting a suitable range of materials with K-edges in the desired energy range, a comparison of the signal levels through the filters relative to each other can deduce the underlying spectrum. In some cases pairs of materials are exhibiting almost the same transmission behaviour except for a slightly shifted position of their K-edge. A subtraction their transmission from each other tells the experimentalist the number of photons in a narrow energy bandwidth. These pairs are referred to as `Ross filter pairs'.

The signal can be detected using X-ray sensitive CCD cameras directly in the beam path (direct detection). An indirect detection method is relying on scintillators that are then imaged with a sufficiently sensitive camera.

This method works well for soft X-rays, in a regime where the transmission is very dependent on the material. For harder X-ray spectra in the MeV range the transmission is almost constant for most materials and interaction with matter can result in the production of electron-positron pairs which then in turn produce secondary radiation.

The author developed several filter packs over the course of his studies, two for application at the Astra Gemini laser and one filter pack that was installed at the FlashForward facility at DESY in Hamburg, Germany. This filter pack is also compatible with the ultra-high vacuum of the accelerator beam line.
...
Convert gamma rays into electron-positron pairs and measure the spectrum to deduce the gamma spectrum.
Works for several MeV radiation but becomes less sensitive at higher energies (REF?).
