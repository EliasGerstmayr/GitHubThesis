...
Firstly, a long-focal-length geometry using either an f/40 spherical mirror or OAP with $f = 6\,\mathrm{m}$ to provide an intense laser pulse $a_0 > 1$ with a long Rayleigh length to drive LWFA. Due to the long focal length and the restricted size of the target area the beam is about halfway through the focusing process `folded'/reflected back. Unfortunately, this means that the usable energy in this laser arm is limited by the damage threshold of these `folding' mirrors, which is highly dependent on the actual beam quality, beam size before focus and at the mirrors, cleanliness and the quality of the coating.
...
\EliasComm{Provide a sketch of a full experiment drawing (colour 3D)}
...
At Gemini the laser energies are measured by cross-calibrating the counts on a camera looking at the transmitted beam through an HR optic with a Gentec calorimeter. The Gentec is also used to then measure the throughput of the compressor and the energy reaching TCC.

A typical compressor throughout is 60-70 percent.

\subsection{Laser Pulse Duration Measurement}

Can be measured using autocorrelator, SPIDER, FROG, for instance.

At Gemini a FROG is used.

The Frequency-resolved optical gating (FROG) is a diagnostic to measure the intensity and spectral phase of ultrashort laser pulses as for instance present at Gemini ($\sim 40\,\mathrm{fs}$). The setup is comparable to an autocorrelator and is optically gated by combining two beams in a non-linear medium. Typically SHG is the non-linear medium of choice. The produced SH radiation is then spectrally resolved (hence frequency-resolved) and provides information about the spectral phase.
\vspace{\baselineskip}

At Gemini a FROG trace can be taken on every shot using beams transmitted through HR mirrors.

More details found in FROG BOOK TREBONI? MJV STREETER's THESIS.
...
For each of these applications it is important to characterise the the size and energy distribution of the focal spot. In combination with a measurement of the pulse shape, for instance using a FROG measurement, this provides an intensity map of the laser pulse at focus.
\vspace{\baselineskip}

In this work a CCD camera, an AVT Manta, is coupled to a near infrared (NIR) apochromatic long-working-distance microscope objective (depending on the application Mutotoyo X10 or X20 magnification) to image the spot at its focal plane. 
...
Having calibrated the camera spatially one can then image the focal spot. A thin wire (50-75 microns) can be used to fix the focal plane to a well-defined position. An example of a focal spot from a beam focused with an f/40 optic is shown in figure XXXX.
...
Comparing the pixel counts within the FWHM contour against the pixel counts in the total beam (if the background subtraction was successful the total counts on the image should suffice), one can then put a number on the fraction of energy contained in the FWHM of the beam. This number is important as a lot of energy in the beam will not efficiently drive a wake or scatter well if most of the energy is in the outer wings of the beam which will not be guided and diffract.
...
However, too many aberrations or distortions in the beam prevent it from focusing down to a diffraction-limited spot but result in energy being distributed in wings and less energy being contained within a small spot. This means that the intensity reached is lower and a lot of energy is not efficiently used in the process.  Aberrations or other inhomogeneities impose an additional limit on the minimum size of the focal spot and hence the maximum intensity achievable.
\vspace{\baselineskip}

In addition to ensuring that the beam propagation and source is as close to ideal as possible, one can use a special camera, a wavefront sensor, to characterise the wavefront of the beam. In combination with a deformable mirror or an adaptive optic (AO) a software can deduce a map of which change in the mirror relates to what kind of change in the wavefront. Having characterised this the mirror can be used to flatten the wavefront iteratively to correct for the wavefront errors which makes consequent modelling easier, matching to simulations, and enables reaching a diffraction limited focus at higher intensities.
On the other hand, other applications might benefit from additional aberrations and this method can actively add those.
E.g. astigmatism for radiation reaction (Baird), wavefront tilt for betatron radiation (Mangles).
\vspace{\baselineskip}
...
`Timing' two beams means in this case that the relative path lengths are adjusted to compensate for the measured difference in the time of arrival at the interaction point. At Gemini larger distances (ns) require moving optics physically to extend the beam path, smaller distances are finely adjusted by using a linear translation stage that adds or removes path length in double-pass at few femtosecond precision.
...
\EliasComm{Add a picture and scope image.}
...
Once the vacuum in the target chamber reaches a sufficient quality and the turbo pumps are running, the gate valves are opened.
The gate valves at Gemini are made out of sapphire glass but is of different thickness on each arm, see RAL report 2005 D Carroll and CD Murphy. Difference around $60 \pm 3\,\mathrm{\mu m}$.
...
The previous two examples can be cross-checked by repeating some of the timing procedures, for instance spatial interferometry, after the pump-down and adding the correction factors.

In some cases, however, material might be inserted asymmetrically to attenuate the beams. At Gemini glass slides are available to attenuate beams by a factor $\sim 100$ and will be used in conjunction with polarisers and waveplates to improve the contrast of the interference pattern.
...
Recently, the attenuators at Gemini have been matched with `compensator plates' that will be in the beam path when the attenuators are not. The compensator plates and the attenuators are matched, such that introducing an attenuator should not change the delay at all and asymmetric attenuation also would not change the relative delay. The accuracy of this setup, however, also has to be investigated and quantified.
...
Finally, we want to highlight what to consider when wanting to time a laser to an electron bunch produced through LWFA.

One has to consider several factors:

The driver beam propagates through a plasma and is hence slower relative to the in vacuum-timing procedure: it will arrive later at the interaction point than measured previously.
...
Gas and liquid targets offer the advantage of being used in high-repetition experiments as they can be destroyed and replenished in a short amount of time without significant re-alignment and debris production. The limiting factors are the performance of the vacuum system, the time it takes the gas flow to reach equilibrium, the durability of nozzles/casings etc. and the repetition rate of the laser system in use. Facilities like FLASHForward that have in principle the capabilities and the ambition to operate at MHz repetition rate even have to consider ion and plasma dynamics and scintillation times for detectors.
\vspace{\baselineskip}
...
As listing and explaining all those different targets would be a hopeless task, the author will limit himself to focus on examples of gas jet and cell specimen relevant to the work presented later in this thesis.

\subsection{Gas Jets}

\subsubsection{General}
The burst of gas produced when forcing gas with high backing pressure through a small orifice at the bottom of a diverging cone is in the context of this work related to as gas jet.
If the measurements of the cone are chosen correctly at a high enough backing pressure, the flow detaches and becomes supersonic.
Supersonic gas jets are able to produce relatively smooth flat-top density profiles with short density ramps on both sides.
...
Gas jets provide a relatively easy target, diverse in shape and comparatively straightforward to align. The open geometry also allows optical probing from a large solid angle. However, experimental results are less stable (shot-to-shot reproducibility) and inferior in terms of maximum energy \cite{Leemans2014} and stability \cite{Desforges2014,Osterhoff2013} to set-ups relying on gas cells or capillaries at similar conditions, especially at low densities, as the medium is laminar and reproducible. ALSO MENTION MAX ENERGY FROM JET (4GEV CORELS)
\vspace{\baselineskip}
...
The gas nozzles used in experiment are conical aluminium nozzles designed to produce supersonic gas flows. Orifice size is 1 mm wide and diverges over a certain height to its maximum diameter.
...
Here some references for wire, silicon, blade etc.
...
Debris can coat the glass windows and reduce the efficiency of optical diagnostics.
\vspace{\baselineskip}
...
The author will mention three examples of gases each related to a different injection mechanism introduced earlier, but similarly as there is a variety of different target designs other media are used as well.
\vspace{\baselineskip}
...
The third example is methane. When being cooled down some gases start to form clusters held together by the Van-der-Waals force. These clusters are compounds ranging in size from tens to several hundreds of molecules, and locally increase the density, in some cases beyond the critical density, with respect to remaining unclustered background medium (REF). This is comparable to nanparticle-assisted injection mechanisms (REF).
...
\EliasComm{Add image of cross sections to indicate why Compton spectrometer is not reliable above a certain energy of tens of MeV.}
...
\begin{tabular}{l|l|l|l|l}
Name & Crystals & Crystal Dimension & Front Plate & Divider\\ \hline \hline
RAL & $47 \times 33$ & $5 \times 5 \times 50$ & Steel 9 mm & Al 1 mm\\
RAL & $47 \times 33$ & $5 \times 5 \times 50$ & PTFE 1 mm & Al 1 mm\\
Dual & $10 \times 70$ & $5 \times 5 \times 50$ & Al 2 mm & Plastic X mm
\end{tabular}
