...
All of the experimental results were acquired at Target Area 3 using the Astra-Gemini laser of the Central Laser Facility (CLF) at the Rutherford-Appleton Laboratory (RAL), UK.
\vspace{\baselineskip}

The titanium-sapphire doped (Ti:Sa) Gemini laser provides two beams up to every 20 seconds with central wavelength $800\,\mathrm{nm}$, a compressed pulse duration of $45\,\mathrm{fs}$ \textsc{fwhm} and up to $15\,\mathrm{J}$ energy on each arm, making it a dual-beam 300-TW laser facility on paper. The pulse duration can be adjusted using a Dazzler or if a ps scale laser pulse is required the gratings can be de-optimised on purpose.
\vspace{\baselineskip}

The target area Gemini supplies with the lasers is housed in a concrete bunker with lead door. 
In the target area, named Target Area 3 or short TA3, the two beams enter the main vacuum chamber through separate pipes in the ceiling. Sapphire gate valves separate the vacuum chamber, often at air, from the compressor vacuum upstairs, and are only opened when the vacuum reaches sufficient quality.

The aluminium main chamber is roughly sized XX times XX times XX and equipped with an aluminium breadboard.
The laser beam lines, diagnostics etc. are freely designed and adjusted from each experiment to another.

The limiting factor is the space to use long-focal-length optics a folding mirror is required. The energy is limited by the damage threshold of these mirrors.
In addition, limiting factors in the laser chain make this laser system efficiently something closer to 200-TW.
...
Due to the longer timescale of recombination (microseconds) this is not an ultrafast diagnostic.
...
The electrons are emitted with a momentum distribution that is typically dominated by the component in the direction of the laser propagation. Additional transverse components are present, for instance visible in betatron oscillations, and lead to a divergence of the beam, typically a few $\mathrm{mrad}$.
...
To obtain an absolute number the screen has to be spatially calibrated. This includes the relative angle of screen and camera.
A regular grid, for instance printed on some paper, can be used as spatial reference to evaluate this. The angle of the screen relative to the beam is also important. One way to compensate both is to place the screen at 45 degrees to the beam and the camera looking at 45 degrees as well restoring the right dimensions.
Sometimes a thin layer of aluminium foil is added at the back as protection against laser light.
