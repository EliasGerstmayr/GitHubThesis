...
Since the experimental realisation of the maser (REF), shortly afterwards followed by the laser in 1960 \cite{Maiman1960}, lasers have taken an indispensable role in a wide range of fields covering communication, construction, navigation, medicine, pharmaceuticals, machining, defense, numerous applications in science and innovation and more. 
...
\paragraph{The radius of curvature} can be expressed as
...
We can use these equations now, for instance, to estimate the conditions expected at the Astra Gemini laser where the experiments presented in this work were performed at. Gemini delivers a maximum of $15\,\mathrm{J}$ on target at a pulse duration of $45\,\mathrm{fs}$, i.e. a power of a bit over $300\,\mathrm{TW}$. The central wavelength of the Ti:Sa based system is  $\lambda = 0.8\,\mu m$. 
The LWFA setups presented in this work use a focusing optic with an F-number of $F = 40$ (focal length $f=6\,\mathrm{m}$, beam diameter $D=0.15\,\mathrm{m}$) for the beam driving the wake.
...
A typical focusing optic for the second Gemini beam as scatterer has an F-number of $F = 2$ (focal length $f=0.3\,\mathrm{m}$, beam diameter $D=0.15\,\mathrm{m}$ , which is by a factor $10$ smaller, leading to a $10$ times smaller minimum focal spot with $2 w_0 \approx 2\,\mathrm{\mu m}$, a $100$ times higher peak intensity $I_0 = XX\times 10^{20}\,\mathrm{W}\,\mathrm{cm}^{-2}$ and finally a $10$ times higher peak normalised vector potential $a_0 \approx XX$.
\vspace{\baselineskip}


\EliasComm{Real lasers are typically not Gaussian. Aberrations.}
...
In laser amplification a laser pulse passes once (single pass) or multiple times (multipass) through an amplifier medium, called the laser gain medium. To generate ultrashort pulses a large bandwith gain medium is required in order to amplify continuously over a large frequency domain. A popular choice are lasers based on titanium-doped sapphire (Ti:Sa) as gain medium due to their large bandwidth and tunability. At short pulse durations and high intensities non-linear effects such as relativistic self-guiding start to occur, leading to a local increase of the field strength. At very high field strengths the gain medium starts to ionize and in turn to take damage: amplification of these short pulse lasers is limited by this breakdown. Further amplification can then only be achieved if the beam is expanded for which then a larger gain medium becomes necessary. This limitation was overcome or at least pushed further by the invention of chirped-pulse amplification (CPA) \cite{Strickland1985} based on a technique employed in radar technology.
\vspace{\baselineskip}
...
In experiments involving two or more pulsed beams the beams have to be timed to each other within their combined pulse duration.
...
In experiment this means that introducing or removing media asymetrically from beam paths will change the relative timing.
...
Glass in the beam: gate valves at Gemini are made out of sapphire glas. The glass is of different thickness, see RAL report 2005 D Carroll and CD Murphy.
...
One simple but effective way to measure the source size of an X-ray source is using a pinhole camera. An X-ray pinhole camera works the same way as its optical counterpart: the X-ray sensitive detector e.g. a CCD and the source are separated by an opaque wall only with a small hole (smaller than the source) to let light pass through. The magnification M is geometric and hence determined by the ratio of distances detector to pinhole and pinhole to source:
...
The smaller the size the sharper the image but the signal becomes weaker. Optimum size is...
...
Specs DX420: (REF Andor)
1024 x 255 pixel
pixel 26 um x 26 um
Image area 26.6 mm x 6.7 mm

Alignment using alignment spikes that are then replaced with the pinhole.

\begin{figure}
\centering
\includegraphics[width=0.8\columnwidth]{20180406r002s023_PinholeCam.jpg}
\caption{Example of a Pinhole image. Double pinhole. Magnification M. Pixel at ..., so source size was... Need to work out all these things and plot with line outs in both axes. Compare with a beam profile from the heater beam.}
\end{figure}
...
A list of crystals being used in experiments: HOPG.
...
\subsubsection{GEANT simulations?}
