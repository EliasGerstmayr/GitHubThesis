...
Finally, the techniques used to diagnose the plasma, the interaction of the laser with the plasma as well as particles and radiation generated in the process are discussed.
...
The typical compressor throughput at Gemini is 60-70 percent.
...
More details found in Matthew Streeter's thesis \cite{StreeterThesis} and \cite{FROG}.
...
Gas jets provide a relatively easy target, diverse in shape and comparatively straightforward to align. The open geometry also allows optical probing from a large solid angle. However, experimental results are less stable (shot-to-shot reproducibility) and inferior in terms of maximum energy \cite{Leemans2014} and stability \cite{Desforges2014,Osterhoff2013} to setups relying on gas cells or capillaries at similar conditions, especially at low densities, as the medium is laminar and reproducible. ALSO MENTION MAX ENERGY FROM JET (4 GEV CORELS) \cite{Kim2013}
\vspace{\baselineskip}
...
In Chapter XX a conical nozzle with $1\,\mathrm{mm}$ orifice diverges over $19.7\,\mathrm{mm}$ to its maximum diameter of $15\,\mathrm{mm}$ (opening angle $\theta = 19.56^\circ$)\footnote{designed by Stuart Mangles, Imperial College}.
In Chapter XX a conical nozzle with $1\,\mathrm{mm}$ orifice diverges over $23.3\,\mathrm{mm}$ to its maximum diameter of $15\,\mathrm{mm}$(opening angle $\theta = 16.72^\circ$)\footnote{designed by Stefan Kneip, Imperial College}.
...
Just as 3D printing methods have been considered for gas jets \cite{Jolly2012_3DJET} researchers have demonstrated that this is also possible and feasible for gas cell designs \cite{Vargas2014} AMINA REF AS WELL FOR NEWER DESIGN
\vspace{\baselineskip}



Gas cell targets have sometimes produced superior results from gas jets using the same laser system (Poder v Kneip, Osterhoff, \cite{Kuschel2018_SELF}) in terms of charge, shot-to-shot stability and maximum energy gain.
However, some of the most stable LWFA sources have been achieved using gas jet targets (Faure Nature 2006) and the open geometry allows for diagnostic access and colliding pulse experiments as well as a route to high repetition rate operation.
...
Three examples of gases are presented, each related to a different injection mechanism introduced earlier in the \nameref{Chap:Theory}. Similarly as there is a variety of different target designs other media are used as well.
\vspace{\baselineskip}
...
In Chapters XX RR19lin, RR19betamax helium was used in a gas jet with a blade (shock injection).

In Chapter XX BW18 helium with nitrogen dopant was used (ionisation injection) in a gas cell.

In Chapter XX RR15 helium was used in a gas jet, indications of self-injection and shock-injection.
...
\end{figure}
From Jason's thesis \cite{ColeThesis}:
...
\caption{Example of a shadowgram (top) and an interferometry image taken on an experiment run at TA2 (Central Laser Facility, RAL) in Spring 2016. The image shows a plasma channel produced by the Astra laser pulse in a gas cell of $3\,\mathrm{mm}$ length filled with helium. The density is relatively high leading to strong scattering and filamentation of the channel.}
\end{figure}
...
\subsection{Raman Side Scattering}

Jason and Y Ma paper?
...
\caption{Visualisation of an electron deflected in a homogeneous circular magnetic field (grey) with radius $r_b$. The larger green circle indicates the Larmor radius, determining the electron trajectory (blue) from the point of entry into the field (A) to its exit point (C) resulting in a total deflection $\theta$. Bottom: Example of particle tracking through a three-piece magnet for electrons of -X, 0 and +X MRAD pointing.}
\end{figure}
...
Pinhole camera. Simplest diagnostic.
...
\caption{Pinhole Cam Example Image. Replace this by integrated images to indicate the spot size and spectral content?}
\end{figure}
...
\caption{Crystal Spec Example Image, K-Edge from aluminium filter. X-axis is dispersion direction..}
\end{figure}
...
More details on the extraction procedure can be found in Jon Wood's thesis \cite{WoodThesis}.
...
If we assume only absorption following Beer's law, the signal through a filter on the camera including quantum efficiencies (QE) etc. (taken from Jon Wood's thesis \cite{WoodThesis})
...
\caption{Examples of profile along with GEANT representation. Example of gamma profile from experiment.}
\end{figure}
...
QUB & $20 \times 20$ & $2 \times 2 \times 20$ mm & Al 10 mm & Al 0.1 mm\\
Jena & $45 \times 45$ & $1 \times 1 \times 10$ mm & TiO2 0.5 mm & TiO2 0.2 mm\\
DESY & $30 \times 30$ & $1.5 \times 1.5 \times 10$ mm & Epoxy 0.5 mm & Epoxy 0.2 mm\\
RAL & $47 \times 33$ & $5 \times 5 \times 50$ mm & None & Al 1 mm
\end{tabular}
...
In the range of few to maybe 30 MeV the cross section rises rapidly (SEE FIGURE XX NUMBER) compensating for the fewer pairs at higher energies and making this a feasible spectrometer to resolve the full spectrum in this range (SEE GIANLUCA DESIGNS HERE. DJ CORVAN REV SCI 85 (2014)). At higher energies the cross-section flattens which means that the contribution to the total production of pair equalises over all energies but due to the exponential spectrum of the pairs produced very few pairs at the maximum photon energies are produced.
...
\caption{Compton spectrometer AS TAKEN FROM CORVAN REF. PERMISSION IS FREE BUT REQUIRES TO SUBMIT FORM}
\end{figure}
...
For setups that have more signal on the chips, a `quiet' corner can be used to estimate the background and subtract the average pixel value. A median filter can be used to smoothen out the image and remove hot pixels.
...
The real measurement is the energy deposition in the crystals along the longitudinal axis. Due to the spacers there is non-signal area.
We can mark the position of the crystals as they do not move from shot to shot. Take the average value of each of the crystals.

Width is the angular divergence.

Here some estimate why slight angle is not important for spectral retrieval. Thickness of crystal and typical pointing/divergence.

\subsubsection{Stitching together dual-axis}

In case of the dual axis detector, the detector is so long that only a part of it is being imaged. The parts are combined by using references.
The signal levels are not adjusted and are done as part of the correction factor.

The references from the images are used to mark a suitable line where crystal rows overlap.

Based on the reference ruler the number of rows can be assigned.

The scintillation signal with individual crystal extraction is shown.

Signal levels will be different.

Side on and top were imaged simultaneously to make full use of the chip. Only one part can be in focus and the stitching together adds complexity.

\begin{figure}
\centering
\includegraphics[height=.5\columnwidth]{DualAxisFrontExample.png}\includegraphics[height=.5\columnwidth]{DualAxisRearExample.png}
\includegraphics[width=1.0\columnwidth]{DualAxisCrystalExtrExample.png}
\caption{Dual axis images.}
\end{figure}


\subsubsection{Simulation Code and Functionality}
...
\subsubsection{Bremsstrahlung simulations}
...
As seen in Figure XX there are brighter pixels, these pixels often remain brighter or less due to the quality of the crystal.

In this simulation realistic (average) electron spectra measured in the respective experiment were launched at a converter foil at distance...
...
Measuring conversion of electrons into bremsstrahlung and response for experiment factors.

Compare measured bremsstrahlung detector response with simulations and correct this factor.

This gives a correction factor encapsulating viewing angles and crystal variations and so on.
\EliasComm{Add figure of experimental spectrum, sim spectrum and correction factor.}
\begin{figure}
\centering
...
\subsubsection{Mono-energetic response}
...
\subsubsection{Comparison of scintillator array responses}
...
\subsubsection{Combining responses to spectra}
...
Explain more about matrix multiplication.

Diagram: VECTOR MATRIX THEN INTERPOLATE THEN GET WEIGHT FROM EXPONENTIAL FUNCTION OR ANY SPECTRAL FUNCTION AND MULTIPLY WITH RESPONSE MATRIX.



\subsubsection{Determining spectrum by fitting}

Assemble spectra by matrix multiplication assuming a certain shape, then use the combined response (weighed by the spectrum) as guesses.
Use a least square fit to compare that to the experimental data (corrected with the brems factor) to then nail down a one-parameter critical energy or similar.

\EliasComm{Add here 2D contour plot showing the parameter fit (amplitude and critical energy).}