...
In the context of LWFA the laser is focused down to reach high enough intensities to drive a wake and to guide itself over a long range. The second application in this context is using the laser as scatterer which also requires focusing the beam down to a small volume with high energy density.

A CCD camera is used to image the focal spot and characterise its size and energy content. Due to the small size of the focal spot, typically around 30 to 50 microns or around few microns, microscope objectives are employed.
...
\EliasComm{Present typical focal spot analysis. How much energy is in spot?}

Spatial Calibration

As the exact size of the focal spot is important, the focal spot camera is spatially calibrated. This can be done by carefully measuring the distances between the components of the optical system and knowing about their properties. Alternatively, the camera can be used to image an object of well-defined spatial extent. A typically used tool are USAF targets provided by Thorlabs. Knowing the pixel size of the camera one can then relate the width of the lines to a real spatial extent.

Estimating the focal spot size

Having calibrated the camera spatially one can then image the focal spot. Typically a thin wire is used to fix the focal plane to a well-defined position.
There are various ways to approach the problem. One can take the maximum value of the distribution and find the boundary when the value drops to half of that value or whatever threshold one decides on. Fitting an ellipse to it one can then find the spatial dimensions.

Estimating energy contained in the FWHM

Comparing the number of counts in the ellipse with the counts on the image, one can estimate how much of the total energy of the beam is contained within the focal spot.

Under the assumption that the intensity is proportional to counts on the CCD, knowing the energy contained in the beam and its pulse duration one can then calculate the intensity of the laser pulse. 

All of this is done under a lower intensity version of the beam as a high intensity laser pulse would damage the camera and optics.


\subsection{Wavefront Sensor and Adaptive Optics}

The wavefront of a laser can be distorted by aberrations in the system, thermal effects, or others.
Aberrations or other inhomogeneities impose and additional limit on the minimum size of the focal spot and hence the maximum intensity.

A trained eye can spot the different aberrations when looking at the focal spot and attempt to find the source (e.g. wrong angle onto off-axis parabola can introduce astigmatism).
...
In this work a Shack-Hartmann sensor or company HASO was used. This type of wavefront sensor is an array of microlenses.
...
Using this method one can attempt to optimise the wavefront as such to reach a diffraction limited focus.
...
Two identically chirped pulses are being spectrally dispersed using a grating. The individual spectral components can overlap.

Visible over hundreds of femtoseconds and hence over a much larger time duration than for spatial interferometry.
...
Timing two beams to each other down to the femtosecond level comes with challenges and considerations one has to do.
