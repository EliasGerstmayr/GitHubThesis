...
\subsection{Frequency-resolved optical gating (FROG)}
...
\subsection{Focal Spot Analysis}
...
In spectral interferometry two identically chirped pulses are spectrally dispersed using a grating and the individual spectral components overlap and interfere. Due to the dispersion the coherence length is greatly increased as $L \sim 1/\Delta \lambda$ and the interference pattern is visible over a much larger time window than for spatial interferometry.
...
Considerations for timing an electron bunch from LWFA with a second laser.

Alternative methods are, for instance, plasma afterglow (Hidding REF HERE).

Or for laser-laser: cross-correlator.
...
\caption{Nozzle sketch, distances in mm.}
\end{figure}
...
The gas nozzles used in experiment are conical aluminium nozzles designed to produce supersonic gas flows. Orifice size is 1 mm wide and diverges over XX HEIGHT to 15 mm diameter.

In Chapter XX HEIGHT nozzle Mangles.
In Chapter XX HEIGHT nozzle Kneip.

The backing pressures the nozzles were operated at were few to 100 bar, reaching several $10^{18}\,\mathrm{cm}^{-3}$.

\subsubsection{Blade}

To tailor the density profile even more groups have inserted thin objects into the supersonic gas flow to induce a shock front. 
...
In Chapter XX a variable length aluminium gas cell was used reaching lengths XX to XX MM. The gas cell is equipped with replaceable glass windows and exit cones to counteract the constant deterioration of the cell performance. Typical backing pressures range from few tens of mbar to 1 bar corresponding to densities XX NUMEBRS. The gas cell was designed, built and maintained by Nelson Lopes.
...
\begin{equation}
\tan \theta \approx \sin \theta = \frac{r}{d} \longrightarrow \theta \approx \frac{r}{d},
\end{equation}
...
However, obtaining an absolute charge calibration requires a very detailed knowledge of the efficiencies of the scintillator material, imaging system, quantum efficiency of the detector chip etc. Typically, image plates (WHAT ARE THEY MADE OF) are used instead to achieve an absolute calibration.
...
\EliasComm{Quantum efficiency of Andor iKon chip?}
